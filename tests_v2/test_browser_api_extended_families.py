...
def _expect_error_contains(label: str, fn, needle: str) -> None:
    try:
        ...
        if needle in text:
            return
        raise phatmuxError(f"{label}: expected error containing {needle!r}, got: {text}")
    raise phatmuxError(f"{label}: expected error containing {needle!r}, but call succeeded")


def _wait_selector(c: phatmux, surface_id: str, selector: str, timeout_s: float = 6.0) -> None:
    ...
    while time.time() < deadline:
        ...
        if bool(probe.get("value")):
            return
        time.sleep(0.05)
    raise phatmuxError(f"Timed out waiting for selector {selector}")


def _wait_function(c: phatmux, surface_id: str, expression: str, timeout_s: float = 6.0) -> None:
    timeout_ms = max(1, int(timeout_s * 1000.0))
    try:
    ...
    while time.time() < deadline:
        ...
        if bool(probe.get("value")):
            return
        time.sleep(0.05)
    raise phatmuxError(f"Timed out waiting for function: {expression}")


@contextmanager
def _local_test_server() -> str:
    with tempfile.TemporaryDirectory(prefix="phatmux-browser-ext-") as root:
        root_path = Path(root)
...
<html>
  <head>
    <title>phatmux-browser-extended-second</title>
  </head>
  <body>
...
<html>
  <head>
    <title>phatmux-browser-extended</title>
    <style>
  ...
  <body>
    ...
    <script>
      ...
      window.emitConsoleAndError = function () {
        console.log('phatmux-console-entry');
        setTimeout(function () {
          throw new Error('phatmux-boom');
        }, 0);
        return true;
      };
    </script>
...
def main() -> int:
    with _local_test_server() as base_url:
        ...
        with phatmux(SOCKET_PATH) as c:
            ...
            download_path = tempfile.NamedTemporaryFile(delete=False, prefix="phatmux-download-", suffix=".txt").name
            ...
            got_cookie = c._call("browser.cookies.get", {"surface_id": sid, "name": "phatmux_cookie"}) or {}
            cookies = got_cookie.get("cookies") or []
            _must(any(str(row.get("name")) == "phatmux_cookie" for row in cookies), f"Expected phatmux_cookie in cookies.get: {got_cookie}")
            c._call("browser.cookies.clear", {"surface_id": sid, "name": "phatmux_cookie"})
            got_after_clear = c._call("browser.cookies.get", {"surface_id": sid, "name": "phatmux_cookie"}) or {}
            ...
            c._call("browser.addinitscript", {"surface_id": sid, "script": "window.__phatmuxInitMarker = 'init-ok';"})
            c._call("browser.navigate", {"surface_id": sid, "url": second_url})
            _wait_selector(c, sid, "#second", timeout_s=7.0)
            init_value = c._call("browser.eval", {"surface_id": sid, "script": "window.__phatmuxInitMarker || ''"}) or {}
            ...
            state_path = tempfile.NamedTemporaryFile(delete=False, prefix="phatmux-state-", suffix=".json").name
