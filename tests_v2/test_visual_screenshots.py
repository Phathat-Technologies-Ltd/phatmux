#!/usr/bin/env python3
"""
Visual Screenshot Tests for phatmux
...
def get_client() -> phatmux:
    c = phatmux(SOCKET_PATH)
    c.connect()
    return c


def take_screenshot(client: phatmux, label: str) -> Optional[Screenshot]:
...
def capture_state(client: phatmux) -> str:
    try:
...
def stamp_terminals(client: phatmux, label: str) -> None:
    ...
    for h in terminal_surfaces:
        ...
        try:
            ...
            client.send_surface(idx, f"echo PHATMUX_VIS {safe} surf={idx}\n")
        except Exception:
            pass


def capture(client: phatmux, label: str):
...
def reset_workspace(client: phatmux) -> phatmux:
...
def surface_count(client: phatmux) -> int:
    return len(client.list_surfaces())

def pane_count(client: phatmux) -> int:
...
def wait_surface_count(client: phatmux, expected: int, timeout: float = 3.0) -> bool:
    start = time.time()
...
def wait_url_contains(client: phatmux, panel_id: str, needle: str, timeout: float = 8.0) -> bool:
...
def cleanup_workspaces(client: phatmux):
...
def _verify_surface_responsive(client: phatmux, surface_idx: int, marker: Path,
                                retries: int = 3) -> bool:
...
def verify_views_in_window(client: phatmux, label: str = "", timeout: float = 5.0) -> Optional[str]:
...
def verify_all_responsive(client: phatmux, label: str = "") -> Optional[str]:
    ...
    for idx, h in enumerate(terminal_surfaces):
        surface_idx = h["index"]
        marker = Path(tempfile.gettempdir()) / f"phatmux_vis_{os.getpid()}_{idx}"
        try:
...
def test_a1_initial_state(client: phatmux) -> StateChange:
...
def test_a2_split_right(client: phatmux) -> StateChange:
...
def test_a3_split_down(client: phatmux) -> StateChange:
...
def _close_and_verify(client: phatmux, change: StateChange, close_idx: int,
...
def test_b4_close_right(client: phatmux) -> StateChange:
...
def test_b5_close_left(client: phatmux) -> StateChange:
...
def test_b6_close_bottom(client: phatmux) -> StateChange:
...
def test_b7_close_top(client: phatmux) -> StateChange:
...
def test_c8_3way_close_middle(client: phatmux) -> StateChange:
...
def test_c9_grid_close_topleft(client: phatmux) -> StateChange:
...
def test_c10_grid_close_bottomright(client: phatmux) -> StateChange:
...
def test_d11_nested_close_bottomright(client: phatmux) -> StateChange:
...
def test_d12_nested_close_top(client: phatmux) -> StateChange:
...
def test_d13_4pane_close_second(client: phatmux) -> StateChange:
...
def test_e14_browser_close_terminal(client: phatmux) -> StateChange:
...
def test_e15_browser_close_browser(client: phatmux) -> StateChange:
...
def test_f16_nested_tabs_close_first(client: phatmux) -> StateChange:
...
def test_g17_rapid_down_close_top(client: phatmux) -> StateChange:
...
def test_g18_rapid_right_close_left(client: phatmux) -> StateChange:
...
def test_g19_alternating_close_reverse(client: phatmux) -> StateChange:
...
def test_h20_workspace_switch_back(client: phatmux) -> StateChange:
...
def _create_browser_surface(client: phatmux, url: Optional[str] = None) -> str:
    return client.new_surface(panel_type="browser", url=url)


def test_i21_browser_drag_split_right_wait_load(client: phatmux) -> StateChange:
...
def test_i22_browser_drag_split_right_immediate(client: phatmux) -> StateChange:
...
def test_i23_browser_drag_split_right_webview_focused(client: phatmux) -> StateChange:
...
def test_i24_browser_drag_split_right_focus_bounce(client: phatmux) -> StateChange:
...
def test_i25_browser_drag_split_right_then_switch_panes(client: phatmux) -> StateChange:
...
def test_i26_browser_drag_split_right_initial_url(client: phatmux) -> StateChange:
...
def test_i27_browser_drag_split_right_after_reload(client: phatmux) -> StateChange:
...
def test_i28_browser_drag_split_right_double_drag(client: phatmux) -> StateChange:
...
<html>
<head>
    <title>phatmux Visual Test Report</title>
    <style>
        body {
...
<body>
    <h1>phatmux Visual Test Report</h1>
...
def run_visual_tests():
    ...
    print("=" * 60)
    print(f"phatmux Visual Screenshot Tests ({len(test_fns)} scenarios)")
    print("=" * 60)
    print()
