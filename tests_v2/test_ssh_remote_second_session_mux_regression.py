#!/usr/bin/env python3
"""Regression: opening a second `phatmux ssh` workspace to the same host must not mux-refuse."""
...
def _wait_remote_ready(client: phatmux, workspace_id: str, timeout: float = 20.0) -> None:
...
def _wait_text_contains(client: phatmux, surface_id: str, needle: str, timeout: float = 8.0) -> str:
    deadline = time.time() + timeout
    last = ""
    while time.time() < deadline:
        ...
        if needle in last:
            return last
        time.sleep(0.1)
    raise phatmuxError(f"Timed out waiting for {needle!r} in surface {surface_id}: {last[-800:]!r}")


def main() -> int:
    if not SSH_HOST:
        print("SKIP: set PHATMUX_SSH_TEST_HOST to run second-session ssh mux regression")
        return 0
    ...
    try:
        with phatmux(SOCKET_PATH) as client:
            ...
            _must(bool(first_workspace_id), f"first phatmux ssh output missing workspace_id: {first}")
            ...
            _wait_text_contains(client, first_surface_id, "phatmux in ~", timeout=12.0)

            second = _run_cli_json(cli, ["ssh", SSH_HOST])
            second_workspace_id = _workspace_id_from_payload(client, second)
            _must(bool(second_workspace_id), f"second phatmux ssh output missing workspace_id: {second}")
            _must(
                second_workspace_id != first_workspace_id,
                f"second phatmux ssh should create a distinct workspace: {first_workspace_id} vs {second_workspace_id}",
            )
            ...
            text = _wait_text_contains(client, second_surface_id, "phatmux in ~", timeout=12.0)
            ...
            _must(
                not hits,
                "second phatmux ssh session printed mux refusal text instead of starting cleanly: "
            ...
            _must(
                ...
                f"second phatmux ssh session accepted corrupted input after startup: {text[-1200:]!r}",
            )
    finally:
        if workspace_ids:
            try:
    ...
    print("PASS: second phatmux ssh session opens cleanly without mux refusal")
    return 0
