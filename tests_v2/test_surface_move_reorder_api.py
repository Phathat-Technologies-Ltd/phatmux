...
def main() -> int:
    with phatmux(SOCKET_PATH) as c:
        ...
        if len(panes) < 2:
            raise phatmuxError(f"Expected >=2 panes, got {len(panes)}")
        dst_pane = next((pid for pid in panes if pid != src_pane), None)
        if not dst_pane:
            raise phatmuxError("Failed to find destination pane")
        ...
        if s_move in after_src:
            raise phatmuxError(f"Expected moved surface to leave source pane (src={src_pane}, ids={after_src})")
        if s_move not in after_dst:
            raise phatmuxError(f"Expected moved surface in destination pane (dst={dst_pane}, ids={after_dst})")
        ...
        if len(after_dst) < 2:
            ...
            if extra not in after_dst:
                raise phatmuxError("Failed to create extra destination surface for reorder test")
        ...
        if s_move not in reordered:
            raise phatmuxError(f"Expected moved surface to remain in destination pane after reorder (ids={reordered})")
        if reordered[0] != s_move:
            raise phatmuxError(f"Expected moved surface at front after reorder (ids={reordered})")
        ...
        if not ordered_ws:
            raise phatmuxError("workspace.list returned empty after reorder")
        if ordered_ws[0] != ws2:
            raise phatmuxError(f"Expected ws2 first after reorder (ordered={ordered_ws}, ws2={ws2})")
