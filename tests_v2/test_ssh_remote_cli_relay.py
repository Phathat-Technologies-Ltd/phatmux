#!/usr/bin/env python3
"""Docker integration: verify phatmux CLI commands work over SSH via reverse socket forwarding."""
...
REMOTE_HTTP_PORT = int(os.environ.get("PHATMUX_SSH_TEST_REMOTE_HTTP_PORT", "81"))
...
def _run_cli_json(cli: str, args: list[str]) -> dict:
    ...
    env.pop("PHATMUX_SOCKET_PATH", None)
...
def _wait_for_remote_ready(client, workspace_id: str, timeout: float = 45.0) -> dict:
    ...
    while time.time() < deadline:
        ...
        if state == "connected" and daemon_state == "ready":
            return last_status
        time.sleep(0.5)
    raise phatmuxError(f"Remote daemon did not become ready: {last_status}")
...
def _assert_remote_ping(host: str, host_port: int, key_path: Path, remote_socket_addr: str, *, label: str) -> None:
    ping_result = _ssh_run(
        ...
        f"PHATMUX_SOCKET_PATH={remote_socket_addr} $HOME/.phatmux/bin/phatmux ping",
        check=False,
    )
    _must(
        ping_result.returncode == 0 and "pong" in ping_result.stdout.lower(),
        f"{label} phatmux ping failed: rc={ping_result.returncode} stdout={ping_result.stdout!r} stderr={ping_result.stderr!r}",
    )
...
def main() -> int:
    ...
    temp_dir = Path(tempfile.mkdtemp(prefix="phatmux-ssh-cli-relay-"))
    image_tag = f"phatmux-ssh-test:{secrets.token_hex(4)}"
    container_name = f"phatmux-ssh-cli-relay-{secrets.token_hex(4)}"
    workspace_id = ""
    workspace_id_2 = ""
    ...
    try:
        ...
        with phatmux(SOCKET_PATH) as client:
            ...
            _must(
                'PATH="$HOME/.phatmux/bin:$PATH"' in startup_cmd,
                f"ssh startup command should prepend ~/.phatmux/bin for remote phatmux CLI: {startup_cmd!r}",
            )
            _must(
                f"PHATMUX_SOCKET_PATH={remote_socket_addr}" in startup_cmd,
                f"ssh startup command should pin PHATMUX_SOCKET_PATH to workspace relay: {startup_cmd!r}",
            )
            ...
            _must(
                ...
                f"phatmux ssh should focus created workspace: current={current_workspace_id!r} created={workspace_id!r}",
            )
            ...
            # Verify remote phatmux wrapper + relay-specific daemon mapping were installed.
            wrapper_check = None
            wrapper_deadline = time.time() + 10.0
            while time.time() < wrapper_deadline:
                wrapper_check = _ssh_run(
                    host, host_ssh_port, key_path,
                    f"test -x \"$HOME/.phatmux/bin/phatmux\" && test -f \"$HOME/.phatmux/bin/phatmux\" && "
                    f"map=\"$HOME/.phatmux/relay/{remote_relay_port}.daemon_path\" && "
            ...
            _must(
                ...
                f"Expected remote phatmux wrapper+relay mapping to exist: {wrapper_check.stdout if wrapper_check else ''} {wrapper_check.stderr if wrapper_check else ''}",
            )
            ...
            if not workspace_id_2 and workspace_ref_2.startswith("workspace:"):
                ...
                for row in listed_2.get("workspaces") or []:
                    if str(row.get("ref") or "") == workspace_ref_2:
                        ...
                        break
            _must(bool(workspace_id_2), f"second phatmux ssh output missing workspace_id: {payload_2}")

            remote_relay_port_2 = payload_2.get("remote_relay_port")
            _must(remote_relay_port_2 is not None, f"second phatmux ssh output missing remote_relay_port: {payload_2}")
            ...
            _must(
                f"PHATMUX_SOCKET_PATH={remote_socket_addr_2}" in startup_cmd_2,
                f"second ssh startup command should pin PHATMUX_SOCKET_PATH to second relay: {startup_cmd_2!r}",
            )
            ...
            # Test 1: phatmux ping (v1)
            _assert_remote_ping(host, host_ssh_port, key_path, remote_socket_addr, label="phatmux")

            # Test 2: phatmux list-workspaces --json (v2)
            list_ws_result = _ssh_run(
                host, host_ssh_port, key_path,
                f"PHATMUX_SOCKET_PATH={remote_socket_addr} $HOME/.phatmux/bin/phatmux --json list-workspaces",
                check=False,
            )
            _must(
                list_ws_result.returncode == 0,
                f"phatmux list-workspaces failed: rc={list_ws_result.returncode} stderr={list_ws_result.stderr!r}",
            )
            try:
                ...
                raise phatmuxError(f"list-workspaces returned invalid JSON: {list_ws_result.stdout!r}")

            # Test 3: phatmux new-window (v1)
            new_win_result = _ssh_run(
                host, host_ssh_port, key_path,
                f"PHATMUX_SOCKET_PATH={remote_socket_addr} $HOME/.phatmux/bin/phatmux new-window",
                check=False,
            )
            _must(
                new_win_result.returncode == 0,
                f"phatmux new-window failed: rc={new_win_result.returncode} stderr={new_win_result.stderr!r}",
            )

            # Test 4: phatmux rpc system.capabilities (v2 passthrough)
            rpc_result = _ssh_run(
                host, host_ssh_port, key_path,
                f"PHATMUX_SOCKET_PATH={remote_socket_addr} $HOME/.phatmux/bin/phatmux rpc system.capabilities",
                check=False,
            )
            _must(
                rpc_result.returncode == 0,
                f"phatmux rpc system.capabilities failed: rc={rpc_result.returncode} stderr={rpc_result.stderr!r}",
            )
            try:
                ...
                raise phatmuxError(f"rpc system.capabilities returned invalid JSON: {rpc_result.stdout!r}")
        ...
        print("PASS: phatmux CLI commands relay correctly over SSH reverse socket forwarding")
        return 0
