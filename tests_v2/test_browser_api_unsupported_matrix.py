...
def _expect_not_supported(c: phatmux, method: str, params: dict) -> None:
    try:
        ...
        if "not_supported" in text:
            return
        raise phatmuxError(f"Expected not_supported for {method}, got: {text}")
    raise phatmuxError(f"Expected not_supported for {method}, but call succeeded")
