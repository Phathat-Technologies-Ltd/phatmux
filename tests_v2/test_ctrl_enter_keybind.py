...
def infer_app_name_for_osascript(socket_path: str) -> str:
    ...
    Examples:
      - /tmp/phatmux-debug.sock          -> "phatmux DEV"
      - /tmp/phatmux-debug-foo.sock      -> "phatmux DEV foo"
      - ~/Library/Application Support/phatmux/phatmux.sock -> "phatmux"
      - /tmp/phatmux-foo.sock            -> "phatmux foo"
    """
    base = Path(socket_path).name
    if base.startswith("phatmux-debug") and base.endswith(".sock"):
        suffix = base[len("phatmux-debug") : -len(".sock")]
        if suffix.startswith("-") and suffix[1:]:
            return f"phatmux DEV {suffix[1:]}"
        return "phatmux DEV"
    if base.startswith("phatmux") and base.endswith(".sock"):
        suffix = base[len("phatmux") : -len(".sock")]
        if suffix.startswith("-") and suffix[1:]:
            return f"phatmux {suffix[1:]}"
        return "phatmux"
...
def test_ctrl_enter_keybind(client: phatmux) -> tuple[bool, str]:
...
def run_tests() -> int:
    print("=" * 60)
    print("phatmux Ctrl+Enter Keybind Test")
    print("=" * 60)
    print()
