...
LAST_SOCKET_HINT_PATH = Path("/tmp/phatmux-last-socket-path")
...
def main() -> int:
    ...
    _must("phatmux" in version_out, f"--version output should mention phatmux: {version_out!r}")

    # Debug builds should auto-resolve the active debug socket via /tmp/phatmux-last-socket-path
    # when PHATMUX_SOCKET_PATH is not set.
    ...
    try:
        ...
        auto_env.pop("PHATMUX_SOCKET_PATH", None)
        auto_env.pop("PHATMUX_SOCKET", None)
    ...
    ping_proc = _run([cli, "--socket", SOCKET_PATH, "--password", "ignored-in-phatmuxonly", "ping"])
