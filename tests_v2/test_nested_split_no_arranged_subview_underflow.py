...
def main() -> int:
    with phatmux(SOCKET_PATH) as c:
        ...
        panes = c.list_panes()
        if len(panes) < 2:
            raise phatmuxError(f"expected >=2 panes after first split, got {len(panes)}: {panes}")
        ...
        if underflows != 0:
            ...
            raise phatmuxError(f"bonsplit arranged-subview underflow observed ({underflows}); screenshot: {shot}")
