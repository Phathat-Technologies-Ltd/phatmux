...
<!doctype html>
<html>
  <head><title>phatmux-browser-p0</title></head>
  <body>
    <input id='name' value=''>
...
""".strip()
        ...
        _must("phatmux" in str(out.get("value", "")), f"Expected #out text to include 'phatmux': {out}")
        ...
        _must(str(eval_res.get("value", "")) == "phatmux", f"Expected eval value 'phatmux': {eval_res}")

        snap = c._call("browser.snapshot", {"surface_id": target}) or {}
        snapshot_text = str(snap.get("snapshot") or "")
        _must("phatmux-browser-p0" in snapshot_text, f"Expected snapshot text to include page title: {snap}")
    ...
    print("PASS: browser.* P0 methods work on phatmux webview with ref handles")
    return 0
