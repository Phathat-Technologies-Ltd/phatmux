...
def main() -> int:
    token = "PHATMUX_PALETTE_FOCUS_PROBE_9412"
    restore_token = "PHATMUX_PALETTE_RESTORE_PROBE_7731"
    ...
    with phatmux(SOCKET_PATH) as client:
        ...
        if token in post_text and token not in pre_text:
            raise phatmuxError("typed probe text leaked into terminal while palette is open")
        ...
        if restore_token not in restore_text:
            raise phatmuxError("terminal did not receive typing after closing command palette")
