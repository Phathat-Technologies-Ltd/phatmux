    ...
    with phatmux(SOCKET_PATH) as client:
        ...
        try:
            ...
            if str(after_first.get("mode") or "") != "rename_input":
                raise phatmuxError(f"palette exited rename mode too early after first backspace: {after_first}")
            ...
            if not _palette_visible(client, window_id):
                raise phatmuxError("palette closed unexpectedly instead of navigating back to command list")
