...
def _focused_pane_id(client):
    ...
    if not focused:
        raise phatmuxError(f"no focused pane: {panes}")
...
def _selected_surface_title(client, pane_id):
    ...
    if not selected:
        raise phatmuxError(f"no selected surface in pane {pane_id}: {rows}")
...
def main():
    with phatmux(SOCKET_PATH) as client:
        ...
        if new_title != rename_to:
            raise phatmuxError(f"rename not applied: expected '{rename_to}', got '{new_title}'")
