...
def _resolve_socket_path() -> str:
    socket_path = os.environ.get("PHATMUX_SOCKET", "").strip()
    if not socket_path:
        raise phatmuxError("PHATMUX_SOCKET is required (expected /tmp/phatmux-debug-<tag>.sock)")
    if not re.fullmatch(r"/tmp/phatmux-debug-[^/]+\.sock", socket_path):
        raise phatmuxError(f"PHATMUX_SOCKET must be a tagged debug socket, got: {socket_path!r}")
    return socket_path
...
def _find_cli_binary() -> str:
    ...
    candidates = glob.glob(
        os.path.expanduser("~/Library/Developer/Xcode/DerivedData/**/Build/Products/Debug/phatmux"),
        recursive=True,
    )
...
def _create_git_repo(root: Path) -> Path:
    ...
    subprocess.run(
        ["git", "config", "user.name", "phatmux-test"],
        cwd=repo,
        check=True,
        stdout=subprocess.DEVNULL,
        stderr=subprocess.DEVNULL,
    ...
    subprocess.run(
        ["git", "config", "user.email", "phatmux-test@example.com"],
        cwd=repo,
        check=True,
        stdout=subprocess.DEVNULL,
        stderr=subprocess.DEVNULL,
...
def main() -> int:
    ...
    temp_root = Path(tempfile.mkdtemp(prefix="phatmux_issue_915_external_git_"))
