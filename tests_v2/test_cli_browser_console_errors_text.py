...
def _run_cli(cli: str, args: list[str]) -> str:
    ...
    if proc.returncode != 0:
        ...
        raise phatmuxError(f"CLI failed ({' '.join(args)}): {merged}")
...
def _open_server() -> tuple[str, socketserver.TCPServer, threading.Thread, tempfile.TemporaryDirectory[str]]:
    root = tempfile.TemporaryDirectory(prefix="phatmux-browser-cli-logs-")
  ...
  <body>
    ...
    <script>
      window.emitLogs = function () {
        ...
        setTimeout(function () { throw new Error('phatmux-browser-boom'); }, 0);
        return true;
      };
    </script>
  </body>
...
def main() -> int:
    ...
    try:
        with phatmux(SOCKET_PATH) as c:
            ...
            _must("phatmux-console-entry" in console_output, f"browser console text mode should print entries: {console_output!r}")
            ...
            _must("phatmux-browser-boom" in errors_output, f"browser errors text mode should print entries: {errors_output!r}")
