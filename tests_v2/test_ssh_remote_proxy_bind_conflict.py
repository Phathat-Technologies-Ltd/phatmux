...
def _wait_for_proxy_conflict_status(client: phatmux, workspace_id: str, expected_local_proxy_port: int, timeout: float = 30.0) -> dict:
    deadline = time.time() + timeout
    last_status = {}
    while time.time() < deadline:
    ...
    raise phatmuxError(f"Remote did not reach structured proxy_unavailable status for bind conflict: {last_status}")
...
def main() -> int:
    ...
    temp_dir = Path(tempfile.mkdtemp(prefix="phatmux-ssh-proxy-conflict-"))
    image_tag = f"phatmux-ssh-test:{secrets.token_hex(4)}"
    container_name = f"phatmux-ssh-proxy-conflict-{secrets.token_hex(4)}"
    workspace_id = ""
