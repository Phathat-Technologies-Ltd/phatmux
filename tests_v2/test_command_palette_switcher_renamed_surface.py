...
def _rename_surface(client: phatmux, surface_id: str, title: str) -> None:
    client._call(
        "surface.action",
        {
...
def _current_surface_id(client: phatmux, workspace_id: str) -> str:
...
def main() -> int:
    with phatmux(SOCKET_PATH) as client:
        ...
        if len(rows) < 2:
            raise phatmuxError(f"expected at least two surfaces after split: {payload}")
        ...
        if not left_surface_id:
            raise phatmuxError(f"failed to resolve left surface id: {payload}")
        ...
        if not result_rows:
            raise phatmuxError("switcher returned no rows for renamed surface query")
