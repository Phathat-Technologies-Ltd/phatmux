...
def _wait_focused_index(client: phatmux, index: int, timeout: float = 4.0) -> bool:
    start = time.time()
...
def _ensure_surfaces(client: phatmux, count: int) -> None:
    surfaces = client.list_surfaces()
...
def test_close_middle_keeps_index(client: phatmux) -> TestResult:
...
def test_close_last_selects_previous(client: phatmux) -> TestResult:
