...
DEFAULT_SOCKET_PATHS = ["/tmp/phatmux-debug.sock", "/tmp/phatmux.sock"]
...
def _extract_segment_lines(
    ...
    *,
    ...
    if not saw_start:
        raise phatmuxError(f"start marker not found in scrollback: {start_marker}")
    if require_end and not saw_end:
        raise phatmuxError(f"end marker not found in scrollback: {end_marker}")
    return out


def _run_once(socket_path: str) -> int:
    workspace_id = ""
    fixture_dir = Path(tempfile.mkdtemp(prefix="phatmux-ls-resize-regression-"))
    try:
        with phatmux(socket_path) as client:
            ...
            start_marker = f"PHATMUX_LS_SCROLLBACK_START_{secrets.token_hex(4)}"
            end_marker = f"PHATMUX_LS_SCROLLBACK_END_{secrets.token_hex(4)}"
...
def main() -> int:
    env_socket = os.environ.get("PHATMUX_SOCKET")
    if env_socket:
    ...
    if last_error is not None:
        raise last_error
    raise phatmuxError("No socket candidates configured")
