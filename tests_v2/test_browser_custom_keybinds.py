...
def wait_url_contains(client: phatmux, panel_id: str, needle: str, timeout_s: float = 10.0) -> None:
    start = time.time()
...
def test_cmd_ctrl_h_goto_split_left_from_webview(client: phatmux) -> tuple[bool, str]:
    """
...
def test_cmd_opt_left_arrow_goto_split_left_from_webview(client: phatmux) -> tuple[bool, str]:
    """
...
def main() -> int:
    print("phatmux Browser Custom Keybind Tests")
    print("=" * 50)
    client = phatmux()
    client.connect()
