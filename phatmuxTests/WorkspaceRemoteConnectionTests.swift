...
#if canImport(phatmux)
@testable import phatmux
#elseif canImport(phatmux_DEV)
...
final class WorkspaceRemoteConnectionTests: XCTestCase {
    ...
    private func runRelayZshHistfile(
        ...
        let home = fileManager.temporaryDirectory.appendingPathComponent("phatmux-relay-zsh-\(UUID().uuidString)")
        let relayDir = home.appendingPathComponent(".phatmux/relay/64011.shell")
        ...
        let result = runProcess(
            ...
            arguments: [
                ...
                "SHELL=/bin/zsh",
                "USER=\(NSUserName())",
                "PHATMUX_REAL_ZDOTDIR=\(home.path)",
    ...
    func testRemoteRelayMetadataCleanupScriptRemovesMatchingSocketAddr() {
        let fileManager = FileManager.default
        let home = fileManager.temporaryDirectory.appendingPathComponent("phatmux-relay-cleanup-\(UUID().uuidString)")
        let relayDir = home.appendingPathComponent(".phatmux/relay")
        let socketAddrURL = home.appendingPathComponent(".phatmux/socket_addr")
    ...
    func testRemoteRelayMetadataCleanupScriptPreservesDifferentSocketAddr() {
        let fileManager = FileManager.default
        let home = fileManager.temporaryDirectory.appendingPathComponent("phatmux-relay-cleanup-preserve-\(UUID().uuidString)")
        let relayDir = home.appendingPathComponent(".phatmux/relay")
        let socketAddrURL = home.appendingPathComponent(".phatmux/socket_addr")
    ...
    func testRemoteTerminalSurfaceLookupTracksOnlyActiveSSHSurfaces() throws {
        ...
        let config = WorkspaceRemoteConfiguration(
            ...
            localSocketPath: "/tmp/phatmux-debug-test.sock",
    ...
    func testRemoteDropPathUsesLowercasedExtensionAndProvidedUUID() throws {
        ...
        XCTAssertEqual(remotePath, "/tmp/phatmux-drop-12345678-1234-1234-1234-1234567890ab.png")
    }
    ...
    func testDetectedSSHUploadFailureCleansUpEarlierRemoteUploads() throws {
        ...
        let directoryURL = fileManager.temporaryDirectory.appendingPathComponent(
            "phatmux-detected-ssh-upload-\(UUID().uuidString)",
            isDirectory: true
        )
        ...
        let session = TerminalSSHSessionDetector.detectForTesting(
            ...
            argumentsByPID: [
                2145: [
                    ...
                    "-o", "ControlPath=/tmp/phatmux-ssh-%C",
    ...
    func testDetectsForegroundSSHSessionWithShortControlPathFlag() {
        ...
        XCTAssertEqual(session?.controlPath, "/tmp/phatmux-ssh-%C")
        let scpArgs = session?.scpArgumentsForTesting(
            localPath: "/tmp/local.png",
            remotePath: "/tmp/phatmux-drop-123.png"
        ) ?? []
        XCTAssertTrue(scpArgs.contains("ControlPath=/tmp/phatmux-ssh-%C"))
    ...
    func testDetectedSSHSessionBracketsIPv6LiteralSCPDestination() {
        ...
        XCTAssertEqual(scpArgs.last, "lawrence@[2001:db8::1]:/tmp/phatmux-drop-123.png")
    }
    ...
    func testProxyOnlyErrorsKeepSSHWorkspaceConnectedAndLoggedInSidebar() {
        ...
        let proxyError = "Remote proxy to phatmux-macmini unavailable: Failed to start local daemon proxy: daemon RPC timeout waiting for hello response (retry in 3s)"
        workspace.applyRemoteConnectionStateUpdate(.error, detail: proxyError, target: "phatmux-macmini")
        ...
        XCTAssertEqual(
            ...
            "Remote proxy unavailable (phatmux-macmini): \(proxyError)"
        )
        ...
        workspace.applyRemoteConnectionStateUpdate(.connecting, detail: "Connecting to phatmux-macmini", target: "phatmux-macmini")
        ...
        workspace.applyRemoteConnectionStateUpdate(
            .connected,
            detail: "Connected to phatmux-macmini via shared local proxy 127.0.0.1:9999",
            target: "phatmux-macmini"
        )
