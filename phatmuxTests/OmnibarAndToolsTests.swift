...
final class FinderServicePathResolverTests: XCTestCase {
    func testOrderedUniqueDirectoriesUsesParentForFilesAndDedupes() {
        let input: [URL] = [
            URL(fileURLWithPath: "/tmp/phatmux-services/project", isDirectory: true),
            URL(fileURLWithPath: "/tmp/phatmux-services/project/README.md", isDirectory: false),
            URL(fileURLWithPath: "/tmp/phatmux-services/../phatmux-services/project", isDirectory: true),
            URL(fileURLWithPath: "/tmp/phatmux-services/other", isDirectory: true),
        ]
        ...
        XCTAssertEqual(
            directories,
            [
                "/tmp/phatmux-services/project",
                "/tmp/phatmux-services/other",
            ]
        )
    }
    ...
    func testOrderedUniqueDirectoriesPreservesFirstSeenOrder() {
        let input: [URL] = [
            URL(fileURLWithPath: "/tmp/phatmux-services/b", isDirectory: true),
            URL(fileURLWithPath: "/tmp/phatmux-services/a/file.txt", isDirectory: false),
            URL(fileURLWithPath: "/tmp/phatmux-services/a", isDirectory: true),
            URL(fileURLWithPath: "/tmp/phatmux-services/b/file.txt", isDirectory: false),
        ]
        ...
        XCTAssertEqual(
            directories,
            [
                "/tmp/phatmux-services/b",
                "/tmp/phatmux-services/a",
            ]
        )
    }
}
...
final class VSCodeServeWebURLBuilderTests: XCTestCase {
    ...
    func testOpenFolderURLAppendsFolderQueryWhilePreservingToken() {
        ...
        let url = VSCodeServeWebURLBuilder.openFolderURL(
            ...
            directoryPath: "/Users/tester/Projects/phatmux"
        )

        let components = URLComponents(url: url!, resolvingAgainstBaseURL: false)
        XCTAssertEqual(components?.queryItems?.first(where: { $0.name == "tkn" })?.value, "test-token")
        XCTAssertEqual(components?.queryItems?.first(where: { $0.name == "folder" })?.value, "/Users/tester/Projects/phatmux")
    }
