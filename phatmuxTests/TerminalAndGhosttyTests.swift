...
final class GhosttyPasteboardHelperTests: XCTestCase {
    ...
    func testHTMLOnlyPasteboardExtractsPlainText() {
        let pasteboard = NSPasteboard(name: .init("phatmux-test-html-\(UUID().uuidString)"))
        pasteboard.clearContents()
        pasteboard.setString("<p>Hello <strong>world</strong></p>", forType: .html)

        XCTAssertEqual(phatmuxPasteboardStringContentsForTesting(pasteboard), "Hello world")
        XCTAssertNil(phatmuxPasteboardImagePathForTesting(pasteboard))
    }

    func testImageHTMLClipboardFallsBackToImagePath() throws {
        let pasteboard = NSPasteboard(name: .init("phatmux-test-image-html-\(UUID().uuidString)"))
        pasteboard.clearContents()
        ...
        XCTAssertNil(phatmuxPasteboardStringContentsForTesting(pasteboard))

        let imagePath = try XCTUnwrap(phatmuxPasteboardImagePathForTesting(pasteboard))
    ...
    func testImageHTMLClipboardWithVisibleTextPrefersText() throws {
        let pasteboard = NSPasteboard(name: .init("phatmux-test-image-html-text-\(UUID().uuidString)"))
        pasteboard.clearContents()
        ...
        XCTAssertEqual(phatmuxPasteboardStringContentsForTesting(pasteboard), "Hello")
        XCTAssertNil(phatmuxPasteboardImagePathForTesting(pasteboard))
    }

    func testJPEGClipboardFallsBackToImagePath() throws {
        let pasteboard = NSPasteboard(name: .init("phatmux-test-jpeg-\(UUID().uuidString)"))
        pasteboard.clearContents()
    ...
    func testAttachmentOnlyRTFDClipboardFallsBackToImagePath() throws {
        let pasteboard = NSPasteboard(name: .init("phatmux-test-rtfd-attachment-\(UUID().uuidString)"))
        pasteboard.clearContents()
    ...
    func testAttachmentOnlyRTFDNonImageClipboardDoesNotFallBackToImagePath() throws {
        let pasteboard = NSPasteboard(name: .init("phatmux-test-rtfd-non-image-\(UUID().uuidString)"))
        pasteboard.clearContents()
    ...
    func testRTFDClipboardWithVisibleTextPrefersText() throws {
        let pasteboard = NSPasteboard(name: .init("phatmux-test-rtfd-text-\(UUID().uuidString)"))
        pasteboard.clearContents()
    ...
    func testImageOnlyPasteboardProducesTempFileURL() throws {
        let pasteboard = NSPasteboard(name: .init("phatmux-test-drop-image-\(UUID().uuidString)"))
        pasteboard.clearContents()
        pasteboard.setData(try make1x1PNG(color: .red), forType: .png)

        let fileURL = try XCTUnwrap(phatmuxPasteboardImageFileURLForTesting(pasteboard))
    ...
    func testRemoteImageDropPlanUploadsMaterializedFile() throws {
        let pasteboard = NSPasteboard(name: .init("phatmux-test-remote-drop-\(UUID().uuidString)"))
        pasteboard.clearContents()
    ...
    func testLocalImageDropPlanInsertsEscapedLocalPath() throws {
        let pasteboard = NSPasteboard(name: .init("phatmux-test-local-drop-\(UUID().uuidString)"))
        pasteboard.clearContents()
    ...
    func testRemoteImagePastePlanUploadsMaterializedFile() throws {
        let pasteboard = NSPasteboard(name: .init("phatmux-test-remote-paste-\(UUID().uuidString)"))
        pasteboard.clearContents()
    ...
    func testRemoteFileURLPastePlanUploadsReadableFile() throws {
        ...
        let pasteboard = NSPasteboard(name: .init("phatmux-test-remote-file-url-paste-\(UUID().uuidString)"))
        pasteboard.clearContents()
    ...
    func testRemoteDirectoryPastePlanFallsBackToEscapedPathInsertion() throws {
        ...
        let pasteboard = NSPasteboard(name: .init("phatmux-test-remote-directory-paste-\(UUID().uuidString)"))
        pasteboard.clearContents()
    ...
    func testLazyPastePlanSkipsTargetResolutionForPlainText() {
        let pasteboard = NSPasteboard(name: .init("phatmux-test-lazy-text-paste-\(UUID().uuidString)"))
        pasteboard.clearContents()
    ...
    func testLazyPastePlanResolvesTargetForFileURLPaste() throws {
        ...
        let pasteboard = NSPasteboard(name: .init("phatmux-test-lazy-file-paste-\(UUID().uuidString)"))
        pasteboard.clearContents()
        XCTAssertTrue(pasteboard.writeObjects([fileURL as NSURL]))
    ...
    func testLocalImagePastePlanInsertsEscapedLocalPath() throws {
        let pasteboard = NSPasteboard(name: .init("phatmux-test-local-paste-\(UUID().uuidString)"))
        pasteboard.clearContents()
    ...
    func testRemoteImagePasteExecutionUploadsAndCompletesWithRemotePath() throws {
        ...
        TerminalImageTransferPlanner.executeForTesting(
            ...
            uploadWorkspaceRemote: { _, _, finish in finish(.success(["/tmp/phatmux-drop-123.png"])) },
        ...
        XCTAssertEqual(completedText, "/tmp/phatmux-drop-123.png")
    }
    ...
    func testCancelledRemoteImagePasteExecutionSuppressesCompletionHandlers() throws {
        ...
        completion?(.success(["/tmp/phatmux-drop-cancelled.png"]))
    ...
    func testCancelledOperationSuppressesLateLocalInsert() {
        ...
        let returnedOperation = TerminalImageTransferPlanner.executeForTesting(
            plan: .insertText("/tmp/phatmux-drop-local.png"),
            operation: operation,
    ...
    func testRemoteImageDropHandlerUploadsAndSendsRemotePath() throws {
        let pasteboard = NSPasteboard(name: .init("phatmux-test-remote-handler-\(UUID().uuidString)"))
        pasteboard.clearContents()
        ...
        let handled = GhosttyNSView.handleDropForTesting(
            ...
            uploadRemote: { urls, finish in
                uploadedURLs = urls
                finish(.success(["/tmp/phatmux-drop-abc123.png"]))
            },
        ...
        XCTAssertEqual(sentText, ["/tmp/phatmux-drop-abc123.png"])
        XCTAssertEqual(failureCount, 0)
    }

    func testRemoteImageDropHandlerCleansUpMaterializedTemporaryImageAfterSuccess() throws {
        let pasteboard = NSPasteboard(name: .init("phatmux-test-remote-handler-cleanup-\(UUID().uuidString)"))
        pasteboard.clearContents()
        pasteboard.setData(try make1x1PNG(color: .orange), forType: .png)
    ...
    func testRemoteDropUploadFailureTriggersFailureHandler() throws {
        let pasteboard = NSPasteboard(name: .init("phatmux-test-remote-handler-fail-\(UUID().uuidString)"))
        pasteboard.clearContents()
    ...
    func testRemoteImageDropHandlerCleansUpMaterializedTemporaryImageAfterFailure() throws {
        let pasteboard = NSPasteboard(name: .init("phatmux-test-remote-handler-failure-cleanup-\(UUID().uuidString)"))
        pasteboard.clearContents()
        pasteboard.setData(try make1x1PNG(color: .cyan), forType: .png)
...
final class GhosttyResponderResolutionTests: XCTestCase {
    ...
    func testResolvesGhosttyViewFromDescendantResponder() {
        ...
        XCTAssertTrue(phatmuxOwningGhosttyView(for: descendant) === ghosttyView)
    }

    func testResolvesGhosttyViewFromGhosttyResponder() {
        let ghosttyView = GhosttyNSView(frame: NSRect(x: 0, y: 0, width: 200, height: 120))
        XCTAssertTrue(phatmuxOwningGhosttyView(for: ghosttyView) === ghosttyView)
    }

    func testReturnsNilForUnrelatedResponder() {
        let view = FocusProbeView(frame: NSRect(x: 0, y: 0, width: 40, height: 40))
        XCTAssertNil(phatmuxOwningGhosttyView(for: view))
    }
}
...
final class TerminalOpenURLTargetResolutionTests: XCTestCase {
    ...
    func testResolvesFileSchemeAsExternal() throws {
        let target = try XCTUnwrap(resolveTerminalOpenURLTarget("file:///tmp/phatmux.txt"))
        switch target {
        case let .external(url):
            XCTAssertTrue(url.isFileURL)
            XCTAssertEqual(url.path, "/tmp/phatmux.txt")
        default:
    ...
    func testResolvesAbsolutePathAsExternalFileURL() throws {
        let target = try XCTUnwrap(resolveTerminalOpenURLTarget("/tmp/phatmux-path.txt"))
        switch target {
        case let .external(url):
            XCTAssertTrue(url.isFileURL)
            XCTAssertEqual(url.path, "/tmp/phatmux-path.txt")
        default:
    ...
    func testResolvesHostlessHTTPSAsExternal() throws {
        let target = try XCTUnwrap(resolveTerminalOpenURLTarget("https:///tmp/phatmux.txt"))
        switch target {
        case let .external(url):
            XCTAssertEqual(url.scheme, "https")
...
final class TerminalControllerSocketListenerHealthTests: XCTestCase {
    ...
    func testNonStableSocketBindFailureDoesNotFallback() {
        XCTAssertNil(
            TerminalController.fallbackSocketPathAfterBindFailure(
                requestedPath: "/tmp/phatmux-debug.sock",
                stage: "bind",
                errnoCode: EACCES,
                currentUserID: 501
            )
    ...
    private func makeTempSocketPath() -> String {
        "/tmp/phatmux-socket-health-\(UUID().uuidString).sock"
    }
