...
var phatmuxUnitTestInspectorAssociationKey: UInt8 = 0
var phatmuxUnitTestInspectorOverrideInstalled = false

extension CmuxWebView {
    @objc func phatmuxUnitTestInspector() -> NSObject? {
        objc_getAssociatedObject(self, &phatmuxUnitTestInspectorAssociationKey) as? NSObject
    }
}

extension WKWebView {
    func phatmuxSetUnitTestInspector(_ inspector: NSObject?) {
        objc_setAssociatedObject(
            self,
            &phatmuxUnitTestInspectorAssociationKey,
            inspector,
...
func installCmuxUnitTestInspectorOverride() {
    guard !phatmuxUnitTestInspectorOverrideInstalled else { return }

    guard let replacementMethod = class_getInstanceMethod(
        CmuxWebView.self,
        #selector(CmuxWebView.phatmuxUnitTestInspector)
    ) else {
    ...
    phatmuxUnitTestInspectorOverrideInstalled = true
}
...
final class BrowserSessionHistoryRestoreTests: XCTestCase {
    ...
    func testGoBackPrefersLiveWKWebViewHistoryBeforeRestoredFallback() throws {
        let tempDir = FileManager.default.temporaryDirectory
            .appendingPathComponent("phatmux-browser-history-\(UUID().uuidString)", isDirectory: true)
...
final class BrowserDeveloperToolsVisibilityPersistenceTests: XCTestCase {
    ...
    private func makePanelWithInspector(
        ...
        panel.webView.phatmuxSetUnitTestInspector(inspector)
...
final class BrowserNavigableURLResolutionTests: XCTestCase {
    func testResolvesFileSchemeAsNavigableURL() throws {
        let resolved = try XCTUnwrap(resolveBrowserNavigableURL("file:///tmp/phatmux-local-test.html"))
        XCTAssertTrue(resolved.isFileURL)
        XCTAssertEqual(resolved.path, "/tmp/phatmux-local-test.html")
    }
...
final class BrowserExternalNavigationSchemeTests: XCTestCase {
    ...
    func testEmbeddedBrowserSchemesStayInWebView() throws {
        ...
        let file = try XCTUnwrap(URL(string: "file:///tmp/phatmux-local-test.html"))
