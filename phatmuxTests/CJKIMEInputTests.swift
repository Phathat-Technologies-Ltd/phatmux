...
private extension GhosttyNSView {
    @objc func phatmuxUnitTest_interpretKeyEvents(_ eventArray: [NSEvent]) {
        if let hook = cjkIMEInterpretKeyEventsHook, hook(self, eventArray) {
            return
        }
        phatmuxUnitTest_interpretKeyEvents(eventArray)
    }
}
...
private func installCJKIMEInterpretKeyEventsSwizzle() {
    ...
    let swizzledSelector = #selector(GhosttyNSView.phatmuxUnitTest_interpretKeyEvents(_:))
...
// MARK: - Space release regression (Codex hold-to-talk in phatmux)
