...
final class CommandPaletteSwitcherSearchIndexerTests: XCTestCase {
    func testKeywordsIncludeDirectoryBranchAndPortMetadata() {
        let metadata = CommandPaletteSwitcherSearchMetadata(
            directories: ["/Users/example/dev/phatmux-hq/worktrees/feat-cmd-palette"],
        ...
        XCTAssertTrue(keywords.contains("/Users/example/dev/phatmux-hq/worktrees/feat-cmd-palette"))
    ...
    func testFuzzyMatcherMatchesDirectoryBranchAndPortMetadata() {
        let metadata = CommandPaletteSwitcherSearchMetadata(
            directories: ["/tmp/phatmux/worktrees/issue-123-switcher-search"],
    ...
    func testSurfaceDetailOutranksWorkspaceDetailForPathToken() throws {
        let metadata = CommandPaletteSwitcherSearchMetadata(
            directories: ["/tmp/worktrees/phatmux"],
        ...
        let workspaceScore = try XCTUnwrap(
            CommandPaletteFuzzyMatcher.score(query: "phatmux", candidates: workspaceKeywords)
        )
        let surfaceScore = try XCTUnwrap(
            CommandPaletteFuzzyMatcher.score(query: "phatmux", candidates: surfaceKeywords)
        )
