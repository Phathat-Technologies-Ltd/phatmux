...
final class SocketControlPasswordStoreTests: XCTestCase {
    ...
    func testSaveLoadAndClearRoundTripUsesFileStorage() throws {
        let tempDir = FileManager.default.temporaryDirectory
            .appendingPathComponent("phatmux-socket-password-tests-\(UUID().uuidString)", isDirectory: true)
    ...
    func testDefaultPasswordFileURLUsesCmuxAppSupportPath() throws {
        ...
        XCTAssertEqual(
            resolved?.path,
            tempDir.appendingPathComponent("phatmux", isDirectory: true)
    ...
    func testLegacyKeychainMigrationCopiesPasswordDeletesLegacyAndRunsOnlyOnce() throws {
        ...
        let fileURL = tempDir.appendingPathComponent("socket-password.txt", isDirectory: false)
        let defaultsSuiteName = "phatmux-socket-password-migration-tests-\(UUID().uuidString)"
...
final class CmuxCLIPathInstallerTests: XCTestCase {
    func testInstallAndUninstallRoundTripWithoutAdministratorPrivileges() throws {
        ...
        let root = fileManager.temporaryDirectory
            .appendingPathComponent("phatmux-cli-installer-tests-\(UUID().uuidString)", isDirectory: true)
        try fileManager.createDirectory(at: root, withIntermediateDirectories: true)
        defer { try? fileManager.removeItem(at: root) }

        let bundledCLIURL = root
            .appendingPathComponent("phatmux.app/Contents/Resources/bin/phatmux", isDirectory: false)
        ...
        let destinationURL = root.appendingPathComponent("usr/local/bin/phatmux", isDirectory: false)
