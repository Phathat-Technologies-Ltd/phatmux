...
final class NotificationDockBadgeTests: XCTestCase {
    ...
    func testNotificationCustomStagingPreservesSourceFileWithCmuxPrefix() {
        ...
        let sourceURL = soundsDirectory.appendingPathComponent(
            "phatmux-custom-notification-sound.source-\(UUID().uuidString).wav",
            isDirectory: false
        )
        defer {
        ...
        XCTAssertTrue(stagedName.hasPrefix("phatmux-custom-notification-sound-"))
    ...
    func testNotificationCustomUnsupportedExtensionsStageAsCaf() {
        ...
        XCTAssertTrue(stagedA.hasPrefix("phatmux-custom-notification-sound-"))
    ...
    func testNotificationCustomPreparationKeepsActiveSourceMetadataSidecar() {
        ...
        let sourceURL = soundsDirectory.appendingPathComponent(
            "phatmux-custom-notification-sound.metadata-\(UUID().uuidString).wav",
            isDirectory: false
        )
        do {
            try Data("test".utf8).write(to: sourceURL, options: .atomic)
    ...
    func testNotificationCustomSoundReturnsNilWhenPreparationFails() {
        ...
        let invalidSourceURL = FileManager.default.temporaryDirectory
            .appendingPathComponent("phatmux-invalid-sound-\(UUID().uuidString).mp3", isDirectory: false)
        defer {
            try? FileManager.default.removeItem(at: invalidSourceURL)
            let stagedURL = URL(fileURLWithPath: NSHomeDirectory(), isDirectory: true)
                .appendingPathComponent("Library", isDirectory: true)
                .appendingPathComponent("Sounds", isDirectory: true)
                .appendingPathComponent("phatmux-custom-notification-sound.caf", isDirectory: false)
    ...
    func testNotificationCustomPreparationReportsMissingFile() {
        let missingPath = FileManager.default.temporaryDirectory
            .appendingPathComponent("phatmux-missing-\(UUID().uuidString).wav", isDirectory: false)
            .path
    ...
    func testFocusedTerminalSuppressedNotificationRunsCustomCommand() throws {
        ...
        let commandOutputURL = FileManager.default.temporaryDirectory
            .appendingPathComponent("phatmux-notification-command-\(UUID().uuidString).txt", isDirectory: false)
        ...
        defaults.set("none", forKey: NotificationSoundSettings.key)
        defaults.set(
            "printf '%s\\n%s\\n%s' \"$PHATMUX_NOTIFICATION_TITLE\" \"$PHATMUX_NOTIFICATION_SUBTITLE\" \"$PHATMUX_NOTIFICATION_BODY\" > '\(commandOutputURL.path)'",
...
final class MenuBarBuildHintFormatterTests: XCTestCase {
    func testReleaseBuildShowsNoHint() {
        XCTAssertNil(MenuBarBuildHintFormatter.menuTitle(appName: "phatmux DEV menubar-extra", isDebugBuild: false))
    }

    func testDebugBuildWithTagShowsTag() {
        XCTAssertEqual(
            MenuBarBuildHintFormatter.menuTitle(appName: "phatmux DEV menubar-extra", isDebugBuild: true),
    ...
    func testDebugBuildWithoutTagShowsUntagged() {
        XCTAssertEqual(
            MenuBarBuildHintFormatter.menuTitle(appName: "phatmux DEV", isDebugBuild: true),
            "Build: DEV (untagged)"
        )
    }
}
