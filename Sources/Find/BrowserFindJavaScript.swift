...
enum BrowserFindJavaScript {
    ...
    static func searchScript(query: String) -> String {
        ...
        return """
        (() => {
          const MARK_CLASS = '__phatmux-find';
          const CURRENT_CLASS = '__phatmux-find-current';
          ...
          window.__phatmuxFindMatches = matches;
          window.__phatmuxFindIndex = 0;
          ...
          if (!document.getElementById('__phatmux-find-style')) {
            const style = document.createElement('style');
            style.id = '__phatmux-find-style';
            style.textContent = `
              mark.__phatmux-find { background: #facc15; color: #000; border-radius: 2px; }
              mark.__phatmux-find.__phatmux-find-current { background: #f97316; color: #fff; }
            `;
    ...
    static func nextScript() -> String {
        """
        (() => {
          const matches = window.__phatmuxFindMatches || [];
          if (matches.length === 0) return JSON.stringify({ total: 0, current: 0 });
          let idx = window.__phatmuxFindIndex || 0;
          if (!matches[idx] || !matches[idx].isConnected) {
            window.__phatmuxFindMatches = [];
            window.__phatmuxFindIndex = 0;
            return JSON.stringify({ total: 0, current: 0 });
          }
          matches[idx].classList.remove('__phatmux-find-current');
          ...
          matches[idx].classList.add('__phatmux-find-current');
          matches[idx].scrollIntoView({ block: 'center', behavior: 'smooth' });
          window.__phatmuxFindIndex = idx;
    ...
    static func clearScript() -> String {
        ...
        (() => {
          ...
          window.__phatmuxFindMatches = [];
          window.__phatmuxFindIndex = 0;
          const style = document.getElementById('__phatmux-find-style');
    ...
    document.querySelectorAll('mark.__phatmux-find').forEach(mark => {
