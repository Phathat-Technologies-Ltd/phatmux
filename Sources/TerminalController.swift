...
extension Notification.Name {
    static let socketListenerDidStart = Notification.Name("phatmux.socketListenerDidStart")
    static let terminalSurfaceDidBecomeReady = Notification.Name("phatmux.terminalSurfaceDidBecomeReady")
    static let terminalSurfaceHostedViewDidMoveToWindow = Notification.Name("phatmux.terminalSurfaceHostedViewDidMoveToWindow")
    static let mainWindowContextsDidChange = Notification.Name("phatmux.mainWindowContextsDidChange")
    static let browserDownloadEventDidArrive = Notification.Name("phatmux.browserDownloadEventDidArrive")
}
...
class TerminalController {
    ...
    private var accessMode: SocketControlMode = .phatmuxOnly
    ...
    private static let v2BrowserEvalEnvelopeTypeKey = "__phatmux_t"
    private static let v2BrowserEvalEnvelopeValueKey = "__phatmux_v"
    ...
    private final class SocketFastPathState: @unchecked Sendable {
        private let queue = DispatchQueue(label: "com.phatmux.socket-fast-path")
    ...
    private func handleClient(_ socket: Int32, peerPid: pid_t? = nil) {
        ...
        // In phatmuxOnly mode, verify the connecting process is a descendant of phatmux.
        // In allowAll mode (env-var only), skip the ancestry check.
        if accessMode == .phatmuxOnly {
            ...
            if let pid {
                guard isDescendant(pid) else {
                    let msg = "ERROR: Access denied — only processes started inside phatmux can connect\n"
    ...
    private func v2RunBrowserJavaScript(
        ...
        if let frameSelector = v2BrowserCurrentFrameSelector(surfaceId: surfaceId) {
            ...
            framePrelude = """
            let __phatmuxDoc = document;
            try {
              const __phatmuxFrame = document.querySelector(\(selectorLiteral));
              if (__phatmuxFrame && __phatmuxFrame.contentDocument) {
                __phatmuxDoc = __phatmuxFrame.contentDocument;
              }
            } catch (_) {}
            """
        } else {
            framePrelude = "const __phatmuxDoc = document;"
        }
        ...
        const __phatmuxMaybeAwait = async (__r) => {
        ...
        const __phatmuxEvalInFrame = async function() {
          const document = __phatmuxDoc;
          \(executionBlock)
          const __value = await __phatmuxMaybeAwait(__r);
          return {
            __phatmux_t: (typeof __value === 'undefined') ? 'undefined' : 'value',
            __phatmux_v: __value
          };
        };

        return await __phatmuxEvalInFrame();
        """
    ...
    private func v2BrowserEnsureInitScriptsApplied(surfaceId: UUID, browserPanel: BrowserPanel) {
        ...
        let injector = """
        (() => {
          window.__phatmuxInitScriptsApplied = window.__phatmuxInitScriptsApplied || { scripts: [], styles: [] };
          return true;
        })()
        """
        ...
        for css in styles {
            ...
            let styleScript = """
            (() => {
              const id = 'phatmux-init-style-' + btoa(unescape(encodeURIComponent(\(cssLiteral)))).replace(/=+$/g, '');
    ...
    private func v2BrowserScreenshot(params: [String: Any]) -> V2CallResult {
        return v2BrowserWithPanel(params: params) { _, ws, surfaceId, browserPanel in
            ...
            let screenshotsDirectory = FileManager.default.temporaryDirectory
                .appendingPathComponent("phatmux-browser-screenshots", isDirectory: true)
    ...
    private func v2BrowserFindWithScript(
        ...
        return v2BrowserWithPanel(params: params) { _, ws, surfaceId, browserPanel in
            ...
            (() => {
              ...
              const __phatmuxFound = (() => {
            \(finderBody)
              })();
              if (!__phatmuxFound) return { ok: false, error: 'not_found' };
              const selector = __phatmuxCssPath(__phatmuxFound);
              if (!selector) return { ok: false, error: 'not_found' };
              return {
                ok: true,
                selector,
                tag: String(__phatmuxFound.tagName || '').toLowerCase(),
                text: String(__phatmuxFound.textContent || '').trim()
              };
            })()
            """
    ...
    private func v2BrowserDialogRespond(params: [String: Any], accept: Bool) -> V2CallResult {
        return v2BrowserWithPanel(params: params) { _, ws, surfaceId, browserPanel in
            ...
            let script = """
            (() => {
              const q = window.__phatmuxDialogQueue || [];
              ...
              if (entry.type === 'confirm') {
                ...
                window.__phatmuxDialogDefaults.confirm = \(acceptLiteral);
              }
              if (entry.type === 'prompt') {
                window.__phatmuxDialogDefaults = window.__phatmuxDialogDefaults || { confirm: false, prompt: null };
                if (\(acceptLiteral)) {
                  window.__phatmuxDialogDefaults.prompt = \(textLiteral);
                } else {
                  window.__phatmuxDialogDefaults.prompt = null;
                }
              }
    ...
    private func v2BrowserConsoleList(params: [String: Any]) -> V2CallResult {
        return v2BrowserWithPanel(params: params) { _, ws, surfaceId, browserPanel in
            ...
            let script = """
            (() => {
              const items = Array.isArray(window.__phatmuxConsoleLog) ? window.__phatmuxConsoleLog.slice() : [];
              if (\(clearLiteral)) {
                window.__phatmuxConsoleLog = [];
              }
    ...
    private func v2BrowserErrorsList(params: [String: Any]) -> V2CallResult {
        return v2BrowserWithPanel(params: params) { _, ws, surfaceId, browserPanel in
            ...
            let script = """
            (() => {
              const items = Array.isArray(window.__phatmuxErrorLog) ? window.__phatmuxErrorLog.slice() : [];
              if (\(clearLiteral)) {
                window.__phatmuxErrorLog = [];
              }
              return { ok: true, items };
            })()
            """
    ...
    private func dragPasteboardType(from token: String) -> NSPasteboard.PasteboardType? {
        ...
        case "sidebarreorder", "sidebar-reorder", "sidebar_tab_reorder",
            "com.phatmux.sidebar-tab-reorder":
    ...
    private func panelSnapshot(_ args: String) -> String {
        ...
        let outputDir = FileManager.default.temporaryDirectory
            .appendingPathComponent("phatmux-screenshots")
