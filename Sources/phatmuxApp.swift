// Main SwiftUI app entry. Upstream cmux: `Sources/cmuxApp.swift` (`struct cmuxApp`).
import AppKit
import SwiftUI
import Darwin
import Bonsplit
...
enum UITestLaunchManifest {
    static let argumentName = "-phatmuxUITestLaunchManifest"
...
struct phatmuxApp: App {
    ...
    init() {
        ...
        if let stored = defaults.string(forKey: SocketControlSettings.appStorageKey) {
            ...
            defaults.set(legacy ? SocketControlMode.phatmuxOnly.rawValue : SocketControlMode.off.rawValue,
    ...
    private static func terminateForMissingLaunchTag() -> Never {
        let message = "error: refusing to launch untagged phatmux DEV; start with ./scripts/reload.sh --tag <name> (or set PHATMUX_TAG for test harnesses)"
        fputs("\(message)\n", stderr)
        fflush(stderr)
        NSLog("%@", message)
        Darwin.exit(64)
    ...
    var body: some Scene {
        WindowGroup {
            ContentView(updateViewModel: appDelegate.updateViewModel, windowId: primaryWindowId)
                ...
                .onAppear {
#if DEBUG
                    if ProcessInfo.processInfo.environment["PHATMUX_UI_TEST_MODE"] == "1" {
                        UpdateLogStore.shared.append("ui test: phatmuxApp onAppear")
                    }
#endif
                    ...
                    appDelegate.configure(tabManager: tabManager, notificationStore: notificationStore, sidebarState: sidebarState)
                    applyAppearance()
                    if ProcessInfo.processInfo.environment["PHATMUX_UI_TEST_SHOW_SETTINGS"] == "1" {
        ...
        .commands {
            ...
            CommandGroup(replacing: .appInfo) {
                Button(String(localized: "menu.app.about", defaultValue: "About phatmux")) {
                    showAboutPanel()
                }
    ...
    private func closePanelOrWindow() {
        if let window = NSApp.keyWindow ?? NSApp.mainWindow,
           phatmuxWindowShouldOwnCloseShortcut(window) {
...
private let phatmuxAuxiliaryWindowIdentifiers: Set<String> = [
    "phatmux.settings",
    "phatmux.about",
    "phatmux.licenses",
    "phatmux.browser-popup",
    "phatmux.settingsAboutTitlebarDebug",
    "phatmux.debugWindowControls",
    "phatmux.browserImportHintDebug",
    "phatmux.sidebarDebug",
    "phatmux.menubarDebug",
    "phatmux.backgroundDebug",
]

/// Returns whether the given window should handle the standard close shortcut
/// as a standalone auxiliary window instead of routing it through workspace or
/// panel-close behavior.
func phatmuxWindowShouldOwnCloseShortcut(_ window: NSWindow?) -> Bool {
    guard let identifier = window?.identifier?.rawValue else { return false }
    return phatmuxAuxiliaryWindowIdentifiers.contains(identifier)
}
...
private enum SettingsAboutWindowKind: String, CaseIterable, Identifiable {
    ...
    var windowIdentifier: String {
        switch self {
        case .settings:
            return "phatmux.settings"
        case .about:
            return "phatmux.about"
        }
    }
...
private final class SettingsAboutTitlebarDebugStore: ObservableObject {
    ...
    private func ensureToolbar(on window: NSWindow, kind: SettingsAboutWindowKind) {
        ...
        let identifier = NSToolbar.Identifier("phatmux.debug.titlebar.\(kind.rawValue)")
...
private final class SettingsAboutTitlebarDebugWindowController: NSWindowController, NSWindowDelegate {
    ...
    private init() {
        ...
        window.identifier = NSUserInterfaceItemIdentifier("phatmux.settingsAboutTitlebarDebug")
        window.center()
    ...
    private func editor(for kind: SettingsAboutWindowKind) -> some View {
        ...
        return GroupBox(kind.displayTitle) {
            VStack(alignment: .leading, spacing: 10) {
                ...
                Text("When disabled, phatmux uses normal default titlebar behavior for this window.")
                    .font(.caption)
...
private final class DebugWindowControlsWindowController: NSWindowController, NSWindowDelegate {
    ...
    private init() {
        ...
        window.isMovableByWindowBackground = true
        window.isReleasedWhenClosed = false
        window.identifier = NSUserInterfaceItemIdentifier("phatmux.debugWindowControls")
        window.center()
...
private final class BrowserImportHintDebugWindowController: NSWindowController, NSWindowDelegate {
    ...
    private init() {
        ...
        window.isMovableByWindowBackground = true
        window.isReleasedWhenClosed = false
        window.identifier = NSUserInterfaceItemIdentifier("phatmux.browserImportHintDebug")
        window.center()
...
private final class BrowserProfilePopoverDebugWindowController: NSWindowController, NSWindowDelegate {
    ...
    private init() {
        ...
        window.isMovableByWindowBackground = true
        window.isReleasedWhenClosed = false
        window.identifier = NSUserInterfaceItemIdentifier("phatmux.browserProfilePopoverDebug")
        window.center()
...
private final class AboutWindowController: NSWindowController, NSWindowDelegate {
    ...
    private init() {
        ...
        window.isReleasedWhenClosed = false
        window.identifier = NSUserInterfaceItemIdentifier("phatmux.about")
        window.center()
...
private final class AcknowledgmentsWindowController: NSWindowController, NSWindowDelegate {
    ...
    private init() {
        ...
        window.identifier = NSUserInterfaceItemIdentifier("phatmux.licenses")
        window.center()
...
final class SettingsWindowController: NSWindowController, NSWindowDelegate {
    ...
    private init() {
        ...
        window.isReleasedWhenClosed = false
        window.identifier = NSUserInterfaceItemIdentifier("phatmux.settings")
        window.center()
...
enum SettingsNavigationRequest {
    static let notificationName = Notification.Name("phatmux.settings.navigate")
...
private final class SidebarDebugWindowController: NSWindowController, NSWindowDelegate {
    ...
    private init() {
        ...
        window.isMovableByWindowBackground = true
        window.isReleasedWhenClosed = false
        window.identifier = NSUserInterfaceItemIdentifier("phatmux.sidebarDebug")
        window.center()
...
private struct AboutPanelView: View {
    ...
    private var commit: String? {
        if let value = Bundle.main.infoDictionary?["PHATMUXCommit"] as? String, !value.isEmpty {
            return value
        }
        let env = ProcessInfo.processInfo.environment["PHATMUX_COMMIT"] ?? ""
        ...
        VStack(alignment: .center) {
            ...
            VStack(alignment: .center, spacing: 32) {
                VStack(alignment: .center, spacing: 8) {
                    Text(String(localized: "about.appName", defaultValue: String.LocalizationValue(stringLiteral: Branding.appName)))
                        .bold()
                        .font(.title)
...
private final class MenuBarExtraDebugWindowController: NSWindowController, NSWindowDelegate {
    ...
    private init() {
        ...
        window.isMovableByWindowBackground = true
        window.isReleasedWhenClosed = false
        window.identifier = NSUserInterfaceItemIdentifier("phatmux.menubarDebug")
        window.center()
...
private final class BackgroundDebugWindowController: NSWindowController, NSWindowDelegate {
    ...
    private init() {
        ...
        window.isMovableByWindowBackground = true
        window.isReleasedWhenClosed = false
        window.identifier = NSUserInterfaceItemIdentifier("phatmux.backgroundDebug")
        window.center()
...
enum WelcomeSettings {
    static let shownKey = "phatmuxWelcomeShown"
}
...
struct SettingsView: View {
    ...
    @AppStorage("phatmuxPortBase") private var phatmuxPortBase = 9100
    @AppStorage("phatmuxPortRange") private var phatmuxPortRange = 10
    ...
    private var paneFirstClickFocusSubtitle: String {
        if paneFirstClickFocusEnabled {
            return String(
                ...
                defaultValue: "When phatmux is inactive, clicking a pane activates the window and focuses that pane in one click."
            )
        }
        return String(
            localized: "settings.app.paneFirstClickFocus.subtitleOff",
            defaultValue: "When phatmux is inactive, the first click only activates the window. Click again to focus the pane."
        )
    }
            ...
            ScrollView {
                VStack(alignment: .leading, spacing: 14) {
                    ...
                    SettingsCard {
                        SettingsCardRow(
                            ...
                            subtitle: appLanguage != LanguageSettings.languageAtLaunch.rawValue
                                ? String(localized: "settings.app.language.restartSubtitle", defaultValue: "Restart phatmux to apply")
                                : nil,
                        ...
                        SettingsCardRow(
                            ...
                            subtitle: String(localized: "settings.app.showInMenuBar.subtitle", defaultValue: "Keep phatmux in the menu bar for unread notifications and quick actions.")
                        ) {
                        ...
                        SettingsCardRow(
                            ...
                            subtitle: String(localized: "settings.notifications.paneFlash.subtitle", defaultValue: "Briefly flash a blue outline when phatmux highlights a pane.")
                        ) {
                        ...
                        SettingsCardRow(
                            ...
                            subtitle: "Run a shell command when a notification arrives. $PHATMUX_NOTIFICATION_TITLE, $PHATMUX_NOTIFICATION_SUBTITLE, $PHATMUX_NOTIFICATION_BODY are set."
                        ) {
                        ...
                        SettingsCardRow(
                            ...
                            subtitle: sendAnonymousTelemetry != telemetryValueAtLaunch
                                ...
                                : String(localized: "settings.app.telemetry.subtitle", defaultValue: "Share anonymized crash and usage data to help improve phatmux.")
                        ) {
                        ...
                        SettingsCardRow(
                            String(localized: "settings.app.openSidebarPRLinks", defaultValue: "Open Sidebar PR Links in phatmux Browser"),
                            subtitle: openSidebarPullRequestLinksInCmuxBrowser
                                ? String(localized: "settings.app.openSidebarPRLinks.subtitleOn", defaultValue: "Clicks open inside phatmux browser.")
                    ...
                    SettingsCard {
                        ...
                        SettingsCardNote(String(localized: "settings.automation.socketOverrides.note", defaultValue: "Overrides: PHATMUX_SOCKET_ENABLE, PHATMUX_SOCKET_MODE, and PHATMUX_SOCKET_PATH (set PHATMUX_ALLOW_SOCKET_OVERRIDE=1 for stable/nightly builds)."))
                    }
                    ...
                    SettingsCard {
                        SettingsCardRow(
                            ...
                            subtitle: claudeCodeHooksEnabled
                                ...
                                : String(localized: "settings.automation.claudeCode.subtitleOff", defaultValue: "Claude Code runs without phatmux integration.")
                        ) {
                        ...
                        SettingsCardNote(String(localized: "settings.automation.claudeCode.note", defaultValue: "When enabled, phatmux wraps the claude command to inject session tracking and notification hooks. Disable if you prefer to manage Claude Code hooks yourself."))
                    }

                    SettingsCard {
                        SettingsCardRow(String(localized: "settings.automation.portBase", defaultValue: "Port Base"), subtitle: String(localized: "settings.automation.portBase.subtitle", defaultValue: "Starting port for PHATMUX_PORT env var."), controlWidth: pickerColumnWidth) {
                            TextField("", value: $phatmuxPortBase, format: .number)
                                .textFieldStyle(.roundedBorder)
                                .multilineTextAlignment(.trailing)
                        }
                        ...
                        SettingsCardRow(String(localized: "settings.automation.portRange", defaultValue: "Port Range Size"), subtitle: String(localized: "settings.automation.portRange.subtitle", defaultValue: "Number of ports per workspace."), controlWidth: pickerColumnWidth) {
                            TextField("", value: $phatmuxPortRange, format: .number)
                                .textFieldStyle(.roundedBorder)
                                .multilineTextAlignment(.trailing)
                        }
                        ...
                        SettingsCardNote(String(localized: "settings.automation.port.note", defaultValue: "Each workspace gets PHATMUX_PORT and PHATMUX_PORT_END env vars with a dedicated port range. New terminals inherit these values."))
                    }
                    ...
                    SettingsCard {
                        ...
                        SettingsCardRow(
                            String(localized: "settings.browser.openTerminalLinks", defaultValue: "Open Terminal Links in phatmux Browser"),
                        ...
                        if openTerminalLinksInCmuxBrowser || interceptTerminalOpenCommandInCmuxBrowser {
                            ...
                            VStack(alignment: .leading, spacing: 6) {
                                SettingsCardRow(
                                    ...
                                    subtitle: String(localized: "settings.browser.hostWhitelist.subtitle", defaultValue: "Applies to terminal link clicks and intercepted `open https://...` calls. Only these hosts open in phatmux. Others open in your default browser. One host or wildcard per line (for example: example.com, *.internal.example). Leave empty to open all hosts in phatmux.")
                                ) {
                                    EmptyView()
                                }
                        ...
                        VStack(alignment: .leading, spacing: 8) {
                            ...
                            Text(String(localized: "settings.browser.httpAllowlist.description", defaultValue: "Controls which HTTP (non-HTTPS) hosts can open in phatmux without a warning prompt. Defaults include localhost, 127.0.0.1, ::1, 0.0.0.0, and *.localtest.me."))
                                .font(.caption)
                                .foregroundStyle(.secondary)
...
private struct SettingsRootView: View {
    ...
    private func configureSettingsWindow(_ window: NSWindow) {
        ...
        for index in accessories.indices.reversed() {
            ...
            guard identifier.hasPrefix("phatmux.") else { continue }
