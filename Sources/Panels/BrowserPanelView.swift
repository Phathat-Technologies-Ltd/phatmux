...
private extension View {
    func phatmuxFlatSymbolColorRendering() -> some View {
...
struct BrowserPanelView: View {
    ...
    private var remoteSuggestionsEnabled: Bool {
        ...
        if ProcessInfo.processInfo.environment["PHATMUX_UI_TEST_REMOTE_SUGGESTIONS_JSON"] != nil ||
            UserDefaults.standard.string(forKey: "PHATMUX_UI_TEST_REMOTE_SUGGESTIONS_JSON") != nil {
            return true
        }
        // Keep UI tests deterministic by disabling network suggestions when requested.
        if ProcessInfo.processInfo.environment["PHATMUX_UI_TEST_DISABLE_REMOTE_SUGGESTIONS"] == "1" {
            return false
        }
    ...
    var body: some View {
        ...
        .overlay {
            RoundedRectangle(cornerRadius: FocusFlashPattern.ringCornerRadius)
                .stroke(phatmuxAccentColor().opacity(focusFlashOpacity), lineWidth: 3)
                .shadow(color: phatmuxAccentColor().opacity(focusFlashOpacity * 0.35), radius: 10)
    ...
    private var developerToolsButton: some View {
        Button(action: {
            ...
            Image(systemName: devToolsIconOption.rawValue)
                ...
                .phatmuxFlatSymbolColorRendering()
    ...
    private var omnibarField: some View {
        ...
        .overlay(
            RoundedRectangle(cornerRadius: omnibarPillCornerRadius, style: .continuous)
                .stroke(addressBarFocused ? phatmuxAccentColor() : Color.clear, lineWidth: 1)
        )
    ...
    private func syncWebViewResponderPolicyWithViewState(
        ...
        guard let phatmuxWebView = panel.webView as? CmuxWebView else { return }
        let isPanelFocused = isPanelFocusedOverride ?? isFocused
        let next = isPanelFocused && !panel.shouldSuppressWebViewFocus()
        if phatmuxWebView.allowsFirstResponderAcquisition != next {
#if DEBUG
            dlog(
                "browser.focus.policy.resync panel=\(panel.id.uuidString.prefix(5)) " +
                "web=\(ObjectIdentifier(phatmuxWebView)) old=\(phatmuxWebView.allowsFirstResponderAcquisition ? 1 : 0) " +
                "new=\(next ? 1 : 0) reason=\(reason) " +
                "panelFocusedUsed=\(isPanelFocused ? 1 : 0)"
            )
#endif
        }
        phatmuxWebView.allowsFirstResponderAcquisition = next
    }
    ...
    private func forcedRemoteSuggestionsForUITest() -> [String]? {
        let raw = ProcessInfo.processInfo.environment["PHATMUX_UI_TEST_REMOTE_SUGGESTIONS_JSON"]
...
struct WebViewRepresentable: NSViewRepresentable {
        ...
        private func updateHostedInspectorDockControlAvailabilityIfNeeded(reason: String) {
            ...
            hostedInspectorFrontendWebView.evaluateJavaScript(
                ...
                (() => {
                    ...
                    if (!WI.__phatmuxOriginalUpdateDockNavigationItems && typeof WI._updateDockNavigationItems === "function")
                        WI.__phatmuxOriginalUpdateDockNavigationItems = WI._updateDockNavigationItems;
                    if (!WI.__phatmuxOriginalDockLeft && typeof WI._dockLeft === "function")
                        WI.__phatmuxOriginalDockLeft = WI._dockLeft;
                    if (!WI.__phatmuxOriginalDockRight && typeof WI._dockRight === "function")
                        WI.__phatmuxOriginalDockRight = WI._dockRight;
                    if (!WI.__phatmuxOriginalTogglePreviousDockConfiguration && typeof WI._togglePreviousDockConfiguration === "function")
                        WI.__phatmuxOriginalTogglePreviousDockConfiguration = WI._togglePreviousDockConfiguration;
                    ...
                    function enforceDockControls() {
                        const disallowSideDock = !WI.__phatmuxAllowSideDock;
                    ...
                    WI.__phatmuxAllowSideDock = allowSideDock;
                    WI._dockLeft = function(event) {
                        if (!WI.__phatmuxAllowSideDock)
                            return callOriginal(WI._dockBottom, event);
                        return callOriginal(WI.__phatmuxOriginalDockLeft, event);
                    };
                    WI._dockRight = function(event) {
                        if (!WI.__phatmuxAllowSideDock)
                            return callOriginal(WI._dockBottom, event);
                        return callOriginal(WI.__phatmuxOriginalDockRight, event);
                    };
                    WI._togglePreviousDockConfiguration = function(event) {
                        ...
                        if (!WI.__phatmuxAllowSideDock && previousSideDock)
                            return callOriginal(WI._dockBottom, event);
                        return callOriginal(WI.__phatmuxOriginalTogglePreviousDockConfiguration, event);
                    };
                    WI._updateDockNavigationItems = function(...args) {
                        if (typeof WI.__phatmuxOriginalUpdateDockNavigationItems === "function")
                            WI.__phatmuxOriginalUpdateDockNavigationItems.apply(WI, args);
                        enforceDockControls();
                    };
                    WI._updateDockNavigationItems();
                    return WI.__phatmuxAllowSideDock;
                })();
                """,
    ...
    private func updateUsingLocalInlineHosting(_ nsView: NSView, context: Context, webView: WKWebView) -> Bool {
        ...
        host.setHostedInspectorFrontendWebView(webView.phatmuxInspectorFrontendWebView())
    ...
    private static func applyWebViewFirstResponderPolicy(
        ...
        guard let phatmuxWebView = webView as? CmuxWebView else { return }
        let next = isPanelFocused && !panel.shouldSuppressWebViewFocus()
        if phatmuxWebView.allowsFirstResponderAcquisition != next {
#if DEBUG
            dlog(
