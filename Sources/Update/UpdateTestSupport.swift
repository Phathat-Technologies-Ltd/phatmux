...
enum UpdateTestSupport {
    static func applyIfNeeded(to viewModel: UpdateViewModel) {
        ...
        if let detectedVersion = env["PHATMUX_UI_TEST_DETECTED_UPDATE_VERSION"],
        ...
        guard let state = env["PHATMUX_UI_TEST_UPDATE_STATE"] else { return }

        DispatchQueue.main.async {
            switch state {
            case "available":
                let version = env["PHATMUX_UI_TEST_UPDATE_VERSION"] ?? "9.9.9"
    ...
    static func performMockFeedCheckIfNeeded(on viewModel: UpdateViewModel) -> Bool {
        let env = ProcessInfo.processInfo.environment
        guard env["PHATMUX_UI_TEST_TRIGGER_UPDATE_CHECK"] == "1" else { return false }
        guard let feedURLString = env["PHATMUX_UI_TEST_FEED_URL"],
        ...
        let task = URLSession.shared.dataTask(with: feedURL) { data, _, _ in
            ...
            DispatchQueue.main.async {
                let delayMilliseconds = Int(env["PHATMUX_UI_TEST_MOCK_FEED_DELAY_MS"] ?? "") ?? 0
