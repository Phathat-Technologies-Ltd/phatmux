...
private var phatmuxWindowBrowserPortalKey: UInt8 = 0
private var phatmuxWindowBrowserPortalCloseObserverKey: UInt8 = 0
private var phatmuxBrowserSearchOverlayPanelIdAssociationKey: UInt8 = 0
private var phatmuxBrowserPortalNeedsRenderingStateReattachKey: UInt8 = 0
...
private extension WKWebView {
    private var browserPortalNeedsRenderingStateReattach: Bool {
        get {
            (objc_getAssociatedObject(self, &phatmuxBrowserPortalNeedsRenderingStateReattachKey) as? NSNumber)?
                .boolValue ?? false
        }
        set {
            objc_setAssociatedObject(
                self,
                &phatmuxBrowserPortalNeedsRenderingStateReattachKey,
...
final class WindowBrowserSlotView: NSView {
    ...
    func setSearchOverlay(_ configuration: BrowserPortalSearchOverlayConfiguration?) {
        guard let configuration else {
            ...
            if let overlay = searchOverlayHostingView {
                objc_setAssociatedObject(
                    overlay,
                    &phatmuxBrowserSearchOverlayPanelIdAssociationKey,
                    nil,
                    .OBJC_ASSOCIATION_RETAIN_NONATOMIC
                )
            }
    ...
    func searchOverlayPanelId(for responder: NSResponder) -> UUID? {
        ...
        let panelId = objc_getAssociatedObject(overlay, &phatmuxBrowserSearchOverlayPanelIdAssociationKey) as? UUID
...
enum BrowserWindowPortalRegistry {
    ...
    private static func installWindowCloseObserverIfNeeded(for window: NSWindow) {
        guard objc_getAssociatedObject(window, &phatmuxWindowBrowserPortalCloseObserverKey) == nil else { return }
        ...
        objc_setAssociatedObject(
            window,
            &phatmuxWindowBrowserPortalCloseObserverKey,
            observer,
            .OBJC_ASSOCIATION_RETAIN_NONATOMIC
        )
    }
    ...
    private static func removePortal(windowId: ObjectIdentifier, window: NSWindow?) {
        ...
        if let observer = objc_getAssociatedObject(window, &phatmuxWindowBrowserPortalCloseObserverKey) {
            NotificationCenter.default.removeObserver(observer)
        }
        objc_setAssociatedObject(window, &phatmuxWindowBrowserPortalCloseObserverKey, nil, .OBJC_ASSOCIATION_RETAIN_NONATOMIC)
        objc_setAssociatedObject(window, &phatmuxWindowBrowserPortalKey, nil, .OBJC_ASSOCIATION_RETAIN)
    }
    ...
    private static func portal(for window: NSWindow) -> WindowBrowserPortal {
        if let existing = objc_getAssociatedObject(window, &phatmuxWindowBrowserPortalKey) as? WindowBrowserPortal {
        ...
        objc_setAssociatedObject(window, &phatmuxWindowBrowserPortalKey, portal, .OBJC_ASSOCIATION_RETAIN)
