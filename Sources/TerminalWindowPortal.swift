...
private var phatmuxWindowTerminalPortalKey: UInt8 = 0
private var phatmuxWindowTerminalPortalCloseObserverKey: UInt8 = 0
...
#if PHATMUX_ISSUE_483_PORTAL_RECOVERY
...
enum TerminalWindowPortalRegistry {
    ...
    private static func installWindowCloseObserverIfNeeded(for window: NSWindow) {
        guard objc_getAssociatedObject(window, &phatmuxWindowTerminalPortalCloseObserverKey) == nil else { return }
        ...
        objc_setAssociatedObject(
            window,
            &phatmuxWindowTerminalPortalCloseObserverKey,
            observer,
    ...
    private static func removePortal(windowId: ObjectIdentifier, window: NSWindow?) {
        ...
        if let observer = objc_getAssociatedObject(window, &phatmuxWindowTerminalPortalCloseObserverKey) {
            NotificationCenter.default.removeObserver(observer)
        }
        objc_setAssociatedObject(window, &phatmuxWindowTerminalPortalCloseObserverKey, nil, .OBJC_ASSOCIATION_RETAIN_NONATOMIC)
        objc_setAssociatedObject(window, &phatmuxWindowTerminalPortalKey, nil, .OBJC_ASSOCIATION_RETAIN)
    }
    ...
    private static func portal(for window: NSWindow) -> WindowTerminalPortal {
        if let existing = objc_getAssociatedObject(window, &phatmuxWindowTerminalPortalKey) as? WindowTerminalPortal {
        ...
        objc_setAssociatedObject(window, &phatmuxWindowTerminalPortalKey, portal, .OBJC_ASSOCIATION_RETAIN)
