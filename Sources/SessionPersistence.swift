...
enum SessionRestorePolicy {
    static func isRunningUnderAutomatedTests(
        ...
        if environment["PHATMUX_UI_TEST_MODE"] == "1" {
            return true
        }
    ...
    static func shouldAttemptRestore(
        ...
        if environment["PHATMUX_DISABLE_SESSION_RESTORE"] == "1" {
            return false
        }
...
enum SessionScrollbackReplayStore {
    static let environmentKey = "PHATMUX_RESTORE_SCROLLBACK_FILE"
    private static let directoryName = "phatmux-session-scrollback"
