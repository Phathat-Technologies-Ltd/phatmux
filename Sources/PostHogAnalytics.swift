...
final class PostHogAnalytics {
    ...
    // PostHog Cloud US default (matches other phatmux properties).
    private let host = "https://us.i.posthog.com"

    private let dailyActiveEvent = "phatmux_daily_active"
    private let hourlyActiveEvent = "phatmux_hourly_active"
    ...
    private init() {
        workQueue = DispatchQueue(label: "com.phatmux.posthog.analytics", qos: .utility)
    ...
    private var isEnabled: Bool {
        ...
        return ProcessInfo.processInfo.environment["PHATMUX_POSTHOG_ENABLE"] == "1"
#else
...
#if DEBUG
        config.debug = ProcessInfo.processInfo.environment["PHATMUX_POSTHOG_DEBUG"] == "1"
#endif
    ...
    nonisolated static func superProperties(infoDictionary: [String: Any]) -> [String: Any] {
        var properties: [String: Any] = ["platform": "phatmux"]
    ...
    nonisolated static func shouldFlushAfterCapture(event: String) -> Bool {
        switch event {
        case "phatmux_daily_active", "phatmux_hourly_active":
            return true
        default:
            return false
        }
