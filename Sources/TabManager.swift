...
fileprivate func phatmuxVsyncIOSurfaceTimelineCallback(
...
class TabManager: ObservableObject {
    ...
    /// Global monotonically increasing counter for PHATMUX_PORT ordinal assignment.
    ...
    private let initialWorkspaceGitProbeQueue = DispatchQueue(
        label: "com.phatmux.initial-workspace-git-probe",
        qos: .utility
    )
        ...
        if let terminalPanel = workspace.focusedTerminalPanel,
           terminalPanel.surface.surface != nil {
            DispatchQueue.main.asyncAfter(deadline: .now() + 0.5) {
                ...
                terminalPanel.sendText("phatmux welcome\n")
            }
            return
        }
    ...
    private func inheritedTerminalConfigForNewWorkspace(
        ...
        if let sourceSurface = terminalPanelForWorkspaceConfigInheritanceSource(snapshot: snapshot)?.surface.surface {
            return phatmuxInheritedSurfaceConfig(
...
#if DEBUG
        if ProcessInfo.processInfo.environment["PHATMUX_UI_TEST_FORCE_CONFIRM_CLOSE_WORKSPACE"] == "1" {
            return true
        }
#endif
    ...
    private func windowTitle(for tab: Workspace?) -> String {
        guard let tab else { return "phatmux" }
        ...
        return trimmedDirectory.isEmpty ? "phatmux" : trimmedDirectory
    }
    ...
    private func setupUITestFocusShortcutsIfNeeded() {
        ...
        guard env["PHATMUX_UI_TEST_FOCUS_SHORTCUTS"] == "1" else { return }
    ...
    private func setupSplitCloseRightUITestIfNeeded() {
        ...
        let env = ProcessInfo.processInfo.environment
        guard env["PHATMUX_UI_TEST_SPLIT_CLOSE_RIGHT_SETUP"] == "1" else { return }
        guard let path = env["PHATMUX_UI_TEST_SPLIT_CLOSE_RIGHT_PATH"], !path.isEmpty else { return }
        let visualMode = env["PHATMUX_UI_TEST_SPLIT_CLOSE_RIGHT_VISUAL"] == "1"
        let shotsDir = (env["PHATMUX_UI_TEST_SPLIT_CLOSE_RIGHT_SHOTS_DIR"] ?? "").trimmingCharacters(in: .whitespacesAndNewlines)
        let visualIterations = Int((env["PHATMUX_UI_TEST_SPLIT_CLOSE_RIGHT_ITERATIONS"] ?? "20").trimmingCharacters(in: .whitespacesAndNewlines)) ?? 20
        let burstFrames = Int((env["PHATMUX_UI_TEST_SPLIT_CLOSE_RIGHT_BURST_FRAMES"] ?? "6").trimmingCharacters(in: .whitespacesAndNewlines)) ?? 6
        let closeDelayMs = Int((env["PHATMUX_UI_TEST_SPLIT_CLOSE_RIGHT_CLOSE_DELAY_MS"] ?? "70").trimmingCharacters(in: .whitespacesAndNewlines)) ?? 70
        let pattern = (env["PHATMUX_UI_TEST_SPLIT_CLOSE_RIGHT_PATTERN"] ?? "close_right")
            .trimmingCharacters(in: .whitespacesAndNewlines)
        ...
        for i in 1...iterations {
            ...
            sendText(topLeftId, "printf '\\033[2J\\033[H'; for i in {1..200}; do echo PHATMUX_SPLIT_TOPLEFT_\(i); done; printf '\\033[HPHATMUX_MARKER_TOPLEFT\\n'\r")
            sendText(topRight.id, "printf '\\033[2J\\033[H'; for i in {1..200}; do echo PHATMUX_SPLIT_TOPRIGHT_\(i); done; printf '\\033[HPHATMUX_MARKER_TOPRIGHT\\n'\r")
            if let bottomLeft {
                sendText(bottomLeft.id, "printf '\\033[2J\\033[H'; for i in {1..200}; do echo PHATMUX_SPLIT_BOTTOMLEFT_\(i); done; printf '\\033[HPHATMUX_MARKER_BOTTOMLEFT\\n'\r")
            }
            if let bottomRight {
                sendText(bottomRight.id, "printf '\\033[2J\\033[H'; for i in {1..200}; do echo PHATMUX_SPLIT_BOTTOMRIGHT_\(i); done; printf '\\033[HPHATMUX_MARKER_BOTTOMRIGHT\\n'\r")
            }
	    ...
	    private func captureVsyncIOSurfaceTimeline(
	        ...
	        await withCheckedContinuation { (cont: CheckedContinuation<Void, Never>) in
	            ...
	            CVDisplayLinkSetOutputCallback(link, phatmuxVsyncIOSurfaceTimelineCallback, ctx)
    ...
    private func setupChildExitSplitUITestIfNeeded() {
        ...
        let env = ProcessInfo.processInfo.environment
        guard env["PHATMUX_UI_TEST_CHILD_EXIT_SPLIT_SETUP"] == "1" else { return }
        guard let path = env["PHATMUX_UI_TEST_CHILD_EXIT_SPLIT_PATH"], !path.isEmpty else { return }
        let requestedIterations = Int(env["PHATMUX_UI_TEST_CHILD_EXIT_SPLIT_ITERATIONS"] ?? "1") ?? 1
    ...
    private func setupChildExitKeyboardUITestIfNeeded() {
        ...
        let env = ProcessInfo.processInfo.environment
        guard env["PHATMUX_UI_TEST_CHILD_EXIT_KEYBOARD_SETUP"] == "1" else { return }
        guard let path = env["PHATMUX_UI_TEST_CHILD_EXIT_KEYBOARD_PATH"], !path.isEmpty else { return }
        let autoTrigger = env["PHATMUX_UI_TEST_CHILD_EXIT_KEYBOARD_AUTO_TRIGGER"] == "1"
        let strictKeyOnly = env["PHATMUX_UI_TEST_CHILD_EXIT_KEYBOARD_STRICT"] == "1"
        let triggerMode = (env["PHATMUX_UI_TEST_CHILD_EXIT_KEYBOARD_TRIGGER_MODE"] ?? "shell_input")
            .trimmingCharacters(in: .whitespacesAndNewlines)
        ...
        let layout = (env["PHATMUX_UI_TEST_CHILD_EXIT_KEYBOARD_LAYOUT"] ?? "lr")
            .trimmingCharacters(in: .whitespacesAndNewlines)
        let expectedPanelsAfter = max(
            1,
            Int((env["PHATMUX_UI_TEST_CHILD_EXIT_KEYBOARD_EXPECTED_PANELS_AFTER"] ?? "1")
                .trimmingCharacters(in: .whitespacesAndNewlines)
            ) ?? 1
        )
...
extension Notification.Name {
    static let commandPaletteToggleRequested = Notification.Name("phatmux.commandPaletteToggleRequested")
    static let commandPaletteRequested = Notification.Name("phatmux.commandPaletteRequested")
    static let commandPaletteSwitcherRequested = Notification.Name("phatmux.commandPaletteSwitcherRequested")
    static let commandPaletteSubmitRequested = Notification.Name("phatmux.commandPaletteSubmitRequested")
    static let commandPaletteDismissRequested = Notification.Name("phatmux.commandPaletteDismissRequested")
    static let commandPaletteRenameTabRequested = Notification.Name("phatmux.commandPaletteRenameTabRequested")
    static let commandPaletteRenameWorkspaceRequested = Notification.Name("phatmux.commandPaletteRenameWorkspaceRequested")
    static let commandPaletteMoveSelection = Notification.Name("phatmux.commandPaletteMoveSelection")
    static let commandPaletteRenameInputInteractionRequested = Notification.Name("phatmux.commandPaletteRenameInputInteractionRequested")
    static let commandPaletteRenameInputDeleteBackwardRequested = Notification.Name("phatmux.commandPaletteRenameInputDeleteBackwardRequested")
    static let feedbackComposerRequested = Notification.Name("phatmux.feedbackComposerRequested")
    ...
    static let terminalPortalVisibilityDidChange = Notification.Name("phatmux.terminalPortalVisibilityDidChange")
    static let browserPortalRegistryDidChange = Notification.Name("phatmux.browserPortalRegistryDidChange")
}
