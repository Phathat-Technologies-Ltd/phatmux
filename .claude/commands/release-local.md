...
If the script fails, run `say "phatmux release failed"`.

### 7. Verify homebrew cask

- Run `bash tests/test_homebrew_sha.sh` to confirm the cask SHA matches the release DMG
- Update the homebrew-phatmux submodule pointer: `git add homebrew-phatmux && git commit -m "Update homebrew-phatmux submodule to latest" && git push origin main`
