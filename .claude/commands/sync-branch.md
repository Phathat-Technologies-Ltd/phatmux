...
1. **Update submodules to latest**
   - For each submodule (ghostty, homebrew-phatmux, vendor/bonsplit):
     - `cd <submodule>`
     - `git fetch origin`
...
2. **Commit submodule updates on main**
   ...
   - If changed, stage and commit: `git add ghostty homebrew-phatmux vendor/bonsplit && git commit -m "Update submodules: <brief description>"`
