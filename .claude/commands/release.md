...
Prepare a new release for phatmux. This command updates the changelog, bumps the version, creates a PR, monitors CI, and then merges and tags.
...
12. **Verify homebrew cask update**
    ...
    - Verify: `cd homebrew-phatmux && git pull && grep version Casks/phatmux.rb`
    - Run `bash tests/test_homebrew_sha.sh` to confirm the SHA matches

13. **Notify**
    - On success: `say "phatmux release complete"`
    - On failure: `say "phatmux release failed"`
