...
final class MultiWindowNotificationsUITests: XCTestCase {
    ...
    override func setUp() {
        ...
        dataPath = "/tmp/phatmux-ui-test-multi-window-notifs-\(UUID().uuidString).json"
        socketPath = "/tmp/phatmux-ui-test-socket-\(UUID().uuidString).sock"
    ...
    func testNotificationsRouteToCorrectWindow() {
        ...
        app.launchEnvironment["PHATMUX_UI_TEST_MULTI_WINDOW_NOTIF_SETUP"] = "1"
        app.launchEnvironment["PHATMUX_UI_TEST_MULTI_WINDOW_NOTIF_PATH"] = dataPath
        app.launchEnvironment["PHATMUX_TAG"] = launchTag
        app.launch()
        XCTAssertTrue(
    ...
    func testEmptyNotificationsPopoverBlocksTerminalTyping() throws {
        ...
        app.launchEnvironment["PHATMUX_SOCKET_PATH"] = socketPath
        app.launchEnvironment["PHATMUX_SOCKET_MODE"] = "allowAll"
        app.launchEnvironment["PHATMUX_SOCKET_ENABLE"] = "1"
        app.launchEnvironment["PHATMUX_UI_TEST_SOCKET_SANITY"] = "1"
        app.launchEnvironment["PHATMUX_TAG"] = launchTag
        app.launch()
        XCTAssertTrue(
            ensureForegroundAfterLaunch(app, timeout: 12.0),
        ...
        let marker = "phatmux_notif_block_\(UUID().uuidString.replacingOccurrences(of: "-", with: "").prefix(8))"
    ...
    func testNotifyCLIDoesNotStealFocusAcrossWindows() throws {
        ...
        app.launchEnvironment["PHATMUX_SOCKET_ENABLE"] = "1"
        app.launchEnvironment["PHATMUX_UI_TEST_SOCKET_SANITY"] = "1"
        app.launchEnvironment["PHATMUX_UI_TEST_NOTIFY_SOURCE_TERMINAL_READY"] = "1"
        app.launchEnvironment["PHATMUX_UI_TEST_ENABLE_DUPLICATE_LAUNCH_OBSERVER"] = "1"
        app.launchEnvironment["PHATMUX_TAG"] = launchTag
        app.launch()
        XCTAssertTrue(
            ensureForegroundAfterLaunch(app, timeout: 12.0),
        ...
        let commandStatusPath = FileManager.default.temporaryDirectory
            .appendingPathComponent("phatmux-ui-test-notify-\(commandResultStem).status")
            .path
        let commandStdoutPath = FileManager.default.temporaryDirectory
            .appendingPathComponent("phatmux-ui-test-notify-\(commandResultStem).stdout")
            .path
        let commandStderrPath = FileManager.default.temporaryDirectory
            .appendingPathComponent("phatmux-ui-test-notify-\(commandResultStem).stderr")
            .path
        let commandScriptPath = FileManager.default.temporaryDirectory
            .appendingPathComponent("phatmux-ui-test-notify-\(commandResultStem).sh")
            .path
        defer {
        ...
        guard let bundledCLIPath = resolveCmuxCLIPaths(strategy: .bundledOnly).first else {
            XCTFail("Failed to locate bundled phatmux CLI for notify regression test")
            return
        }
        ...
        do {
            ...
            XCTFail(
                "Failed to write delayed bundled `phatmux notify` script. " +
        ...
        XCTAssertTrue(
            ...
            "Expected phatmux to move to background before delayed notify command runs. state=\(app.state.rawValue)"
        )
        ...
        XCTAssertTrue(
            ...
            "Expected delayed bundled `phatmux notify` command to finish without foregrounding phatmux. state=\(app.state.rawValue)"
        )
        ...
        XCTAssertFalse(
            ...
            "Expected phatmux to remain in background after bundled `phatmux notify`. state=\(app.state.rawValue) stderr=\(notifyStderr)"
        )
        guard notifyExitStatus == "0" else {
            XCTFail(
                "Expected bundled `phatmux notify` launched from the in-app shell to succeed. " +
    ...
    private func runCmuxCommand(
        ...
        environment["PHATMUXTERM_CLI_RESPONSE_TIMEOUT_SEC"] = String(responseTimeoutSeconds)
        ...
        if cliPaths.isEmpty, cliStrategy == .bundledOnly {
            return (
                terminationStatus: -1,
                stdout: "",
                stderr: "Failed to locate bundled phatmux CLI"
            )
        }
        ...
        if cliStrategy == .bundledOnly {
            return lastPermissionFailure ?? (
                terminationStatus: -1,
                stdout: "",
                stderr: "Bundled phatmux CLI command failed without an executable path"
            )
        }

        let fallbackArgs = ["phatmux"] + args
    ...
    private func resolveCmuxCLIPaths(strategy: CmuxCLIStrategy) -> [String] {
        ...
        if strategy == .any {
            for key in ["PHATMUX_UI_TEST_CLI_PATH", "PHATMUXTERM_CLI"] {
        ...
        candidates.append("/tmp/phatmux-\(launchTag)/Build/Products/Debug/phatmux DEV.app/Contents/Resources/bin/phatmux")
        candidates.append("/tmp/phatmux-\(launchTag)/Build/Products/Debug/phatmux.app/Contents/Resources/bin/phatmux")
        if strategy == .any {
            candidates.append("/tmp/phatmux-\(launchTag)/Build/Products/Debug/phatmux")
        }
    ...
    private func appendCLIPathCandidates(
        ...
        candidates.append("\(productsDir)/phatmux DEV.app/Contents/Resources/bin/phatmux")
        candidates.append("\(productsDir)/phatmux.app/Contents/Resources/bin/phatmux")
        if strategy == .any {
            candidates.append("\(productsDir)/phatmux")
        }
        ...
        for entry in entries.sorted() where entry.hasSuffix(".app") {
            let cliPath = URL(fileURLWithPath: productsDir)
                ...
                .appendingPathComponent("Contents/Resources/bin/phatmux")
                .path
            candidates.append(cliPath)
        }
        if strategy == .any {
            for entry in entries.sorted() where entry == "phatmux" {
                let cliPath = URL(fileURLWithPath: productsDir)
                    .appendingPathComponent(entry)
                    .path
                candidates.append(cliPath)
    ...
    private func executeCmuxCommand(
        ...
        do {
            ...
            return (
                terminationStatus: -1,
                stdout: "",
                stderr: "Failed to run phatmux command: \(error.localizedDescription) (cliPath=\(executablePath))"
            )
        }
    ...
    private func resolveSocketPath(timeout: TimeInterval, requiredWorkspaceId: String? = nil) -> String? {
        ...
        _ = waitForCondition(timeout: timeout) {
            for candidate in primaryCandidates {
                ...
                // Primary candidate is the explicitly requested PHATMUX_SOCKET_PATH. If it responds,
    ...
    private func expectedSocketCandidates(includeGlobalFallback: Bool) -> [String] {
        ...
        let taggedDebugSocket = "/tmp/phatmux-debug-\(launchTag).sock"
        ...
        if includeGlobalFallback {
            ...
            candidates.append("/tmp/phatmux-debug.sock")
            candidates.append(stableSocketPath())
            candidates.append("/tmp/phatmux.sock")
        }
    ...
    private func stableSocketPath() -> String {
        FileManager.default.urls(for: .applicationSupportDirectory, in: .userDomainMask).first?
            ...
            .appendingPathComponent("phatmux.sock", isDirectory: false)
            .path ?? "/tmp/phatmux.sock"
    }
    ...
    private func discoverTmpSocketCandidates(limit: Int) -> [String] {
        ...
        let matches = entries.filter { $0.hasPrefix("phatmux") && $0.hasSuffix(".sock") }
