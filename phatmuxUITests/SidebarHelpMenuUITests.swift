...
final class SidebarHelpMenuUITests: XCTestCase {
    ...
    func testHelpMenuOpensKeyboardShortcutsSection() {
        ...
        app.launchEnvironment["PHATMUX_UI_TEST_MODE"] = "1"
    ...
    func testHelpMenuCheckForUpdatesTriggersSidebarUpdatePill() {
        let app = XCUIApplication()
        app.launchEnvironment["PHATMUX_UI_TEST_MODE"] = "1"
        app.launchEnvironment["PHATMUX_UI_TEST_FEED_URL"] = "https://phatmux.test/appcast.xml"
        app.launchEnvironment["PHATMUX_UI_TEST_FEED_MODE"] = "available"
        app.launchEnvironment["PHATMUX_UI_TEST_UPDATE_VERSION"] = "9.9.9"
        app.launchEnvironment["PHATMUX_UI_TEST_AUTO_ALLOW_PERMISSION"] = "1"
        launchAndActivate(app)
...
final class FeedbackComposerShortcutUITests: XCTestCase {
    ...
    func testCmdOptionFWorksFromSettingsWindow() {
        let app = XCUIApplication()
        app.launchEnvironment["PHATMUX_UI_TEST_MODE"] = "1"
        app.launchEnvironment["PHATMUX_UI_TEST_SHOW_SETTINGS"] = "1"
        app.launch()
        app.activate()
...
final class CommandPaletteAllSurfacesUITests: XCTestCase {
    ...
    private let hiddenSurfaceToken = "phatmux-command-palette-hidden-surface"
    private let visibleSurfaceToken = "phatmux-command-palette-visible-surface"
    private let noMatchWorkspaceQuery = "phatmux-command-palette-no-match"

    override func setUp() {
        super.setUp()
        continueAfterFailure = false
        socketPath = "/tmp/phatmux-ui-test-command-palette-\(UUID().uuidString).sock"
    ...
    func testMinimalModeToggleKeepsSettingsWindowFocused() throws {
        let app = XCUIApplication()
        let diagnosticsPath = "/tmp/phatmux-ui-test-settings-focus-\(UUID().uuidString).json"
        ...
        let diagnostics = waitForDiagnostics(
            ...
            data["keyWindowIdentifier"] == "phatmux.settings" && data["settingsWindowIsKey"] == "1"
        }
