...
final class DisplayResolutionRegressionUITests: XCTestCase {
    private let defaultDisplayHarnessManifestPath = "/tmp/phatmux-ui-test-display-harness.json"
    ...
    override func setUp() {
        ...
        let tempPrefix = FileManager.default.temporaryDirectory
            .appendingPathComponent("phatmux-ui-test-display-\(token)")
            .path
        launchTag = "ui-tests-display-resolution-\(token.prefix(8))"
        diagnosticsPath = "/tmp/phatmux-ui-test-display-churn-\(token).json"
    ...
    private func loadPrebuiltHelperBinaryPath(_ env: [String: String]) -> String? {
        guard let helperBinaryPath = env["PHATMUX_UI_TEST_DISPLAY_HELPER_BINARY_PATH"],
    ...
    private func loadExternalHarnessFromEnvironment(_ env: [String: String]) -> ExternalDisplayHarness? {
        guard let readyPath = env["PHATMUX_UI_TEST_DISPLAY_READY_PATH"], !readyPath.isEmpty,
              let displayIDPath = env["PHATMUX_UI_TEST_DISPLAY_ID_PATH"], !displayIDPath.isEmpty,
              let startPath = env["PHATMUX_UI_TEST_DISPLAY_START_PATH"], !startPath.isEmpty,
              let donePath = env["PHATMUX_UI_TEST_DISPLAY_DONE_PATH"], !donePath.isEmpty else {
            return nil
        }
        ...
        return ExternalDisplayHarness(
            ...
            startPath: startPath,
            donePath: donePath,
            logPath: env["PHATMUX_UI_TEST_DISPLAY_LOG_PATH"],
            helperBinaryPath: nil
        )
    }

    private func loadExternalHarnessFromManifest(_ env: [String: String]) -> ExternalDisplayHarness? {
        let manifestPath = env["PHATMUX_UI_TEST_DISPLAY_HARNESS_MANIFEST_PATH"] ?? defaultDisplayHarnessManifestPath
    ...
    private func launchEnvironment(targetDisplayID: String) -> [String: String] {
        [
            "PHATMUX_UI_TEST_MODE": "1",
            "PHATMUX_UI_TEST_DIAGNOSTICS_PATH": diagnosticsPath,
            "PHATMUX_UI_TEST_DISPLAY_RENDER_STATS": "1",
            "PHATMUX_UI_TEST_TARGET_DISPLAY_ID": targetDisplayID,
            "PHATMUX_TAG": launchTag,
        ]
    }
