...
```bash
phatmux identify --workspace workspace:2
phatmux identify --workspace workspace:2 --surface surface:8
```
...
```bash
phatmux --json identify                 # refs-first output
phatmux --json --id-format both identify
phatmux --json --id-format uuids identify
```
