// CLI entry. Upstream cmux: `CLI/cmux.swift`.
import Foundation
import CryptoKit
import Darwin
...
private final class CLISocketSentryTelemetry {
    ...
    private static func currentSentryBundleIdentifier() -> String? {
        if let bundleIdentifier = ProcessInfo.processInfo.environment["PHATMUX_BUNDLE_ID"]?
    ...
    init(command: String, commandArgs: [String], socketPath: String, processEnv: [String: String]) {
        ...
        self.envSocketPath = processEnv["PHATMUX_SOCKET_PATH"] ?? processEnv["PHATMUX_SOCKET"]
        self.workspaceId = processEnv["PHATMUX_WORKSPACE_ID"]
        self.surfaceId = processEnv["PHATMUX_SURFACE_ID"]
        self.disabledByEnv =
            processEnv["PHATMUX_CLI_SENTRY_DISABLED"] == "1" ||
            processEnv["PHATMUX_CLAUDE_HOOK_SENTRY_DISABLED"] == "1"
    }
    ...
    func breadcrumb(_ message: String, data: [String: Any] = [:]) {
        ...
        let crumb = Breadcrumb(level: .info, category: "phatmux.cli")
    ...
    func captureError(stage: String, error: Error) {
        ...
        _ = SentrySDK.capture(error: error) { scope in
            ...
            scope.setTag(value: "phatmux-cli", key: "component")
    ...
    private func socketDiagnostics() -> [String: Any] {
        ...
        if !tmpSockets.isEmpty {
            context["tmp_phatmux_sockets"] = tmpSockets
        }
        ...
        if CLISocketPathResolver.isImplicitDefaultPath(socketPath),
           ...
           !taggedSockets.isEmpty {
            context["possible_root_cause"] = "PHATMUX_SOCKET_PATH/PHATMUX_SOCKET missing while tagged sockets exist"
        }
    ...
    private static func discoverSockets(in directory: String, limit: Int) -> [String] {
        ...
        for name in entries.sorted() {
            guard name.hasPrefix("phatmux"), name.hasSuffix(".sock") else { continue }
...
private final class ClaudeHookSessionStore {
    private static let defaultStatePath = "~/.phatmux/claude-hook-sessions.json"
    ...
    init(
        ...
        if let overridePath = processEnv["PHATMUX_CLAUDE_HOOK_STATE_PATH"]?.trimmingCharacters(in: .whitespacesAndNewlines),
...
enum SocketPasswordResolver {
    private static let service = "com.phatmux.app.socket-control"
    private static let account = "local-socket-password"
    private static let directoryName = "phatmux"
    ...
    static func resolve(explicit: String?, socketPath: String) -> String? {
        ...
        if let env = normalized(ProcessInfo.processInfo.environment["PHATMUX_SOCKET_PASSWORD"]) {
            return env
        }
    ...
    private static func keychainScope(
        ...
        if let tag = normalized(environment["PHATMUX_TAG"]) {
        ...
        let prefixes = ["phatmux-debug-", "phatmux-"]
...
private enum CLISocketPathResolver {
    private static let appSupportDirectoryName = "phatmux"
    private static let stableSocketFileName = "phatmux.sock"
    private static let lastSocketPathFileName = "last-socket-path"
    static let legacyDefaultSocketPath = "/tmp/phatmux.sock"
    private static let fallbackSocketPath = "/tmp/phatmux-debug.sock"
    private static let stagingSocketPath = "/tmp/phatmux-staging.sock"
    private static let legacyLastSocketPathFile = "/tmp/phatmux-last-socket-path"
    ...
    private static func candidatePaths(requestedPath: String, environment: [String: String]) -> [String] {
        ...
        if let tag = normalized(environment["PHATMUX_TAG"]) {
            ...
            candidates.append("/tmp/phatmux-debug-\(slug).sock")
            candidates.append("/tmp/phatmux-\(slug).sock")
        }
    ...
    private static func discoverTaggedSockets(limit: Int) -> [String] {
        ...
        for directory in socketDiscoveryDirectories() {
            ...
            for name in entries where name.hasPrefix("phatmux") && name.hasSuffix(".sock") {
...
final class SocketClient {
    ...
    private static let responseTimeoutSeconds: TimeInterval = {
        ...
        if let raw = env["PHATMUXTERM_CLI_RESPONSE_TIMEOUT_SEC"],
    ...
    static func waitForConnectableSocket(path: String, timeout: TimeInterval) throws -> SocketClient {
        ...
        guard let watchDirectory = existingWatchDirectory(forPath: path) else {
            throw CLIError(message: "phatmux app did not start in time (socket not found at \(path))")
        }
        ...
        let queue = DispatchQueue(label: "com.phatmux.cli.socket-watch.\(UUID().uuidString)")
    ...
    static func waitForFilesystemPath(_ path: String, timeout: TimeInterval) throws {
        ...
        let queue = DispatchQueue(label: "com.phatmux.cli.path-watch.\(UUID().uuidString)")
        let semaphore = DispatchSemaphore(value: 0)
        var found = false
        let source = DispatchSource.makeFileSystemObjectSource(
            fileDescriptor: watchFD,
...
struct PHATMUXCLI {
    ...
    private static let debugLastSocketHintPath = "/tmp/phatmux-last-socket-path"
    ...
    private static func debugSocketPathFromHintFile() -> String? {
        ...
        guard let hinted = normalizedEnvValue(raw),
              hinted.hasPrefix("/tmp/phatmux-debug"),
    ...
    private static func defaultSocketPath(environment: [String: String]) -> String {
        if let explicit = normalizedEnvValue(environment["PHATMUX_SOCKET_PATH"]) {
            return explicit
        }
#if DEBUG
        ...
        return "/tmp/phatmux-debug.sock"
#else
        return "/tmp/phatmux.sock"
#endif
    }
    ...
    func run() throws {
        ...
        let envSocketPath: String? = {
            for key in ["PHATMUX_SOCKET_PATH", "PHATMUX_SOCKET"] {
        ...
        // so help text is available even when phatmux is not running.
        if command != "__tmux-compat",
           ...
           (commandArgs.contains("--help") || commandArgs.contains("-h")) {
            ...
            print("Unknown command '\(command)'. Run 'phatmux help' to see available commands.")
            return
        }
        ...
        case "identify":
            ...
            if includeCaller {
                ...
                let workspaceArg = idWsFlag ?? (windowId == nil ? ProcessInfo.processInfo.environment["PHATMUX_WORKSPACE_ID"] : nil)
                let surfaceArg = optionValue(commandArgs, name: "--surface") ?? (idWsFlag == nil && windowId == nil ? ProcessInfo.processInfo.environment["PHATMUX_SURFACE_ID"] : nil)
        ...
        case "new-split":
            ...
            let workspaceArg = wsArg ?? (windowId == nil ? ProcessInfo.processInfo.environment["PHATMUX_WORKSPACE_ID"] : nil)
            let surfaceRaw = sfArg ?? panelArg ?? (wsArg == nil && windowId == nil ? ProcessInfo.processInfo.environment["PHATMUX_SURFACE_ID"] : nil)
        ...
        case "close-surface":
            ...
            let workspaceArg = csWsFlag ?? (windowId == nil ? ProcessInfo.processInfo.environment["PHATMUX_WORKSPACE_ID"] : nil)
            let surfaceRaw = optionValue(commandArgs, name: "--surface") ?? optionValue(commandArgs, name: "--panel") ?? (csWsFlag == nil && windowId == nil ? ProcessInfo.processInfo.environment["PHATMUX_SURFACE_ID"] : nil)
            var params: [String: Any] = [:]
            let wsId = try normalizeWorkspaceHandle(workspaceArg, client: client)
            if let wsId { params["workspace_id"] = wsId }
            let sfId = try normalizeSurfaceHandle(surfaceRaw, client: client, workspaceHandle: wsId)
        ...
        case "trigger-flash":
            ...
            let workspaceArg = tfWsFlag ?? (windowId == nil ? ProcessInfo.processInfo.environment["PHATMUX_WORKSPACE_ID"] : nil)
            let surfaceArg = optionValue(commandArgs, name: "--surface") ?? optionValue(commandArgs, name: "--panel") ?? (tfWsFlag == nil && windowId == nil ? ProcessInfo.processInfo.environment["PHATMUX_SURFACE_ID"] : nil)
            var params: [String: Any] = [:]
            let wsId = try normalizeWorkspaceHandle(workspaceArg, client: client)
            if let wsId { params["workspace_id"] = wsId }
        ...
        case "read-screen":
            ...
            let workspaceArg = wsArg ?? (windowId == nil ? ProcessInfo.processInfo.environment["PHATMUX_WORKSPACE_ID"] : nil)
            let surfaceArg = sfArg ?? (wsArg == nil && windowId == nil ? ProcessInfo.processInfo.environment["PHATMUX_SURFACE_ID"] : nil)
        ...
        case "notify":
            ...
            let workspaceArg = notifyWsFlag ?? (windowId == nil ? ProcessInfo.processInfo.environment["PHATMUX_WORKSPACE_ID"] : nil)
            let surfaceArg = optionValue(commandArgs, name: "--surface") ?? (notifyWsFlag == nil && windowId == nil ? ProcessInfo.processInfo.environment["PHATMUX_SURFACE_ID"] : nil)
        ...
        case "clear-notifications":
            ...
            if let wsFlag = optionValue(commandArgs, name: "--workspace") {
                ...
                socketCmd += " --tab=\(wsId)"
                      ...
                      let envWs = ProcessInfo.processInfo.environment["PHATMUX_WORKSPACE_ID"],
    ...
    private func runMarkdownCommand(
        ...
        if let first = args.first, first.lowercased() == "open" {
            ...
            if let first = args.first, first.hasPrefix("-") {
                throw CLIError(
                    message:
                        "markdown open: unknown flag '\(first)'. Usage: phatmux markdown open <path> [--workspace <id|ref|index>] [--surface <id|ref|index>] [--window <id|ref|index>] [--direction right|down|left|up]"
                )
            } else if let first = args.first, looksLikePath(first) || first.contains(".") {
                subArgs = args
            } else if let first = args.first {
                throw CLIError(message: "Unknown markdown subcommand: \(first). Usage: phatmux markdown open <path>")
            } else {
                subArgs = []
            }
        }
        ...
        guard let rawPath = subArgs.first, !rawPath.isEmpty else {
            throw CLIError(message: "markdown open requires a file path. Usage: phatmux markdown open <path>")
        }
        let trailingArgs = Array(subArgs.dropFirst())
        if let unknownFlag = trailingArgs.first(where: { $0.hasPrefix("-") }) {
            throw CLIError(
                message:
                    "markdown open: unknown flag '\(unknownFlag)'. Usage: phatmux markdown open <path> [--workspace <id|ref|index>] [--surface <id|ref|index>] [--window <id|ref|index>] [--direction right|down|left|up]"
            )
        }
        if let extraArg = trailingArgs.first {
            throw CLIError(
                message:
                    "markdown open: unexpected argument '\(extraArg)'. Usage: phatmux markdown open <path> [--workspace <id|ref|index>] [--surface <id|ref|index>] [--window <id|ref|index>] [--direction right|down|left|up]"
            )
        }
        ...
        let workspaceRaw = workspaceOpt ?? (windowOpt == nil ? ProcessInfo.processInfo.environment["PHATMUX_WORKSPACE_ID"] : nil)
        if let workspaceRaw {
    ...
    /// Open a path in phatmux by creating a new workspace with the given directory.
    ...
    private func runFeedback(
        ...
        if emailOpt == nil && bodyOpt == nil && imagePaths.isEmpty {
            var params: [String: Any] = [:]
            let env = ProcessInfo.processInfo.environment
            if let workspaceId = env["PHATMUX_WORKSPACE_ID"]?.trimmingCharacters(in: .whitespacesAndNewlines),
               !workspaceId.isEmpty {
    ...
    private func runWorkspaceAction(
        ...
        let workspaceArg = workspaceOpt ?? (windowOverride == nil ? ProcessInfo.processInfo.environment["PHATMUX_WORKSPACE_ID"] : nil)
    ...
    private func runTabAction(
        ...
        let tabArg = tabOpt
            ...
            ?? (workspaceOpt == nil && windowOverride == nil
                ? (ProcessInfo.processInfo.environment["PHATMUX_TAB_ID"] ?? ProcessInfo.processInfo.environment["PHATMUX_SURFACE_ID"])
                : nil)
    ...
    private func parseSSHCommandOptions(_ commandArgs: [String], localSocketPath: String = "", remoteRelayPort: Int = 0) throws -> SSHCommandOptions {
        ...
        guard let destination else {
            throw CLIError(message: "ssh requires a destination (example: phatmux ssh user@host)")
        }
    ...
    private func buildSSHBootstrapCommandSnippet(
        ...
        let remoteCommandBase64Placeholder = "__PHATMUX_REMOTE_BOOTSTRAP_B64_RUNTIME__"
        ...
        var lines: [String] = [
            "phatmux_workspace_id=\"${PHATMUX_WORKSPACE_ID:-}\"",
            "phatmux_surface_id=\"${PHATMUX_SURFACE_ID:-}\"",
            "phatmux_remote_bootstrap_b64=\(shellQuote(encodedBootstrapScript))",
            "phatmux_remote_bootstrap=\"$(printf %s \"$phatmux_remote_bootstrap_b64\" | base64 -d 2>/dev/null || printf %s \"$phatmux_remote_bootstrap_b64\" | base64 -D 2>/dev/null)\"",
            "phatmux_remote_bootstrap=\"$(printf '%s' \"$phatmux_remote_bootstrap\" | sed \"s/__PHATMUX_WORKSPACE_ID__/$phatmux_workspace_id/g; s/__PHATMUX_SURFACE_ID__/$phatmux_surface_id/g\")\"",
            "phatmux_remote_bootstrap_b64_runtime=\"$(printf '%s' \"$phatmux_remote_bootstrap\" | base64 | tr -d '\\n')\"",
            "phatmux_remote_command_template=\(shellQuote(remoteCommandTemplate))",
            "phatmux_remote_command=\"$(printf '%s' \"$phatmux_remote_command_template\" | sed \"s|\(remoteCommandBase64Placeholder)|$phatmux_remote_bootstrap_b64_runtime|g\")\"",
        ]

        var sshInvocation = "command \(sshPrefix) -o \"RemoteCommand=$phatmux_remote_command\""
        if !hasSSHOptionKey(options.sshOptions, key: "RequestTTY") {
            sshInvocation += " -tt"
        }
    ...
    private func runtimeEncodedRemoteBootstrapCommandShell(base64Placeholder: String) -> String {
        return [
            "phatmux_tmp=$(mktemp \"${TMPDIR:-/tmp}/phatmux-ssh-bootstrap.XXXXXX\") || exit 1",
            "(printf %s '\(base64Placeholder)' | base64 -d 2>/dev/null || printf %s '\(base64Placeholder)' | base64 -D 2>/dev/null) > \"$phatmux_tmp\" || { rm -f \"$phatmux_tmp\"; exit 1; }",
            "chmod 700 \"$phatmux_tmp\" >/dev/null 2>&1 || true",
            "/bin/sh \"$phatmux_tmp\"",
            "phatmux_status=$?",
            "rm -f \"$phatmux_tmp\"",
            "exit $phatmux_status",
    ...
    func buildInteractiveRemoteShellScript(
        ...
        let remoteCallerExportLines = [
            "if [ -n '__PHATMUX_WORKSPACE_ID__' ]; then export PHATMUX_WORKSPACE_ID='__PHATMUX_WORKSPACE_ID__'; fi",
            "if [ -n '__PHATMUX_SURFACE_ID__' ]; then export PHATMUX_SURFACE_ID='__PHATMUX_SURFACE_ID__'; fi",
        ]
        let relaySocket = remoteRelayPort > 0 ? "127.0.0.1:\(remoteRelayPort)" : nil
        let shellStateDir = "$HOME/.phatmux/relay/\(max(remoteRelayPort, 0)).shell"
        var commonShellLines = remoteTerminalLines
        commonShellLines.append(contentsOf: remoteEnvExportLines)
        commonShellLines.append("export PATH=\"$HOME/.phatmux/bin:$PATH\"")
        if let relaySocket {
            commonShellLines.append("export PHATMUX_SOCKET_PATH=\(relaySocket)")
        }
        ...
        var outerLines: [String] = [
            "PHATMUX_LOGIN_SHELL=\"${SHELL:-/bin/zsh}\"",
            "case \"${PHATMUX_LOGIN_SHELL##*/}\" in",
            "  zsh)",
            "    mkdir -p \"$HOME/.phatmux/relay\"",
            "    phatmux_shell_dir=\"\(shellStateDir)\"",
            "    mkdir -p \"$phatmux_shell_dir\"",
            "    cat > \"$phatmux_shell_dir/.zshenv\" <<'PHATMUXZSHENV'",
        ]
        outerLines.append(contentsOf: zshEnvLines)
        outerLines += [
            "PHATMUXZSHENV",
            "    cat > \"$phatmux_shell_dir/.zprofile\" <<'PHATMUXZSHPROFILE'",
        ]
        outerLines.append(contentsOf: zshProfileLines)
        outerLines += [
            "PHATMUXZSHPROFILE",
            "    cat > \"$phatmux_shell_dir/.zshrc\" <<'PHATMUXZSHRC'",
        ]
        outerLines.append(contentsOf: zshRCLines)
        outerLines += [
            "PHATMUXZSHRC",
            "    cat > \"$phatmux_shell_dir/.zlogin\" <<'PHATMUXZSHLOGIN'",
        ]
        outerLines.append(contentsOf: zshLoginLines)
        outerLines += [
            "PHATMUXZSHLOGIN",
            "    chmod 600 \"$phatmux_shell_dir/.zshenv\" \"$phatmux_shell_dir/.zprofile\" \"$phatmux_shell_dir/.zshrc\" \"$phatmux_shell_dir/.zlogin\" >/dev/null 2>&1 || true",
        ]
        outerLines.append(contentsOf: relayWarmupLines.map { "    " + $0 })
        outerLines += [
            "    export PHATMUX_REAL_ZDOTDIR=\"${ZDOTDIR:-$HOME}\"",
            "    export ZDOTDIR=\"$phatmux_shell_dir\"",
            "    exec \"$PHATMUX_LOGIN_SHELL\" -il",
            "    ;;",
            "  bash)",
            "    mkdir -p \"$HOME/.phatmux/relay\"",
            "    phatmux_shell_dir=\"\(shellStateDir)\"",
            "    mkdir -p \"$phatmux_shell_dir\"",
            "    cat > \"$phatmux_shell_dir/.bashrc\" <<'PHATMUXBASHRC'",
        ]
        outerLines.append(contentsOf: bashRCLines)
        outerLines += [
            "PHATMUXBASHRC",
            "    chmod 600 \"$phatmux_shell_dir/.bashrc\" >/dev/null 2>&1 || true",
        ]
        outerLines.append(contentsOf: relayWarmupLines.map { "    " + $0 })
        outerLines += [
            "    exec \"$PHATMUX_LOGIN_SHELL\" --rcfile \"$phatmux_shell_dir/.bashrc\" -i",
            "    ;;",
            "  *)",
        ]
    ...
    private func interactiveRemoteTerminalSetupLines(terminfoSource: String?) -> [String] {
        var lines: [String] = [
            "phatmux_term='xterm-256color'",
            "if command -v infocmp >/dev/null 2>&1 && infocmp xterm-ghostty >/dev/null 2>&1; then",
            "  phatmux_term='xterm-ghostty'",
            "fi",
            "export TERM=\"$phatmux_term\"",
        ]
        ...
        lines += [
            "if [ \"$phatmux_term\" != 'xterm-ghostty' ]; then",
            "  (",
            "    command -v tic >/dev/null 2>&1 || exit 0",
            "    mkdir -p \"$HOME/.terminfo\" 2>/dev/null || exit 0",
            "    cat <<'PHATMUXTERMINFO' | tic -x - >/dev/null 2>&1",
    ...
    func encodedRemoteBootstrapCommand(_ remoteBootstrapScript: String) -> String {
        ...
        return [
            "phatmux_tmp=$(mktemp \"${TMPDIR:-/tmp}/phatmux-ssh-bootstrap.XXXXXX\") || exit 1",
            "(printf %s \(encodedLiteral) | base64 -d 2>/dev/null || printf %s \(encodedLiteral) | base64 -D 2>/dev/null) > \"$phatmux_tmp\" || { rm -f \"$phatmux_tmp\"; exit 1; }",
            "chmod 700 \"$phatmux_tmp\" >/dev/null 2>&1 || true",
            "/bin/sh \"$phatmux_tmp\"",
            "phatmux_status=$?",
            "rm -f \"$phatmux_tmp\"",
    ...
    func buildSSHStartupCommand(
        ...
        scriptLines += [
            "PHATMUX_SSH_SESSION_ENDED=0",
            "phatmux_ssh_session_end() { if [ \"${PHATMUX_SSH_SESSION_ENDED:-0}\" = 1 ]; then return; fi; PHATMUX_SSH_SESSION_ENDED=1; \(lifecycleCleanup); }",
            "trap 'phatmux_ssh_session_end' EXIT HUP INT TERM",
        ]
        if isShellSnippet {
        ...
        scriptLines += [
            ...
            "phatmux_ssh_session_end",
            "exit $phatmux_ssh_status",
        ]
    ...
    private func writeSSHStartupScript(_ scriptBody: String, remoteRelayPort: Int) throws -> String {
        ...
        let scriptURL = tempDir.appendingPathComponent(
            "phatmux-ssh-startup-\(remoteRelayPort)-\(UUID().uuidString.lowercased()).sh"
        )
        let script = "#!/bin/sh\n\(scriptBody)\n"
    ...
    private func buildSSHSessionEndShellCommand(remoteRelayPort: Int) -> String {
        [
            "if [ -n \"${PHATMUX_BUNDLED_CLI_PATH:-}\" ]",
            "&& [ -x \"${PHATMUX_BUNDLED_CLI_PATH}\" ]",
            "&& [ -n \"${PHATMUX_SOCKET_PATH:-}\" ]",
            "&& [ -n \"${PHATMUX_WORKSPACE_ID:-}\" ]",
            "&& [ -n \"${PHATMUX_SURFACE_ID:-}\" ]; then",
            "\"${PHATMUX_BUNDLED_CLI_PATH}\" --socket \"${PHATMUX_SOCKET_PATH}\" ssh-session-end --relay-port \(remoteRelayPort) --workspace \"${PHATMUX_WORKSPACE_ID}\" --surface \"${PHATMUX_SURFACE_ID}\" >/dev/null 2>&1 || true;",
            "elif command -v phatmux >/dev/null 2>&1",
            "&& [ -n \"${PHATMUX_WORKSPACE_ID:-}\" ]",
            "&& [ -n \"${PHATMUX_SURFACE_ID:-}\" ]; then",
            "phatmux ssh-session-end --relay-port \(remoteRelayPort) --workspace \"${PHATMUX_WORKSPACE_ID}\" --surface \"${PHATMUX_SURFACE_ID}\" >/dev/null 2>&1 || true;",
            "fi",
        ].joined(separator: " ")
    }
    ...
    private func runSSHSessionEnd(commandArgs: [String], client: SocketClient) throws {
        ...
        let workspaceRaw = optionValue(commandArgs, name: "--workspace") ?? ProcessInfo.processInfo.environment["PHATMUX_WORKSPACE_ID"]
        let surfaceRaw = optionValue(commandArgs, name: "--surface") ?? ProcessInfo.processInfo.environment["PHATMUX_SURFACE_ID"]
        guard let workspaceRaw,
              let workspaceId = try normalizeWorkspaceHandle(workspaceRaw, client: client),
              !workspaceId.isEmpty else {
            throw CLIError(message: "ssh-session-end requires --workspace or PHATMUX_WORKSPACE_ID")
        }
        guard let surfaceRaw,
            ...
            throw CLIError(message: "ssh-session-end requires --surface or PHATMUX_SURFACE_ID")
        }
    ...
    private func runRemoteDaemonStatus(commandArgs: [String], jsonOutput: Bool) throws {
        ...
        let payload: [String: Any] = [
            ...
            "commit": info["PHATMUXCommit"] ?? NSNull(),
            ...
            "dev_local_build_fallback": ProcessInfo.processInfo.environment["PHATMUX_REMOTE_DAEMON_ALLOW_LOCAL_BUILD"] == "1",
        ...
        if manifest == nil {
            print("note: this build has no embedded remote daemon manifest. Set PHATMUX_REMOTE_DAEMON_ALLOW_LOCAL_BUILD=1 only for dev builds.")
        }
    }
    ...
    private func remoteDaemonManifest() -> RemoteDaemonManifest? {
        for plistURL in candidateInfoPlistURLs() {
            guard let raw = NSDictionary(contentsOf: plistURL) as? [String: Any],
                  let rawManifest = raw["PHATMUXRemoteDaemonManifestJSON"] as? String,
    ...
    private func remoteDaemonCacheURL(version: String, goOS: String, goArch: String) -> URL {
        ...
        do {
            ...
            return URL(fileURLWithPath: NSTemporaryDirectory(), isDirectory: true)
                .appendingPathComponent("phatmux-remote-daemons", isDirectory: true)
    ...
    private func defaultSSHControlPathTemplate(remoteRelayPort: Int? = nil) -> String {
        if let remoteRelayPort, remoteRelayPort > 0 {
            return "/tmp/phatmux-ssh-\(getuid())-\(remoteRelayPort)-%C"
        }
        return "/tmp/phatmux-ssh-\(getuid())-%C"
    }
    ...
    private func cliDebugLog(_ message: @autoclosure () -> String) {
#if DEBUG
        let trimmedExplicit = ProcessInfo.processInfo.environment["PHATMUX_DEBUG_LOG"]?
            .trimmingCharacters(in: .whitespacesAndNewlines)
        let path: String? = {
            if let trimmedExplicit, !trimmedExplicit.isEmpty {
                return trimmedExplicit
            }
            guard let marker = try? String(contentsOfFile: "/tmp/phatmux-last-debug-log-path", encoding: .utf8) else {
                return nil
            }
        ...
        let line = "\(timestamp) [phatmux-cli] \(message())\n"
    ...
    private func runBrowserCommand(
        ...
        func displaySnapshotText(_ payload: [String: Any]) -> String {
            ...
            if url.isEmpty || url == "about:blank" {
                lines.append("hint: run 'phatmux browser <surface> get url' to verify navigation")
            }
        ...
        func displayBrowserValue(_ value: Any) -> String {
            if let dict = value as? [String: Any],
               let type = dict["__phatmux_t"] as? String,
               type == "undefined" {
                return "undefined"
            }
            if value is NSNull {
        ...
        if subcommand == "open" || subcommand == "open-split" || subcommand == "new" {
            ...
            let respectExternalOpenRules: Bool = {
                guard let raw = ProcessInfo.processInfo.environment["PHATMUX_RESPECT_EXTERNAL_OPEN_RULES"] else {
                    return false
                }
            ...
            if let outPathOpt {
                ...
                if !hasText(screenshotPath) && !hasText(screenshotURL) {
                    let outputDir = FileManager.default.temporaryDirectory
                        .appendingPathComponent("phatmux-browser-screenshots-cli", isDirectory: true)
    ...
    private func subcommandUsage(_ command: String) -> String? {
        ...
        case "ping":
            ...
            Check connectivity to the phatmux socket server.
            """
        case "capabilities":
            return """
            Usage: phatmux capabilities
        ...
        case "welcome":
            return """
            Usage: phatmux welcome

            Show a welcome screen with the phatmux logo and useful shortcuts.
            Auto-runs once on first launch.
            """
        case "shortcuts":
            return """
            Usage: phatmux shortcuts

            Open the Settings window to Keyboard Shortcuts.
            """
        case "feedback":
            return """
            Usage: phatmux feedback
                   phatmux feedback --email <email> --body <text> [--image <path> ...]
        ...
        case "themes":
            return """
            Usage: phatmux themes
                   phatmux themes list
                   phatmux themes set <theme>
                   phatmux themes set --light <theme> [--dark <theme>]
                   phatmux themes set --dark <theme> [--light <theme>]
                   phatmux themes clear

            When run in a TTY, `phatmux themes` opens an interactive theme picker with
            live app preview. Use `phatmux themes list` for a plain listing.

            The picker previews the selected theme across the running phatmux app and
            ...
            Commands:
              ...
              clear                     Remove the phatmux theme override and fall back to other config

            Examples:
              phatmux themes
              phatmux themes list
              phatmux themes set "Catppuccin Mocha"
              phatmux themes set --light "Catppuccin Latte" --dark "Catppuccin Mocha"
              phatmux themes clear
            """
        case "claude-teams":
            return String(localized: "cli.claude-teams.usage", defaultValue: """
            Usage: phatmux claude-teams [claude-args...]

            Launch Claude Code with agent teams enabled.

            This command:
              - defaults Claude teammate mode to auto
              - sets a tmux-like environment so Claude auto mode uses phatmux splits
            ...
            The tmux shim translates supported tmux window/pane commands into phatmux
            workspace and split operations in the current phatmux session.

            Examples:
              phatmux claude-teams
              phatmux claude-teams --continue
              phatmux claude-teams --model sonnet
            """)
        case "identify":
            return """
            Usage: phatmux identify [--workspace <id|ref|index>] [--surface <id|ref|index>] [--no-caller]

            Print server identity and caller context details.

            Flags:
              --workspace <id|ref|index>   Caller workspace context (default: $PHATMUX_WORKSPACE_ID)
              --surface <id|ref|index>     Caller surface context (default: $PHATMUX_SURFACE_ID)
              --no-caller                  Omit caller context from the request
            """
        case "list-windows":
            return """
            Usage: phatmux list-windows

            List open windows.
            """
        case "current-window":
            return """
            Usage: phatmux current-window

            Print the currently selected window ID.
            """
        case "new-window":
            return """
            Usage: phatmux new-window
        ...
        case "focus-window":
            return """
            Usage: phatmux focus-window --window <id|ref|index>
            ...
            Example:
              phatmux focus-window --window 0
              phatmux focus-window --window window:1
            """
        case "close-window":
            return """
            Usage: phatmux close-window --window <id|ref|index>
            ...
            Example:
              phatmux close-window --window 0
              phatmux close-window --window window:1
            """
        case "move-workspace-to-window":
            return """
            Usage: phatmux move-workspace-to-window --workspace <id|ref|index> --window <id|ref|index>
            ...
            Example:
              phatmux move-workspace-to-window --workspace workspace:2 --window window:1
            """
        case "move-surface":
            return """
            Usage: phatmux move-surface [--surface <id|ref|index> | <id|ref|index>] [flags]
            ...
            Example:
              phatmux move-surface --surface surface:1 --workspace workspace:2
              phatmux move-surface surface:1 --pane pane:2 --index 0
            """
        case "reorder-surface":
            return """
            Usage: phatmux reorder-surface [--surface <id|ref|index> | <id|ref|index>] [flags]
            ...
            Example:
              phatmux reorder-surface --surface surface:1 --index 0
              phatmux reorder-surface --surface surface:3 --after surface:1
            """
        case "reorder-workspace":
            return """
            Usage: phatmux reorder-workspace [--workspace <id|ref|index> | <id|ref|index>] [flags]
            ...
            Example:
              phatmux reorder-workspace --workspace workspace:2 --index 0
              phatmux reorder-workspace --workspace workspace:3 --after workspace:1
            """
        case "workspace-action":
            return """
            Usage: phatmux workspace-action --action <name> [flags]
            ...
            Flags:
              ...
              --workspace <id|ref|index>   Target workspace (default: current/$PHATMUX_WORKSPACE_ID)
              --title <text>               Title for rename (or pass trailing title text)
              --color <#hex|name>          Color for set-color (e.g. '#C0392B' or 'Red')

            Example:
              phatmux workspace-action --workspace workspace:2 --action pin
              phatmux workspace-action --action rename --title "infra"
              phatmux workspace-action close-others
              phatmux workspace-action --action set-color --workspace workspace:1 --color '#C0392B'
              phatmux workspace-action --action clear-color --workspace workspace:1
            """
        case "tab-action":
            return """
            Usage: phatmux tab-action --action <name> [flags]
            ...
            Flags:
              --action <name>              Action name (required if not positional)
              --tab <id|ref|index>         Target tab (accepts tab:<n> or surface:<n>; default: $PHATMUX_TAB_ID, then $PHATMUX_SURFACE_ID, then focused tab)
              --surface <id|ref|index>     Alias for --tab (backward compatibility)
              --workspace <id|ref|index>   Workspace context (default: current/$PHATMUX_WORKSPACE_ID)
              --title <text>               Title for rename (or pass trailing title text)
              --url <url>                  Optional URL for new-browser-right

            Example:
              phatmux tab-action --tab tab:3 --action pin
              phatmux tab-action --action close-right
              phatmux tab-action --tab tab:2 --action rename --title "build logs"
            """
        case "rename-tab":
            return """
            Usage: phatmux rename-tab [--workspace <id|ref>] [--tab <id|ref>] [--surface <id|ref>] [--] <title>
            ...
            1) --tab
            2) --surface
            3) $PHATMUX_TAB_ID / $PHATMUX_SURFACE_ID
            4) currently focused tab (optionally within --workspace)

            Flags:
              --workspace <id|ref>   Workspace context (default: current/$PHATMUX_WORKSPACE_ID)
            ...
            Examples:
              phatmux rename-tab "build logs"
              phatmux rename-tab --tab tab:3 "staging server"
              phatmux rename-tab --workspace workspace:2 --surface surface:5 --title "agent run"
            """
        case "new-workspace":
            return """
            Usage: phatmux new-workspace [--cwd <path>] [--command <text>]
            ...
            Example:
              ...
              phatmux new-workspace --cwd ~/projects/myapp
              phatmux new-workspace --cwd . --command "npm test"
            """
        case "list-workspaces":
            return """
            Usage: phatmux list-workspaces
        ...
        case "ssh":
            return """
            Usage: phatmux ssh <destination> [flags] [-- <remote-command-args>]

            Create a new workspace, mark it as remote-SSH, and start an SSH session in that workspace.
            phatmux will also establish a local SSH proxy endpoint so browser traffic can egress from the remote host.
            ...
            Example:
              phatmux ssh dev@my-host
              phatmux ssh dev@my-host --name "gpu-box" --port 2222 --identity ~/.ssh/id_ed25519
              phatmux ssh dev@my-host --ssh-option UserKnownHostsFile=/dev/null --ssh-option StrictHostKeyChecking=no
            """
        case "remote-daemon-status":
            return """
            Usage: phatmux remote-daemon-status [--os <darwin|linux>] [--arch <arm64|amd64>]

            Show the embedded phatmuxd-remote release manifest, local cache status, checksum verification state,
            and the GitHub attestation verification command for a target platform.

            Example:
              phatmux remote-daemon-status
              phatmux remote-daemon-status --os linux --arch arm64
            """
        case "new-split":
            return """
            Usage: phatmux new-split <left|right|up|down> [flags]

            Split the current pane in the given direction.

            Flags:
              --workspace <id|ref>   Target workspace (default: $PHATMUX_WORKSPACE_ID)
              --surface <id|ref>     Surface to split from (default: $PHATMUX_SURFACE_ID)
              --panel <id|ref>       Alias for --surface

            Example:
              phatmux new-split right
              phatmux new-split down --workspace workspace:1
            """
        case "list-panes":
            return """
            Usage: phatmux list-panes [--workspace <id|ref>]

            List panes in a workspace.

            Flags:
              --workspace <id|ref>   Workspace context (default: $PHATMUX_WORKSPACE_ID)

            Example:
              phatmux list-panes
              phatmux list-panes --workspace workspace:2
            """
        case "list-pane-surfaces":
            return """
            Usage: phatmux list-pane-surfaces [--workspace <id|ref>] [--pane <id|ref>]
            ...
            Example:
              phatmux list-pane-surfaces
              phatmux list-pane-surfaces --workspace workspace:2 --pane pane:1
            """
        case "tree":
            return """
            Usage: phatmux tree [flags]
            ...
            Output:
              ...
              - ◀ here (caller surface where `phatmux tree` was invoked)
              - workspace [selected]
              - pane [focused]
              - surface [selected]
            ...
            Example:
              phatmux tree
              phatmux tree --all
              phatmux tree --workspace workspace:2
              phatmux --json tree --all
            """
        case "focus-pane":
            return """
            Usage: phatmux focus-pane [--pane <id|ref> | <id|ref>] [flags]
            ...
            Example:
              ...
              phatmux focus-pane pane:1
              phatmux focus-pane --pane pane:1 --workspace workspace:2
            """
        case "new-pane":
            return """
            Usage: phatmux new-pane [flags]
            ...
            Example:
              phatmux new-pane
              phatmux new-pane --type browser --direction down --url https://example.com
            """
        case "new-surface":
            return """
            Usage: phatmux new-surface [flags]
        ...
        case "close-surface":
            return """
            Usage: phatmux close-surface [flags]

            Close a surface. Defaults to the focused surface if none specified.

            Flags:
              --surface <id|ref>     Surface to close (default: $PHATMUX_SURFACE_ID)
              --panel <id|ref>       Alias for --surface
              --workspace <id|ref>   Workspace context (default: $PHATMUX_WORKSPACE_ID)

            Example:
              phatmux close-surface
              phatmux close-surface --surface surface:3
            """
        case "drag-surface-to-split":
            return """
            Usage: phatmux drag-surface-to-split --surface <id|ref> <left|right|up|down>
            ...
            Example:
              phatmux drag-surface-to-split --surface surface:1 right
              phatmux drag-surface-to-split --panel surface:2 down
            """
        case "refresh-surfaces":
            return """
            Usage: phatmux refresh-surfaces

            Refresh surface snapshots for the focused workspace.
            """
        case "surface-health":
            return """
            Usage: phatmux surface-health [--workspace <id|ref>]
        ...
        case "trigger-flash":
            return """
            Usage: phatmux trigger-flash [--workspace <id|ref>] [--surface <id|ref>] [--panel <id|ref>]

            Trigger the unread flash indicator for a surface.

            Flags:
              --workspace <id|ref>   Workspace context (default: $PHATMUX_WORKSPACE_ID)
              --surface <id|ref>     Target surface (default: $PHATMUX_SURFACE_ID)
              --panel <id|ref>       Alias for --surface

            Example:
              phatmux trigger-flash
              phatmux trigger-flash --workspace workspace:2 --surface surface:3
            """
        case "list-panels":
            return """
            Usage: phatmux list-panels [--workspace <id|ref>]
            ...
            Example:
              phatmux list-panels
              phatmux list-panels --workspace workspace:2
            """
        case "focus-panel":
            return """
            Usage: phatmux focus-panel --panel <id|ref> [--workspace <id|ref>]
            ...
            Example:
              phatmux focus-panel --panel surface:2
              phatmux focus-panel --panel surface:5 --workspace workspace:2
            """
        case "close-workspace":
            return """
            Usage: phatmux close-workspace --workspace <id|ref|index>
            ...
            Example:
              phatmux close-workspace --workspace workspace:2
            """
        case "select-workspace":
            return """
            Usage: phatmux select-workspace --workspace <id|ref|index>
            ...
            Example:
              phatmux select-workspace --workspace workspace:2
              phatmux select-workspace --workspace 0
            """
        case "rename-workspace", "rename-window":
            return """
            Usage: phatmux rename-workspace [--workspace <id|ref|index>] [--] <title>

            Rename a workspace. Defaults to the current workspace.
            tmux-compatible alias: rename-window

            Flags:
              --workspace <id|ref|index>   Workspace to rename (default: current/$PHATMUX_WORKSPACE_ID)

            Example:
              phatmux rename-workspace "backend logs"
              phatmux rename-window --workspace workspace:2 "agent run"
            """
        case "current-workspace":
            return """
            Usage: phatmux current-workspace

            Print the currently selected workspace ID.
            """
        case "capture-pane":
            return """
            Usage: phatmux capture-pane [--workspace <id|ref>] [--surface <id|ref>] [--scrollback] [--lines <n>]

            tmux-compatible alias for reading terminal text from a pane.

            Flags:
              --workspace <id|ref>   Workspace context (default: $PHATMUX_WORKSPACE_ID)
              --surface <id|ref>     Surface context (default: $PHATMUX_SURFACE_ID)
              --scrollback           Include scrollback
              --lines <n>            Return only the last N lines (implies --scrollback)

            Example:
              phatmux capture-pane --workspace workspace:2 --surface surface:1 --scrollback --lines 200
            """
        case "resize-pane":
            return """
            Usage: phatmux resize-pane [--pane <id|ref>] [--workspace <id|ref>] [-L|-R|-U|-D] [--amount <n>]
        ...
        case "pipe-pane":
            return """
            Usage: phatmux pipe-pane [--workspace <id|ref>] [--surface <id|ref>] [--command <shell-command> | <shell-command>]
        ...
        case "wait-for":
            return """
            Usage: phatmux wait-for [-S|--signal] <name> [--timeout <seconds>]
        ...
        case "swap-pane":
            return """
            Usage: phatmux swap-pane --pane <id|ref> --target-pane <id|ref> [--workspace <id|ref>]
        ...
        case "break-pane":
            return """
            Usage: phatmux break-pane [--workspace <id|ref>] [--pane <id|ref>] [--surface <id|ref>] [--no-focus]
        ...
        case "join-pane":
            return """
            Usage: phatmux join-pane --target-pane <id|ref> [--workspace <id|ref>] [--pane <id|ref>] [--surface <id|ref>] [--no-focus]
        ...
        case "find-window":
            return """
            Usage: phatmux find-window [--content] [--select] [query]
        ...
        case "clear-history":
            return """
            Usage: phatmux clear-history [--workspace <id|ref>] [--surface <id|ref>]
        ...
        case "set-hook":
            return """
            Usage: phatmux set-hook [--list] [--unset <event>] | <event> <command>
        ...
        case "set-buffer":
            return """
            Usage: phatmux set-buffer [--name <name>] [--] <text>
        ...
        case "paste-buffer":
            return """
            Usage: phatmux paste-buffer [--name <name>] [--workspace <id|ref>] [--surface <id|ref>]
        ...
        case "list-buffers":
            return """
            Usage: phatmux list-buffers

            List tmux-compat buffers.
            """
        case "respawn-pane":
            return """
            Usage: phatmux respawn-pane [--workspace <id|ref>] [--surface <id|ref>] [--command <cmd> | <cmd>]
        ...
        case "display-message":
            return """
            Usage: phatmux display-message [-p|--print] <text>
        ...
        case "read-screen":
            return """
            Usage: phatmux read-screen [flags]
            ...
            Example:
              phatmux read-screen
              phatmux read-screen --surface surface:2 --scrollback --lines 200
            """
        case "send":
            return """
            Usage: phatmux send [flags] [--] <text>
            ...
            Example:
              phatmux send "echo hello"
              phatmux send --surface surface:2 "ls -la\\n"
            """
        case "send-key":
            return """
            Usage: phatmux send-key [flags] [--] <key>
            ...
            Example:
              phatmux send-key enter
              phatmux send-key --surface surface:2 ctrl+c
            """
        case "send-panel":
            return """
            Usage: phatmux send-panel --panel <id|ref> [flags] [--] <text>
            ...
            Example:
              phatmux send-panel --panel surface:2 "echo hello\\n"
            """
        case "send-key-panel":
            return """
            Usage: phatmux send-key-panel --panel <id|ref> [flags] [--] <key>
            ...
            Example:
              phatmux send-key-panel --panel surface:2 enter
              phatmux send-key-panel --panel surface:2 ctrl+c
            """
        case "notify":
            return """
            Usage: phatmux notify [flags]
            ...
            Example:
              phatmux notify --title "Build done" --body "All tests passed"
              phatmux notify --title "Error" --subtitle "test.swift" --body "Line 42: syntax error"
            """
        case "list-notifications":
            return """
            Usage: phatmux list-notifications

            List queued notifications.
            """
        case "clear-notifications":
            return """
            Usage: phatmux clear-notifications

            Clear all queued notifications.
            """
        case "set-status":
            return """
            Usage: phatmux set-status <key> <value> [flags]
            ...
            Example:
              phatmux set-status build "compiling" --icon hammer --color "#ff9500"
              phatmux set-status deploy "v1.2.3" --workspace workspace:2
            """
        case "clear-status":
            return """
            Usage: phatmux clear-status <key> [flags]
            ...
            Example:
              phatmux clear-status build
            """
        case "list-status":
            return """
            Usage: phatmux list-status [flags]
            ...
            Example:
              phatmux list-status
              phatmux list-status --workspace workspace:2
            """
        case "set-progress":
            return """
            Usage: phatmux set-progress <0.0-1.0> [flags]
            ...
            Example:
              phatmux set-progress 0.5 --label "Building..."
              phatmux set-progress 1.0 --label "Done"
            """
        case "clear-progress":
            return """
            Usage: phatmux clear-progress [flags]
            ...
            Example:
              phatmux clear-progress
            """
        case "log":
            return """
            Usage: phatmux log [flags] [--] <message>
        ...
        case "sidebar-state":
            return """
            Usage: phatmux sidebar-state [flags]
            ...
            Example:
              phatmux sidebar-state
              phatmux sidebar-state --workspace workspace:2
            """
        case "set-app-focus":
            return """
            Usage: phatmux set-app-focus <active|inactive|clear>

            Override app focus state for notification routing tests.

            Example:
              phatmux set-app-focus inactive
              phatmux set-app-focus clear
            """
        case "simulate-app-active":
            return """
            Usage: phatmux simulate-app-active

            Trigger the app-active handler used by notification focus tests.
            """
        case "claude-hook":
            return """
            Usage: phatmux claude-hook <session-start|active|stop|idle|notification|notify|prompt-submit> [flags]
            ...
            Example:
              echo '{"session_id":"abc"}' | phatmux claude-hook session-start
              echo '{}' | phatmux claude-hook stop
            """
        case "browser":
            return """
            Usage: phatmux browser [--surface <id|ref|index> | <surface>] <subcommand> [args]
            ...
            Subcommands:
              open|open-split|new [url] [--workspace <id|ref|index>] [--window <id|ref|index>]
                open/open-split/new default to $PHATMUX_WORKSPACE_ID when --workspace is omitted and --window is not set
            ...
            Example:
              phatmux browser open https://example.com
              phatmux browser surface:1 navigate https://google.com
              phatmux browser --surface surface:1 snapshot --interactive
            """
        // Legacy browser aliases — point users to `phatmux browser --help`
        case "open-browser":
            return "Legacy alias for 'phatmux browser open'. Run 'phatmux browser --help' for details."
        case "navigate":
            return "Legacy alias for 'phatmux browser navigate'. Run 'phatmux browser --help' for details."
        case "browser-back":
            return "Legacy alias for 'phatmux browser back'. Run 'phatmux browser --help' for details."
        case "browser-forward":
            return "Legacy alias for 'phatmux browser forward'. Run 'phatmux browser --help' for details."
        case "browser-reload":
            return "Legacy alias for 'phatmux browser reload'. Run 'phatmux browser --help' for details."
        case "get-url":
            return "Legacy alias for 'phatmux browser get-url'. Run 'phatmux browser --help' for details."
        case "focus-webview":
            return "Legacy alias for 'phatmux browser focus-webview'. Run 'phatmux browser --help' for details."
        case "is-webview-focused":
            return "Legacy alias for 'phatmux browser is-webview-focused'. Run 'phatmux browser --help' for details."
        case "markdown":
            return """
            Usage: phatmux markdown open <path> [options]
                   phatmux markdown <path>       (shorthand for 'open')
            ...
            Options:
              --workspace <id|ref|index>   Target workspace (default: $PHATMUX_WORKSPACE_ID)
              --surface <id|ref|index>     Source surface to split from (default: focused surface)
              --window <id|ref|index>      Target window
              --direction <left|right|up|down>  Split direction (default: right)

            Examples:
              phatmux markdown open plan.md
              phatmux markdown ~/project/CHANGELOG.md
              phatmux markdown open ./docs/design.md --workspace 0
              phatmux markdown open plan.md --direction down
            """
        default:
            return nil
        }
    ...
    private func dispatchSubcommandHelp(command: String, commandArgs: [String]) -> Bool {
        ...
        print("phatmux \(command)")
        print("")
        print(text)
        return true
    }

    private static let phatmuxThemeOverrideBundleIdentifier = "com.phatmux.app"
    private static let phatmuxThemesBlockStart = "# phatmux themes start"
    private static let phatmuxThemesBlockEnd = "# phatmux themes end"
    private static let phatmuxThemesReloadNotificationName = "com.phatmux.themes.reload-config"
    ...
    private func runInteractiveThemes() throws {
        ...
        environment["PHATMUX_THEME_PICKER_CONFIG"] = try phatmuxThemeOverrideConfigURL().path
        environment["PHATMUX_THEME_PICKER_BUNDLE_ID"] = currentCmuxAppBundleIdentifier() ?? Self.phatmuxThemeOverrideBundleIdentifier
        environment["PHATMUX_THEME_PICKER_TARGET"] = defaultThemePickerTargetMode(current: selection).rawValue
        environment["PHATMUX_THEME_PICKER_COLOR_SCHEME"] = defaultAppearancePrefersDarkThemes() ? "dark" : "light"
        if let light = selection.light {
            environment["PHATMUX_THEME_PICKER_INITIAL_LIGHT"] = light
        }
        if let dark = selection.dark {
            environment["PHATMUX_THEME_PICKER_INITIAL_DARK"] = dark
        }
    ...
    private func runThemes(commandArgs: [String], jsonOutput: Bool) throws {
        ...
        default:
            if subcommand.hasPrefix("-") {
                throw CLIError(message: "Unknown themes subcommand '\(subcommand)'. Run 'phatmux themes --help'.")
            }
    ...
    private func printThemesList(jsonOutput: Bool) throws {
        ...
        let configPath = try phatmuxThemeOverrideConfigURL().path
    ...
    private func validatedThemeName(_ rawValue: String, availableThemes: [String]) throws -> String {
        ...
        throw CLIError(message: "Unknown theme '\(trimmed)'. Run 'phatmux themes' to list available themes.")
    }
    ...
    private func themeConfigSearchURLs() -> [URL] {
        let rawPaths = [
            ...
            "~/Library/Application Support/\(Self.phatmuxThemeOverrideBundleIdentifier)/config",
            "~/Library/Application Support/\(Self.phatmuxThemeOverrideBundleIdentifier)/config.ghostty",
        ]
    ...
    private func phatmuxThemeOverrideConfigURL() throws -> URL {
        guard let appSupport = FileManager.default.urls(for: .applicationSupportDirectory, in: .userDomainMask).first else {
            throw CLIError(message: "Unable to resolve Application Support directory")
        }
        return appSupport
            .appendingPathComponent(Self.phatmuxThemeOverrideBundleIdentifier, isDirectory: true)
            .appendingPathComponent("config.ghostty", isDirectory: false)
    }

    private func writeManagedThemeOverride(rawThemeValue: String) throws -> URL {
        let fileManager = FileManager.default
        let configURL = try phatmuxThemeOverrideConfigURL()
        ...
        let strippedContents = removingManagedThemeOverride(from: existingContents)
            .trimmingCharacters(in: .whitespacesAndNewlines)
        let block = """
        \(Self.phatmuxThemesBlockStart)
        theme = \(rawThemeValue)
        \(Self.phatmuxThemesBlockEnd)
        """
    ...
    private func reloadThemesIfPossible() -> ThemeReloadStatus {
        let bundleIdentifier = currentCmuxAppBundleIdentifier() ?? Self.phatmuxThemeOverrideBundleIdentifier
        DistributedNotificationCenter.default().post(
            name: Notification.Name(Self.phatmuxThemesReloadNotificationName),
            object: nil,
    ...
    private func currentCmuxAppBundleIdentifier() -> String? {
        if let bundleIdentifier = ProcessInfo.processInfo.environment["PHATMUX_BUNDLE_ID"]?.trimmingCharacters(in: .whitespacesAndNewlines),
           !bundleIdentifier.isEmpty {
            return bundleIdentifier
        }
    ...
    private func workspaceFromArgsOrEnv(_ args: [String], windowOverride: String? = nil) -> String? {
        ...
        return ProcessInfo.processInfo.environment["PHATMUX_WORKSPACE_ID"]
    }
    ...
    private func treeCallerContextFromEnvironment() -> [String: Any]? {
        let env = ProcessInfo.processInfo.environment
        let workspaceRaw = env["PHATMUX_WORKSPACE_ID"]?.trimmingCharacters(in: .whitespacesAndNewlines)
        let surfaceRaw = env["PHATMUX_SURFACE_ID"]?.trimmingCharacters(in: .whitespacesAndNewlines)
        var caller: [String: Any] = [:]
    ...
    private func tmuxCallerWorkspaceHandle() -> String? {
        normalizedTmuxTarget(ProcessInfo.processInfo.environment["PHATMUX_WORKSPACE_ID"])
    }
    ...
    private func tmuxCallerPaneHandle() -> String? {
        guard let pane = normalizedTmuxTarget(ProcessInfo.processInfo.environment["TMUX_PANE"])
            ?? normalizedTmuxTarget(ProcessInfo.processInfo.environment["PHATMUX_PANE_ID"]) else {
            return nil
        }
    ...
    private func tmuxCallerSurfaceHandle() -> String? {
        normalizedTmuxTarget(ProcessInfo.processInfo.environment["PHATMUX_SURFACE_ID"])
    }
    ...
    private func isCmuxClaudeWrapper(at path: String) -> Bool {
        ...
        return prefix.contains("phatmux claude wrapper - injects hooks and session tracking")
    }
    ...
    private func configureClaudeTeamsEnvironment(
        ...
        let fakeTmuxValue: String = {
            if let focusedContext {
                ...
                return "/tmp/phatmux-claude-teams/\(focusedContext.workspaceId),\(windowToken),\(focusedContext.paneHandle)"
            }
            return processEnvironment["TMUX"] ?? "/tmp/phatmux-claude-teams/default,0,0"
        }()
        let fakeTmuxPane = focusedContext.map { "%\($0.paneHandle)" }
            ?? processEnvironment["TMUX_PANE"]
            ?? "%1"
        let fakeTerm = processEnvironment["PHATMUX_CLAUDE_TEAMS_TERM"] ?? "screen-256color"

        setenv("CLAUDE_CODE_EXPERIMENTAL_AGENT_TEAMS", "1", 1)
        setenv("PHATMUX_CLAUDE_TEAMS_CLI_BIN", executablePath, 1)
        setenv("PATH", updatedPath, 1)
        setenv("TMUX", fakeTmuxValue, 1)
        setenv("TMUX_PANE", fakeTmuxPane, 1)
        setenv("TERM", fakeTerm, 1)
        setenv("PHATMUX_SOCKET_PATH", socketPath, 1)
        setenv("PHATMUX_SOCKET", socketPath, 1)
        if let explicitPassword,
           !explicitPassword.trimmingCharacters(in: .whitespacesAndNewlines).isEmpty {
            setenv("PHATMUX_SOCKET_PASSWORD", explicitPassword, 1)
        }
        unsetenv("TERM_PROGRAM")
        if let focusedContext {
            setenv("PHATMUX_WORKSPACE_ID", focusedContext.workspaceId, 1)
            if let surfaceId = focusedContext.surfaceId, !surfaceId.isEmpty {
                setenv("PHATMUX_SURFACE_ID", surfaceId, 1)
            }
        }
    }
    ...
    private func createClaudeTeamsShimDirectory() throws -> URL {
        ...
        #!/usr/bin/env bash
        set -euo pipefail
        exec "${PHATMUX_CLAUDE_TEAMS_CLI_BIN:-phatmux}" __tmux-compat "$@"
        """
    ...
    private func runClaudeTeams(
        ...
        launcherEnvironment["PHATMUX_SOCKET_PATH"] = socketPath
        launcherEnvironment["PHATMUX_SOCKET"] = socketPath
        if let explicitPassword,
           !explicitPassword.trimmingCharacters(in: .whitespacesAndNewlines).isEmpty {
            launcherEnvironment["PHATMUX_SOCKET_PASSWORD"] = explicitPassword
        }
        let shimDirectory = try createClaudeTeamsShimDirectory()
        let executablePath = resolvedExecutableURL()?.path ?? (args.first ?? "phatmux")
    ...
    private func runClaudeTeamsTmuxCompat(
        ...
        case "new-session", "new":
            ...
            if parsed.hasFlag("-A") {
                throw CLIError(message: "new-session -A is not supported in phatmux claude-teams mode")
            }
        ...
        case "new-window", "neww":
            ...
            if parsed.value("-t") != nil {
                throw CLIError(message: "new-window -t is not supported in phatmux claude-teams mode")
            }
            var params: [String: Any] = ["focus": false]
            if let cwd = parsed.value("-c") {
                params["cwd"] = resolvePath(cwd)
    ...
    private func tmuxCompatStoreURL() -> URL {
        let root = NSString(string: "~/.phatmux").expandingTildeInPath
    ...
    private func tmuxWaitForSignalURL(name: String) -> URL {
        ...
        return URL(fileURLWithPath: "/tmp/phatmux-wait-for-\(String(sanitized)).sig")
    }
    ...
    private func runTmuxCompatCommand(
        ...
        case "capture-pane":
            ...
            let (sfArg, rem1) = parseOption(rem0, name: "--surface")
            let (linesArg, rem2) = parseOption(rem1, name: "--lines")
            let workspaceArg = wsArg ?? (windowOverride == nil ? ProcessInfo.processInfo.environment["PHATMUX_WORKSPACE_ID"] : nil)
            let surfaceArg = sfArg ?? (wsArg == nil && windowOverride == nil ? ProcessInfo.processInfo.environment["PHATMUX_SURFACE_ID"] : nil)
        ...
        case "popup":
            throw CLIError(message: "popup is not supported yet in phatmux CLI parity mode")

        case "bind-key", "unbind-key", "copy-mode":
            throw CLIError(message: "\(command) is not supported yet in phatmux CLI parity mode")
        ...
        case "display-message":
            ...
            let payload = try client.sendV2(method: "notification.create", params: ["title": "phatmux", "body": message])
            if jsonOutput {
                print(jsonString(payload))
            } else {
                print(message)
    ...
    private func runClaudeHook(
        ...
        let workspaceArg = hookWsFlag ?? ProcessInfo.processInfo.environment["PHATMUX_WORKSPACE_ID"]
        let surfaceArg = optionValue(hookArgs, name: "--surface") ?? (hookWsFlag == nil ? ProcessInfo.processInfo.environment["PHATMUX_SURFACE_ID"] : nil)
        ...
        case "session-start", "active":
            ...
            let claudePid: Int? = {
                guard let raw = ProcessInfo.processInfo.environment["PHATMUX_CLAUDE_PID"]?
                    .trimmingCharacters(in: .whitespacesAndNewlines),
                    let pid = Int(raw),
                    pid > 0 else {
                    return nil
        ...
        case "help", "--help", "-h":
            ...
            print(
                """
                phatmux claude-hook <session-start|stop|session-end|notification|prompt-submit|pre-tool-use> [--workspace <id|index>] [--surface <id|index>]
                """
            )
    ...
    private func versionSummary() -> String {
        let info = resolvedVersionInfo()
        let commit = info["PHATMUXCommit"].flatMap { normalizedCommitHash($0) }
        let baseSummary: String
        if let version = info["CFBundleShortVersionString"], let build = info["CFBundleVersion"] {
            baseSummary = "phatmux \(version) (\(build))"
        } else if let version = info["CFBundleShortVersionString"] {
            baseSummary = "phatmux \(version)"
        } else if let build = info["CFBundleVersion"] {
            baseSummary = "phatmux build \(build)"
        } else {
            baseSummary = "phatmux version unknown"
        }
    ...
    private func printWelcome() {
        ...
        print("  \(subdued)Run \(reset)\(bold)phatmux shortcuts\(reset)\(subdued) to edit shortcuts.\(reset)")
        print("  \(subdued)Run \(reset)\(bold)phatmux feedback\(reset)\(subdued) to report a bug.\(reset)")
        print()
    }
    ...
    private func resolvedVersionInfo() -> [String: String] {
        ...
        if info["PHATMUXCommit"] == nil,
           let commit = normalizedCommitHash(ProcessInfo.processInfo.environment["PHATMUX_COMMIT"]) {
            info["PHATMUXCommit"] = commit
        }
    ...
    private func versionInfo(from dictionary: [String: Any]?) -> [String: String]? {
        ...
        if let commit = dictionary["PHATMUXCommit"] as? String,
           let normalizedCommit = normalizedCommitHash(commit) {
            info["PHATMUXCommit"] = normalizedCommit
        }
    ...
    private func usage() -> String {
        return """
        phatmux - control phatmux via Unix socket

        Usage:
          phatmux <path>                Open a directory in a new workspace (launches phatmux if needed)
          phatmux [global-options] <command> [options]
        ...
        Socket Auth:
          --password takes precedence, then PHATMUX_SOCKET_PASSWORD env var, then password saved in Settings.
        ...
        Environment:
          PHATMUX_WORKSPACE_ID   Auto-set in phatmux terminals. Used as default --workspace for
                              ALL commands (send, list-panels, new-split, notify, etc.).
          PHATMUX_TAB_ID         Optional alias used by `tab-action`/`rename-tab` as default --tab.
          PHATMUX_SURFACE_ID     Auto-set in phatmux terminals. Used as default --surface.
          PHATMUX_SOCKET_PATH    Override the Unix socket path. Without this, the CLI defaults
                              to ~/Library/Application Support/phatmux/phatmux.sock and auto-discovers tagged/debug sockets.
        """
    }
