# phatmux shell integration for zsh
# Injected automatically — do not source manually

_phatmux_send() {
    local payload="$1"
    if command -v ncat >/dev/null 2>&1; then
        print -r -- "$payload" | ncat -w 1 -U "$PHATMUX_SOCKET_PATH" --send-only
    elif command -v socat >/dev/null 2>&1; then
        print -r -- "$payload" | socat -T 1 - "UNIX-CONNECT:$PHATMUX_SOCKET_PATH" >/dev/null 2>&1
        ...
        if print -r -- "$payload" | nc -N -U "$PHATMUX_SOCKET_PATH" >/dev/null 2>&1; then
            :
        else
            print -r -- "$payload" | nc -w 1 -U "$PHATMUX_SOCKET_PATH" >/dev/null 2>&1 || true
        fi
    fi
}
...
typeset -g _PHATMUX_PWD_LAST_PWD=""
typeset -g _PHATMUX_GIT_LAST_PWD=""
typeset -g _PHATMUX_GIT_LAST_RUN=0
typeset -g _PHATMUX_GIT_JOB_PID=""
typeset -g _PHATMUX_GIT_JOB_STARTED_AT=0
typeset -g _PHATMUX_GIT_FORCE=0
typeset -g _PHATMUX_GIT_HEAD_LAST_PWD=""
typeset -g _PHATMUX_GIT_HEAD_PATH=""
typeset -g _PHATMUX_GIT_HEAD_SIGNATURE=""
typeset -g _PHATMUX_GIT_HEAD_WATCH_PID=""
typeset -g _PHATMUX_PR_POLL_PID=""
typeset -g _PHATMUX_PR_POLL_PWD=""
typeset -g _PHATMUX_PR_POLL_INTERVAL=45
typeset -g _PHATMUX_PR_FORCE=0
typeset -g _PHATMUX_ASYNC_JOB_TIMEOUT=20

typeset -g _PHATMUX_PORTS_LAST_RUN=0
typeset -g _PHATMUX_CMD_START=0
typeset -g _PHATMUX_SHELL_ACTIVITY_LAST=""
typeset -g _PHATMUX_TTY_NAME=""
typeset -g _PHATMUX_TTY_REPORTED=0
typeset -g _PHATMUX_GHOSTTY_SEMANTIC_PATCHED=0
typeset -g _PHATMUX_WINCH_GUARD_INSTALLED=0

_phatmux_ensure_ghostty_preexec_strips_both_marks() {
    local fn_name="$1"
    ...
    if [[ "$updated" == *"$new_strip"* && "$updated" != *"$old_strip"* ]]; then
        ...
        _PHATMUX_GHOSTTY_SEMANTIC_PATCHED=1
        return 0
    fi
...
_phatmux_patch_ghostty_semantic_redraw() {
    ...
    _phatmux_ensure_ghostty_preexec_strips_both_marks _ghostty_deferred_init
    _phatmux_ensure_ghostty_preexec_strips_both_marks _ghostty_preexec
}
_phatmux_patch_ghostty_semantic_redraw

_phatmux_prompt_wrap_guard() {
    local cmd_start="$1"
    local pwd="$2"
...
_phatmux_install_winch_guard() {
    (( _PHATMUX_WINCH_GUARD_INSTALLED )) && return 0
    ...
    if (( $+functions[TRAPWINCH] )) || [[ -n "$existing_winch_trap" ]]; then
        _PHATMUX_WINCH_GUARD_INSTALLED=1
        return 0
    fi
...
_phatmux_ports_kick() {
    ...
    [[ -n "$PHATMUX_TAB_ID" ]] || return 0
    [[ -n "$PHATMUX_PANEL_ID" ]] || return 0
    _PHATMUX_PORTS_LAST_RUN=$EPOCHSECONDS
    {
        _phatmux_send "ports_kick --tab=$PHATMUX_TAB_ID --panel=$PHATMUX_PANEL_ID"
    } >/dev/null 2>&1 &!
}

_phatmux_report_git_branch_for_path() {
    local repo_path="$1"
...
_phatmux_stop_git_head_watch() {
    if [[ -n "$_PHATMUX_GIT_HEAD_WATCH_PID" ]]; then
        kill "$_PHATMUX_GIT_HEAD_WATCH_PID" >/dev/null 2>&1 || true
        _PHATMUX_GIT_HEAD_WATCH_PID=""
    fi
}

_phatmux_start_git_head_watch() {
    [[ -S "$PHATMUX_SOCKET_PATH" ]] || return 0
    [[ -n "$PHATMUX_TAB_ID" ]] || return 0
    [[ -n "$PHATMUX_PANEL_ID" ]] || return 0
    ...
    watch_head_path="$(_phatmux_git_resolve_head_path 2>/dev/null || true)"
    [[ -n "$watch_head_path" ]] || return 0

    local watch_head_signature
    watch_head_signature="$(_phatmux_git_head_signature "$watch_head_path" 2>/dev/null || true)"

    _PHATMUX_GIT_HEAD_LAST_PWD="$watch_pwd"
    _PHATMUX_GIT_HEAD_PATH="$watch_head_path"
    _PHATMUX_GIT_HEAD_SIGNATURE="$watch_head_signature"
    ...
    _phatmux_stop_git_head_watch
    {
        ...
        while true; do
            ...
            local signature
            signature="$(_phatmux_git_head_signature "$watch_head_path" 2>/dev/null || true)"
            if [[ -n "$signature" && "$signature" != "$last_signature" ]]; then
                last_signature="$signature"
                _phatmux_report_git_branch_for_path "$watch_pwd"
            fi
        done
    } >/dev/null 2>&1 &!
    _PHATMUX_GIT_HEAD_WATCH_PID=$!
...
_phatmux_preexec() {
    ...
    _PHATMUX_CMD_START=$EPOCHSECONDS
...
_phatmux_precmd() {
    ...
    local pwd="$PWD"
    local cmd_start="$_PHATMUX_CMD_START"
    _PHATMUX_CMD_START=0

    _phatmux_prompt_wrap_guard "$cmd_start" "$pwd"
    ...
    # While a foreground command is running, _phatmux_start_git_head_watch probes HEAD
    ...
    if [[ "$pwd" != "$_PHATMUX_GIT_LAST_PWD" ]]; then
        should_git=1
    elif (( _PHATMUX_GIT_FORCE )); then
        should_git=1
    elif (( now - _PHATMUX_GIT_LAST_RUN >= 3 )); then
        should_git=1
    fi
    ...
    if (( should_git )); then
        ...
        if [[ -n "$_PHATMUX_GIT_JOB_PID" ]] && kill -0 "$_PHATMUX_GIT_JOB_PID" 2>/dev/null; then
            ...
            if [[ "$pwd" != "$_PHATMUX_GIT_LAST_PWD" ]] || (( _PHATMUX_GIT_FORCE )); then
        ...
        if (( can_launch_git )); then
            _PHATMUX_GIT_FORCE=0
            _PHATMUX_GIT_LAST_PWD="$pwd"
            _PHATMUX_GIT_LAST_RUN=$now
            {
                _phatmux_report_git_branch_for_path "$pwd"
            } >/dev/null 2>&1 &!
            _PHATMUX_GIT_JOB_PID=$!
    ...
    if [[ "$pwd" != "$_PHATMUX_PR_POLL_PWD" ]]; then
    ...
    if (( cmd_dur >= 2 || now - _PHATMUX_PORTS_LAST_RUN >= 10 )); then
        _phatmux_ports_kick
    fi
}
...
_phatmux_fix_path() {
    ...
    add-zsh-hook -d precmd _phatmux_fix_path
}
...
add-zsh-hook preexec _phatmux_preexec
add-zsh-hook precmd _phatmux_precmd
add-zsh-hook precmd _phatmux_fix_path
add-zsh-hook zshexit _phatmux_zshexit
