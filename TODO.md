...
- [x] `phatmux ssh` creates remote workspace metadata and does not require `--name`
- [x] Remote daemon bootstrap/upload/start path with `phatmuxd-remote serve --stdio`
...
- [ ] Extend `phatmuxd-remote` RPC beyond `hello/ping` with proxy stream methods (`proxy.open|close`)
...
- [x] Add browser automation API inspired by `vercel-labs/agent-browser`, but backed by phatmux's WKWebView (wait, click, type, eval, screenshot, etc.).
- [x] Finalize browser parity contract and command mapping decisions in `docs/agent-browser-port-spec.md`.
- [x] Add `phatmux browser` command surface that mirrors agent-browser semantics and targets explicit `surface_id` handles.
...
- [x] Fix `phatmux browser open|open-split|new` URL parsing so routing flags (`--workspace`, `--window`) are removed before URL construction.
...
- [x] Expand end-user skill docs with deep-linkable phatmux-browser references/templates plus a new core `skills/phatmux/` topology skill.
...
- [ ] Add "Install Claude Code integration" menu item in menubar
  ...
  - Implement as part of `phatmux` CLI, menubar just triggers the CLI command
...
- [ ] Cmd+click should open links in phatmux (browser panel) instead of external browser
...
- [x] Extend `phatmux browser ...` CLI grammar for the new families (including aliases).
...
- [x] Re-run full `run-tests-v1.sh` and `run-tests-v2.sh` on `phatmux-vm`.
