...
func shouldRunCLIForInvocation(argv0 string, args []string) bool {
	...
	if !strings.HasPrefix(base, "phatmuxd-remote") || len(args) == 0 {
		return false
	}
...
func usage(w io.Writer) {
	...
	_, _ = fmt.Fprintln(w, "  phatmuxd-remote version")
	_, _ = fmt.Fprintln(w, "  phatmuxd-remote serve --stdio")
	_, _ = fmt.Fprintln(w, "  phatmuxd-remote cli <command> [args...]")
}
