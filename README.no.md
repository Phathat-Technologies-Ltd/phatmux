...
<p align="center">
  <a href="https://github.com/Phathat-Technologies-Ltd/phatmux/releases/latest/download/phatmux-macos.dmg">
    <img src="./docs/assets/macos-badge.png" alt="Last ned phatmux for macOS" width="180" />
  </a>
</p>
...
<p align="center">
  <img src="./docs/assets/main-first-image.png" alt="phatmux skjermbilde" width="900" />
</p>
...
Åpne `.dmg`-filen og dra phatmux til Programmer-mappen. phatmux oppdaterer seg selv automatisk via Sparkle, så du trenger bare å laste ned én gang.
...
Jeg prøvde noen kodeorkestratorer, men de fleste var Electron/Tauri-apper og ytelsen irriterte meg. Jeg foretrekker også terminalen siden GUI-orkestratorer låser deg inn i arbeidsflyten deres. Så jeg bygde phatmux som en nativ macOS-app i Swift/AppKit. Den bruker libghostty for terminalgjengivelse og leser din eksisterende Ghostty-konfigurasjon for temaer, skrifttyper og farger.

Hovedtilleggene er sidefeltet og varselsystemet. Sidefeltet har vertikale faner som viser git-gren, tilknyttet PR-status/nummer, arbeidsmappe, lyttende porter og siste varselstekst for hvert arbeidsområde. Varselsystemet fanger opp terminalsekvenser (OSC 9/99/777) og har en CLI (`phatmux notify`) du kan koble til agentkroker for Claude Code, OpenCode osv. Når en agent venter, får panelet en blå ring og fanen lyser opp i sidefeltet, så jeg kan se hvilken som trenger meg på tvers av delinger og faner. Cmd+Shift+U hopper til det nyeste uleste.
...
phatmux er ikke foreskrivende om hvordan utviklere bruker verktøyene sine. Det er en terminal og nettleser med en CLI, og resten er opp til deg.

phatmux er en primitiv, ikke en løsning. Det gir deg en terminal, en nettleser, varsler, arbeidsområder, delinger, faner og en CLI for å kontrollere alt sammen. phatmux tvinger deg ikke inn i en bestemt måte å bruke kodeagenter på. Hva du bygger med primitivene er ditt.
...
For mer informasjon om hvordan du konfigurerer phatmux, [gå til dokumentasjonen vår](https://phatmux.com/docs/getting-started?utm_source=readme).
...
[Last ned phatmux NIGHTLY](https://github.com/Phathat-Technologies-Ltd/phatmux/releases/download/nightly/phatmux-nightly-macos.dmg)

phatmux NIGHTLY er en separat app med sin egen bundle-ID, så den kjører ved siden av den stabile versjonen. Bygges automatisk fra den siste `main`-commiten og oppdateres automatisk via sin egen Sparkle-feed.

## Sesjonssgjenoppretting (nåværende oppførsel)

Ved omstart gjenoppretter phatmux for øyeblikket kun applayouten og metadata:
- Vindu-/arbeidsområde-/panellayout
- Arbeidsmapper
- Terminal-rullingshistorikk (best effort)
- Nettleser-URL og navigasjonshistorikk

phatmux gjenoppretter **ikke** aktive prosesstilstander inne i terminalapper. For eksempel blir aktive Claude Code/tmux/vim-sesjoner ikke gjenopptatt etter omstart ennå.
...
- Opprett og delta i [GitHub-issues](https://github.com/Phathat-Technologies-Ltd/phatmux/issues) og [diskusjoner](https://github.com/Phathat-Technologies-Ltd/phatmux/discussions)
- Fortell oss hva du bygger med phatmux
...
phatmux er gratis, åpen kildekode, og vil alltid være det. Hvis du vil støtte utviklingen og få tidlig tilgang til det som kommer:

**[Få Grunnleggerutgaven](https://buy.stripe.com/3cI00j2Ld0it5OU33r5EY0q)**

- **Prioriterte funksjonsforespørsler/feilrettinger**
- **Tidlig tilgang: phatmux AI som gir deg kontekst om hvert arbeidsområde, fane og panel**
