...
if ! grep -Fq 'com.phatmux.app.nightly.universal' "$WORKFLOW_FILE"; then
...
if ! awk '
  ...
  in_upload && /phatmux-nightly-macos\*\.dmg/ { saw_arm_artifacts=1 }
  in_upload && /phatmux-nightly-universal-macos\*\.dmg/ { saw_universal_artifacts=1 }
...
if ! awk '
  ...
  in_publish && /phatmux-nightly-universal-macos-\$\{\{ github\.run_id \}\}\*\.dmg/ { saw_universal_immutable=1 }
  in_publish && /phatmux-nightly-universal-macos\.dmg/ { saw_universal_stable=1 }
