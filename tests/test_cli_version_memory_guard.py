#!/usr/bin/env python3
"""
Regression test: `phatmux --version` must not scan huge sibling app lists just to
...
EXPECTED_STDOUT = "phatmux 9.9.9 (999)"


def resolve_phatmux_cli() -> str:
    explicit = os.environ.get("PHATMUX_CLI_BIN") or os.environ.get("PHATMUX_CLI")
    if explicit and os.path.exists(explicit) and os.access(explicit, os.X_OK):
        return explicit

    candidates: list[str] = []
    candidates.extend(glob.glob(os.path.expanduser("~/Library/Developer/Xcode/DerivedData/*/Build/Products/Debug/phatmux")))
    ...
    in_path = shutil.which("phatmux")
    if in_path:
        return in_path

    raise RuntimeError("Unable to find phatmux CLI binary. Set PHATMUX_CLI_BIN.")
...
def build_fixture(root: str, cli_path: str) -> str:
    app_path = os.path.join(root, "phatmux.app")
    contents_path = os.path.join(app_path, "Contents")
    resources_path = os.path.join(contents_path, "Resources")
    bin_path = os.path.join(resources_path, "bin")
    os.makedirs(bin_path, exist_ok=True)

    fixture_cli = os.path.join(bin_path, "phatmux")
    shutil.copy2(cli_path, fixture_cli)
    copy_runtime_frameworks(cli_path, contents_path)

    info = {
        "CFBundleExecutable": "phatmux",
        "CFBundleIdentifier": "test.phatmux.version-memory-guard",
...
def main() -> int:
    ...
    with tempfile.TemporaryDirectory(prefix="phatmux-version-memory-guard-") as root:
        fixture_cli = build_fixture(root, cli_path)
        result = run_with_limits(fixture_cli, "--version")

    if result["failure_reason"]:
        print("FAIL: `phatmux --version` exceeded runtime guard")
    ...
    if result["exit_code"] != 0:
        print("FAIL: `phatmux --version` exited non-zero")
    ...
    print(
        "PASS: `phatmux --version` exits within memory/time limits "
