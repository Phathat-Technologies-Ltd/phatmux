...
This test validates:
  ...
  2) Text insertion via debug socket (`simulate_type`) lands in the expected terminal by writing
     $PHATMUX_SURFACE_ID to a temp file.
"""
...
FOCUS_FILE = Path("/tmp/phatmux_focus_routing.txt")


def _focused_surface_id(c: phatmux) -> str:
    surfaces = c.list_surfaces()
    for _, sid, focused in surfaces:
        if focused:
            return sid
    raise phatmuxError(f"No focused surface in list_surfaces: {surfaces}")
    ...
    while time.time() - start < timeout_s:
        if path.exists():
            ...
            if data:
                return data
        time.sleep(0.05)
    raise phatmuxError(f"Timed out waiting for file content: {path}")


def _wait_for_terminal_focus(c: phatmux, panel_id: str, timeout_s: float = 6.0) -> None:
    start = time.time()
    while time.time() - start < timeout_s:
        if c.is_terminal_focused(panel_id):
            return
        time.sleep(0.05)
    raise phatmuxError(f"Timed out waiting for terminal focus: {panel_id}")


def _focus_and_wait(c: phatmux, panel_id: str, *, total_timeout_s: float = 8.0) -> None:
    """
    ...
    raise phatmuxError(f"Failed to focus terminal surface (panel_id={panel_id}): {last_err}")


def _assert_routed_to_surface(c: phatmux, expected_surface_id: str, panel_id: str) -> None:
    last_actual = "<empty>"
    for attempt in range(4):
        ...
        c.simulate_type(f"echo $PHATMUX_SURFACE_ID > {FOCUS_FILE}")
...
def main() -> int:
    with phatmux(SOCKET_PATH) as c:
        ...
        surfaces = c.list_surfaces()
        if not surfaces:
            raise phatmuxError("Expected at least one surface after new_workspace")
