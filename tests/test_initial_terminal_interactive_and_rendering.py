...
def _wait_for_surface_focus(c: phatmux, panel_id: str, timeout_s: float = 5.0) -> None:
    ...
    raise phatmuxError(f"Timed out waiting for surface focus: {panel_id}")


def _wait_for_render_context(c: phatmux, panel_id: str, timeout_s: float = 5.0) -> dict:
    ...
    while time.time() - start < timeout_s:
        ...
        if bool(last.get("inWindow")):
            return last
        time.sleep(0.1)
    raise phatmuxError(f"Expected inWindow render context, got: {last}")


def main() -> int:
    token = f"PHATMUX_INIT_{int(time.time() * 1000)}"
    tmp = f"/tmp/phatmux_init_{token}.txt"
    with phatmux(SOCKET_PATH) as c:
        ...
        if not surfaces:
            raise phatmuxError("Expected at least 1 surface after new_workspace")
