...
Requires:
  - phatmux running with allowAll socket mode
  - bash shell integration sourced (phatmux-bash-integration.bash)

Run with a tagged instance:
  PHATMUX_TAG=<tag> python3 tests/test_split_cwd_inheritance.py
"""
...
from phatmux import phatmux  # noqa: E402
...
def _focus_first_surface(client: phatmux) -> str:
...
def main() -> int:
    ...
    client = phatmux(socket_path=socket_path)
    client.connect()

    # Use resolved paths to avoid /tmp -> /private/tmp symlink mismatch on macOS
    test_dir_a = str(Path("/tmp/phatmux_split_cwd_test_a").resolve())
    test_dir_b = str(Path("/tmp/phatmux_split_cwd_test_b").resolve())
