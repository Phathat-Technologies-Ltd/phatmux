...
def wait_for_notification(client: phatmux, surface_id: str, is_read: bool, timeout: float = 2.0) -> bool:
...
def surface_id_for_index(client: phatmux, index: int) -> str:
...
def ensure_two_surfaces(client: phatmux) -> None:
    surfaces = client.list_surfaces()
    if len(surfaces) < 2:
        client.new_split("right")
        time.sleep(0.2)

def first_two_terminal_indices(client: phatmux) -> tuple[int, int]:
