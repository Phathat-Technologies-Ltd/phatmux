#!/usr/bin/env python3
"""
Regression: phatmux relies on Ghostty's xterm-ghostty terminfo entry.

In phatmux (embedded GhosttyKit), "bright" SGR 90-97 can render incorrectly
for some palettes. zsh-autosuggestions defaults to `fg=8`, which historically
resolved to SGR 90 via terminfo `setaf`.

phatmux ships a terminfo overlay that forces bright colors to use indexed
