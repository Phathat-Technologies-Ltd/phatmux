...
Environment:
  PHATMUX_SOCKET or PHATMUX_SOCKET_PATH can override the socket path.
"""
...
SOCKET_PATH = os.environ.get("PHATMUX_SOCKET") or os.environ.get("PHATMUX_SOCKET_PATH") or "/tmp/phatmux-debug.sock"
...
def _focused_panel_id(c: phatmux) -> str:
    surfaces = c.list_surfaces()
    if not surfaces:
        raise phatmuxError("Expected at least 1 surface")
    return next((sid for _i, sid, focused in surfaces if focused), surfaces[0][1])


def _snap_panel(c: phatmux, panel_id: str, label: str) -> Shot:
...
def _panel_sequence_blink_and_type(c: phatmux, panel_id: str, prefix: str, typed_char: str = "x") -> tuple[list[Shot], dict]:
    shots: list[Shot] = []
...
<head>
  <meta charset="utf-8" />
  <title>phatmux terminal input render report</title>
  <style>
    :root {{
      --bg: #0b0f14;
      --panel: #111826;
...
<body>
  <h1>phatmux terminal input render report</h1>
