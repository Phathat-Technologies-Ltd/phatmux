#!/bin/bash
# Test script that sends keystrokes to phatmux via AppleScript
# This tests the actual keyboard input path through the app

set -e

echo "=== phatmux Keystroke Test ==="
echo ""

# Check if phatmux is running
if ! pgrep -x "phatmux" > /dev/null; then
    echo "Error: phatmux is not running"
    echo "Please start phatmux first"
    exit 1
fi

echo "phatmux is running"
echo ""

# Activate phatmux
osascript -e 'tell application "phatmux" to activate'
sleep 0.5
...
echo "Please check the phatmux window to verify:"
