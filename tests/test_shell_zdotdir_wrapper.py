...
The phatmux zsh integration sets ZDOTDIR to the app's wrapper directory so zsh
...
def main() -> int:
    ...
    base = Path("/tmp") / f"phatmux_zdotdir_test_{os.getpid()}"
    try:
        if base.exists():
        ...
        (orig / ".zshenv").write_text(
            'echo "$ZDOTDIR" > "$PHATMUX_ZDOTDIR_TEST_OUTPUT"\n',
            encoding="utf-8",
        )

        env = dict(os.environ)
        env["ZDOTDIR"] = str(wrapper_dir)
        env["PHATMUX_ZSH_ZDOTDIR"] = str(orig)
        env["PHATMUX_ZDOTDIR_TEST_OUTPUT"] = str(seen_path)
