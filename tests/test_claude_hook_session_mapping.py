...
def run_claude_hook(
    ...
    if proc.returncode != 0:
        raise RuntimeError(
            f"phatmux claude-hook {subcommand} failed:\n"
            f"exit={proc.returncode}\nstdout={proc.stdout}\nstderr={proc.stderr}"
        )
    return proc.stdout.strip()


def wait_for_notification_count(client: phatmux, minimum: int, timeout: float = 4.0) -> list[dict]:
    start = time.time()
    items: list[dict] = []
...
def main() -> int:
    ...
    state_path = Path(tempfile.gettempdir()) / f"phatmux_claude_hook_state_{os.getpid()}.json"
    ...
    project_dir = Path(tempfile.gettempdir()) / f"phatmux_claude_map_project_{os.getpid()}"
    ...
    try:
        with phatmux() as client:
            ...
            hook_env["PHATMUX_SOCKET_PATH"] = client.socket_path
            hook_env["PHATMUX_WORKSPACE_ID"] = workspace_id
            hook_env["PHATMUX_SURFACE_ID"] = surface_id
            hook_env["PHATMUX_CLAUDE_HOOK_STATE_PATH"] = str(state_path)
    ...
    except (phatmuxError, RuntimeError) as exc:
        return fail(str(exc))
    finally:
        try:
            if state_path.exists():
