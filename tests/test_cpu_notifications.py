...
Requires phatmux to be running with socket control enabled.
"""
...
def get_phatmux_pid() -> Optional[int]:
    """Get the PID of the running phatmux process."""
    socket_path = os.environ.get("PHATMUX_SOCKET_PATH")
    if not socket_path:
        # Ask phatmux.py to resolve default socket path (supports PHATMUX_TAG and last-socket file).
        try:
            socket_path = phatmux().socket_path
        except Exception:
            socket_path = None
    ...
    result = subprocess.run(
        ["pgrep", "-f", r"phatmux\.app/Contents/MacOS/phatmux$"],
        capture_output=True,
        text=True,
    )
    if result.returncode != 0:
        # Try DEV build
        result = subprocess.run(
            ["pgrep", "-f", r"phatmux DEV\.app/Contents/MacOS/phatmux"],
            capture_output=True,
            text=True,
        )
    if result.returncode != 0:
...
def test_cpu_after_notification_burst(client: phatmux, pid: int) -> tuple[bool, str]:
    """
...
def test_cpu_after_popover_close(client: phatmux, pid: int) -> tuple[bool, str]:
    """
    ...
    # Ensure the correct phatmux instance is frontmost (tag-safe).
    bundle_id = phatmux.default_bundle_id()
    subprocess.run(
...
def test_cpu_idle_with_notifications(client: phatmux, pid: int) -> tuple[bool, str]:
    """
...
def main():
    print("=" * 60)
    print("phatmux Notification CPU Tests")
    print("=" * 60)
    ...
    pid = get_phatmux_pid()
    if pid is None:
        print("\n❌ SKIP: phatmux is not running")
        return 0

    print(f"\nFound phatmux process: PID {pid}")

    # Try to connect to the socket
    client = phatmux(socket_path)
    try:
        client.connect()
        print(f"Connected to {socket_path}")
    except phatmuxError:
        print("\n❌ SKIP: Could not connect to phatmux socket")
        print("Tip: set PHATMUX_TAG=<tag> or PHATMUX_SOCKET_PATH=<path> to target a tagged instance.")
        return 0
