...
Why this matters for phatmux:
- phatmux sets ZDOTDIR to the app wrapper directory so zsh loads wrapper
  startup files.
...
def main() -> int:
    ...
    base = Path("/tmp") / f"phatmux_zdotdir_user_override_{os.getpid()}"
    try:
