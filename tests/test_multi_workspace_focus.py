...
def _marker(name: str) -> Path:
    return MARKER_DIR / f"phatmux_mwf_{name}_{os.getpid()}"
...
def _verify_responsive(c: phatmux, marker: Path, surface_idx: int, retries: int = 3) -> bool:
...
def _wait_terminal_in_window(c: phatmux, surface_idx: int, timeout: float = 5.0) -> bool:
    start = time.time()
    while time.time() - start < timeout:
        try:
...
def test_multi_workspace_terminal_responsive(c: phatmux) -> None:
    """
...
def test_three_workspaces_non_last_responsive(c: phatmux) -> None:
    """
...
def test_rapid_workspace_switching_preserves_focus(c: phatmux) -> None:
    """
...
def test_browser_panel_focus_and_return(c: phatmux) -> None:
    """
    ...
    if not surfaces:
        raise phatmuxError("No surfaces after new_workspace")
...
def test_browser_focus_across_workspaces(c: phatmux) -> None:
    """
    ...
    with phatmux() as c:
        ...
        for name, test_fn in tests:
            ...
            try:
                ...
                print("PASS")
                passed += 1
            except (AssertionError, phatmuxError) as e:
                print(f"FAIL: {e}")
                failed += 1
