...
In phatmuxOnly mode (default), only processes descended from the phatmux
app process can connect. External processes (e.g., SSH) are rejected.

Test strategy:
  Phase 1: phatmuxOnly — external processes get rejected
  Phase 2: phatmuxOnly — internal process CAN connect (inject via shell rc)
...
def _preferred_worktree_slug():
    env_slug = os.environ.get("PHATMUX_TAG") or os.environ.get("PHATMUX_BRANCH_SLUG")
    if env_slug:
...
def _derived_app_candidates_for_current_worktree():
    ...
    for info_path in info_paths:
        ...
        app_path = os.path.join(derived_root, "Build/Products/Debug/phatmux DEV.app")
...
def _find_app():
    explicit = os.environ.get("PHATMUX_APP_PATH")
    ...
    if preferred_slug:
        preferred_tmp = []
        preferred_tmp.extend(glob.glob(f"/tmp/phatmux-{preferred_slug}/Build/Products/Debug/phatmux DEV*.app"))
        preferred_tmp.extend(glob.glob(f"/private/tmp/phatmux-{preferred_slug}/Build/Products/Debug/phatmux DEV*.app"))
    ...
    derived_candidates = glob.glob(os.path.join(
        home, "Library/Developer/Xcode/DerivedData/*/Build/Products/Debug/phatmux DEV.app"
    ))
    tmp_candidates = []
    tmp_candidates.extend(glob.glob("/tmp/phatmux-*/Build/Products/Debug/phatmux DEV*.app"))
    tmp_candidates.extend(glob.glob("/private/tmp/phatmux-*/Build/Products/Debug/phatmux DEV*.app"))
...
def _find_cli(preferred_app_path: str = ""):
    explicit = os.environ.get("PHATMUX_CLI_BIN") or os.environ.get("PHATMUX_CLI")
    if explicit and os.path.exists(explicit) and os.access(explicit, os.X_OK):
        return explicit

    if preferred_app_path:
        debug_dir = os.path.dirname(preferred_app_path)
        sibling = os.path.join(debug_dir, "phatmux")
    ...
    candidates.extend(glob.glob(os.path.join(
        home, "Library/Developer/Xcode/DerivedData/*/Build/Products/Debug/phatmux"
    )))
    candidates.extend(glob.glob("/tmp/phatmux-*/Build/Products/Debug/phatmux"))
    candidates.extend(glob.glob("/private/tmp/phatmux-*/Build/Products/Debug/phatmux"))
...
def _kill_phatmux(app_path: str = None):
    if app_path:
        exe = os.path.join(app_path, "Contents/MacOS/phatmux DEV")
        subprocess.run(["pkill", "-f", exe], capture_output=True)
    else:
        subprocess.run(["pkill", "-x", "phatmux DEV"], capture_output=True)
    time.sleep(1.5)


def _launch_phatmux(app_path: str, socket_path: str, mode: str = None, extra_env: dict = None):
    if os.path.exists(socket_path):
        try:
    ...
    env_args = []
    if mode:
        env_args = ["--env", f"PHATMUX_SOCKET_MODE={mode}"]
    launch_env = {
        "PHATMUX_SOCKET_PATH": socket_path,
...
def test_internal_process_allowed(socket_path: str, app_path: str) -> TestResult:
    """
    Verify a phatmux-spawned terminal process CAN connect in phatmuxOnly mode.
    Inject a test via the shell rc file, then launch phatmux in phatmuxOnly mode.
    The shell (a descendant of phatmux) runs the test on startup.
    """
    result = TestResult("Internal process can connect (phatmuxOnly)")
    marker = os.path.join(tempfile.gettempdir(), f"phatmux_internal_{os.getpid()}")
    hook_file = os.path.join(tempfile.gettempdir(), f"phatmux_rc_hook_{os.getpid()}.sh")
...
""")
        ...
        # Kill existing phatmux, launch in phatmuxOnly mode (default)
        _kill_phatmux(app_path)
        _launch_phatmux(app_path, socket_path, mode="phatmuxOnly")
        ...
        if content == "OK":
            result.success("Internal process pinged socket successfully in phatmuxOnly mode")
        else:
...
def test_allowall_mode_works(socket_path: str, app_path: str) -> TestResult:
    """Verify PHATMUX_SOCKET_MODE=allowAll bypasses ancestry check."""
    result = TestResult("allowAll mode allows external")
    try:
        _kill_phatmux(app_path)
        _launch_phatmux(app_path, socket_path, mode="allowAll")
...
def test_password_mode_requires_auth(socket_path: str, app_path: str) -> TestResult:
    ...
    password = f"phatmux-pass-{os.getpid()}"
    try:
        _kill_phatmux(app_path)
        _launch_phatmux(
            app_path,
            socket_path,
            mode="password",
            extra_env={"PHATMUX_SOCKET_PASSWORD": password}
        )
...
def test_password_mode_cli_exit_code(socket_path: str, app_path: str) -> TestResult:
    ...
    try:
        ...
        if not cli_path:
            result.failure("Could not find phatmux CLI binary")
            return result
...
def run_tests():
    print("=" * 60)
    print("phatmux Socket Access Control Tests")
    print("=" * 60)
    print()

    app_path = _find_app()
    if not app_path:
        print("Error: Could not find phatmux DEV.app in DerivedData")
        return 1
    print(f"App: {app_path}")

    socket_path = f"/tmp/phatmux-test-socket-access-{os.getpid()}.sock"
    try:
        os.unlink(socket_path)
    except OSError:
        pass
    ...
    # ── Phase 1: phatmuxOnly — external rejection ──
    print("Phase 1: phatmuxOnly mode — external rejection")
    print("-" * 50)

    # Ensure phatmux is running in phatmuxOnly mode
    _kill_phatmux(app_path)
    print("  Launching phatmux in phatmuxOnly mode...")
    _launch_phatmux(app_path, socket_path, mode="phatmuxOnly")
    ...
    # ── Phase 2: phatmuxOnly — internal process CAN connect ──
    print("Phase 2: phatmuxOnly mode — internal process allowed")
    print("-" * 50)
    ...
    # ── Cleanup: leave phatmux in phatmuxOnly mode ──
    _kill_phatmux(app_path)
    _launch_phatmux(app_path, socket_path, mode="phatmuxOnly")
