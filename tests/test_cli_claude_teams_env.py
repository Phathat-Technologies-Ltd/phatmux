#!/usr/bin/env python3
"""
Regression test: `phatmux claude-teams` injects the tmux-style auto-mode env.
"""
...
from claude_teams_test_utils import resolve_phatmux_cli
...
def main() -> int:
    try:
        cli_path = resolve_phatmux_cli()
    except Exception as exc:
        print(f"FAIL: {exc}")
        return 1

    with tempfile.TemporaryDirectory(prefix="phatmux-claude-teams-env-") as td:
        tmp = Path(td)
        ...
        phatmux_bin_log = tmp / "phatmux-bin.log"
...
printf '%s\\n' "${PHATMUX_CLAUDE_TEAMS_CLI_BIN-__UNSET__}" > "$FAKE_PHATMUX_BIN_LOG"
printf '%s\\n' "$@" > "$FAKE_ARGV_LOG"
...
printf '%s\\n' "${PHATMUX_SOCKET_PATH-__UNSET__}" > "$FAKE_SOCKET_PATH_LOG"
printf '%s\\n' "${PHATMUX_SOCKET_PASSWORD-__UNSET__}" > "$FAKE_SOCKET_PASSWORD_LOG"
""",
        )
        ...
        env["FAKE_PHATMUX_BIN_LOG"] = str(phatmux_bin_log)
        ...
        explicit_socket_path = str(tmp / "explicit-phatmux.sock")
        ...
        if proc.returncode != 0:
            print("FAIL: `phatmux claude-teams --version` exited non-zero")
        ...
        if tmux_path.startswith(str(real_bin)):
            print(f"FAIL: expected phatmux tmux shim to shadow PATH, got {tmux_path!r}")
            return 1

        phatmux_bin_value = read_text(phatmux_bin_log)
        if not phatmux_bin_value or phatmux_bin_value == "__UNSET__":
            print("FAIL: missing PHATMUX_CLAUDE_TEAMS_CLI_BIN")
            return 1

        if not os.path.exists(phatmux_bin_value):
            print(f"FAIL: PHATMUX_CLAUDE_TEAMS_CLI_BIN does not exist: {phatmux_bin_value!r}")
            return 1
        ...
        if socket_path_value != explicit_socket_path:
            print(f"FAIL: expected PHATMUX_SOCKET_PATH={explicit_socket_path!r}, got {socket_path_value!r}")
            return 1

        socket_password_value = read_text(socket_password_log)
        if socket_password_value != explicit_socket_password:
            print(
                "FAIL: expected PHATMUX_SOCKET_PASSWORD to preserve the explicit CLI override, "
                f"got {socket_password_value!r}"
            )
            return 1

    print("PASS: phatmux claude-teams injects the auto-mode tmux env and shim")
    return 0
