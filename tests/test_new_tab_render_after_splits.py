...
def _wait_for_terminal_focus(c: phatmux, panel_id: str, timeout_s: float = 6.0) -> bool:
    start = time.time()
...
def _panel_snapshot_retry(c: phatmux, panel_id: str, label: str, timeout_s: float = 3.0) -> dict:
    start = time.time()
    ...
    while time.time() - start < timeout_s:
        try:
            ...
            if "Failed to capture panel image" not in str(e):
                raise
            time.sleep(0.05)
    raise phatmuxError(f"Timed out waiting for panel_snapshot: panel_id={panel_id} label={label}: {last_err!r}")
...
def main() -> int:
    with phatmux(SOCKET_PATH) as c:
        ...
        draw_cmd = "for i in {1..40}; do echo PHATMUX_DRAW_$i; done"
        ...
        if w1 <= 0 or h1 <= 0 or (w1, h1) != (w2, h2):
            raise phatmuxError(f"panel_snapshot dims differ: {(w1,h1)} {(w2,h2)}; paths: {s1.get('path')} {s2.get('path')}")
