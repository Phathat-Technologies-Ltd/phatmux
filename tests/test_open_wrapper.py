...
def run_wrapper(
    *,
    ...
    with tempfile.TemporaryDirectory(prefix="phatmux-open-wrapper-test-") as td:
        tmp = Path(td)
        wrapper = tmp / "open"
...
if [[ -n "${FAKE_PHATMUX_FAIL_URLS:-}" ]]; then
  IFS=',' read -r -a failures <<< "$FAKE_PHATMUX_FAIL_URLS"
...
""",
        ...
        env = os.environ.copy()
        env["PHATMUX_SOCKET_PATH"] = "/tmp/phatmux-open-wrapper-test.sock"
        env["PHATMUX_BUNDLE_ID"] = "com.phatmux.app.debug.test"
        env["PHATMUX_OPEN_WRAPPER_SYSTEM_OPEN"] = str(system_open)
        env["PHATMUX_OPEN_WRAPPER_DEFAULTS"] = str(defaults)
        env["FAKE_OPEN_LOG"] = str(open_log)
        env["FAKE_PHATMUX_LOG"] = str(phatmux_log)
        if python_bin is None:
            env.pop("PHATMUX_OPEN_WRAPPER_PYTHON3", None)
        else:
            env["PHATMUX_OPEN_WRAPPER_PYTHON3"] = python_bin
        ...
        if fail_urls:
            env["FAKE_PHATMUX_FAIL_URLS"] = ",".join(fail_urls)
        else:
            env.pop("FAKE_PHATMUX_FAIL_URLS", None)
        ...
        return read_log(open_log), read_log(phatmux_log), result.returncode, result.stderr.strip()
...
def test_toggle_disabled_passthrough(failures: list[str]) -> None:
    ...
    open_log, phatmux_log, code, stderr = run_wrapper(
        args=[url],
        intercept_setting="0",
        whitelist="",
    )
    expect(code == 0, f"toggle off: wrapper exited {code}: {stderr}", failures)
    expect(phatmux_log == [], f"toggle off: phatmux should not be called, got {phatmux_log}", failures)
...
def test_toggle_disabled_case_insensitive_passthrough(failures: list[str]) -> None:
    ...
    expect(
        phatmux_log == [],
        f"toggle off (case-insensitive): phatmux should not be called, got {phatmux_log}",
        failures,
    )
    expect(
        open_log == [url],
...
def test_whitelist_miss_passthrough(failures: list[str]) -> None:
    ...
    expect(phatmux_log == [], f"whitelist miss: phatmux should not be called, got {phatmux_log}", failures)
    expect(open_log == [url], f"whitelist miss: expected system open [{url}], got {open_log}", failures)


def test_whitelist_match_routes_to_phatmux(failures: list[str]) -> None:
    ...
    expect(phatmux_log == [f"browser open {url}"], f"whitelist match: unexpected phatmux log {phatmux_log}", failures)
...
def test_external_literal_pattern_is_deferred_to_app(failures: list[str]) -> None:
    ...
    expect(
        phatmux_log == [f"browser open {url}"],
        f"external literal deferred: expected wrapper to pass URL to phatmux, got {phatmux_log}",
        failures,
    )
    expect(
        open_log == [],
...
def test_external_regex_pattern_is_deferred_to_app(failures: list[str]) -> None:
    ...
    expect(
        phatmux_log == [f"browser open {url}"],
        f"external regex deferred: expected wrapper to pass URL to phatmux, got {phatmux_log}",
        failures,
    )
    expect(
        open_log == [],
...
def test_external_regex_with_icu_features_is_deferred_to_app(failures: list[str]) -> None:
    ...
    expect(
        phatmux_log == [f"browser open {url}"],
        f"external regex icu deferred: expected wrapper to pass URL to phatmux, got {phatmux_log}",
        failures,
    )
    expect(
        open_log == [],
...
def test_external_invalid_regex_is_ignored_silently(failures: list[str]) -> None:
    ...
    expect(
        phatmux_log == [f"browser open {url}"],
        f"external invalid regex: expected phatmux open for {url}, got {phatmux_log}",
        failures,
    )
    expect(
        open_log == [],
...
def test_partial_failures_only_fallback_failed_urls(failures: list[str]) -> None:
    ...
    expect(
        phatmux_log == [f"browser open {good}", f"browser open {failed}"],
        f"partial failure: phatmux log mismatch {phatmux_log}",
        failures,
    )
    expect(
...
def test_legacy_toggle_fallback_passthrough(failures: list[str]) -> None:
    ...
    expect(phatmux_log == [], f"legacy fallback: phatmux should not be called, got {phatmux_log}", failures)
...
def test_legacy_toggle_fallback_case_insensitive_passthrough(failures: list[str]) -> None:
    ...
    expect(
        phatmux_log == [],
        f"legacy fallback (case-insensitive): phatmux should not be called, got {phatmux_log}",
        failures,
    )
    expect(
        open_log == [url],
...
def test_uppercase_scheme_routes_to_phatmux(failures: list[str]) -> None:
    url = "HTTPS://api.example.com/path?q=1"
    open_log, phatmux_log, code, stderr = run_wrapper(
        args=[url],
        intercept_setting="1",
    ...
    expect(phatmux_log == [f"browser open {url}"], f"uppercase scheme: unexpected phatmux log {phatmux_log}", failures)


def test_local_html_file_routes_to_phatmux(failures: list[str]) -> None:
    ...
    expect(len(phatmux_log) == 1, f"local html file: expected exactly one phatmux call, got {phatmux_log}", failures)
    if phatmux_log:
        expect(
            phatmux_log[0].startswith("browser open file://"),
            f"local html file: expected file:// target, got {phatmux_log[0]}",
            failures,
        )
        expect(
            "hello%20page.HTML" in phatmux_log[0],
            f"local html file: expected URL-encoded filename in phatmux target, got {phatmux_log[0]}",
            failures,
        )


def test_file_url_html_routes_to_phatmux(failures: list[str]) -> None:
    url = "file:///tmp/phatmux-open-wrapper-fixture.html"
    open_log, phatmux_log, code, stderr = run_wrapper(
        args=[url],
        intercept_setting="1",
        whitelist="",
    ...
    expect(phatmux_log == [f"browser open {url}"], f"file url html: unexpected phatmux log {phatmux_log}", failures)


def test_file_url_html_routes_to_phatmux_without_python_binary(failures: list[str]) -> None:
    url = "file:///tmp/phatmux-open-wrapper-fixture.html"
    open_log, phatmux_log, code, stderr = run_wrapper(
        args=[url],
        intercept_setting="1",
    ...
    expect(
        phatmux_log == [f"browser open {url}"],
        f"file url html no-python fallback: unexpected phatmux log {phatmux_log}",
        failures,
    )


def test_local_html_file_routes_to_phatmux_without_python_binary(failures: list[str]) -> None:
    ...
    expect(
        len(phatmux_log) == 1,
        f"local html no-python fallback: expected exactly one phatmux call, got {phatmux_log}",
        failures,
    )
    if phatmux_log:
        expect(
            phatmux_log[0].startswith("browser open file://"),
            f"local html no-python fallback: expected file:// target, got {phatmux_log[0]}",
            failures,
        )
        expect(
            "no%20python%20fallback.html" in phatmux_log[0],
            f"local html no-python fallback: expected URL-encoded filename, got {phatmux_log[0]}",
            failures,
        )
...
def test_domain_like_html_argument_passthrough(failures: list[str]) -> None:
    ...
    expect(
        phatmux_log == [],
        f"domain-like html argument: phatmux should not be called, got {phatmux_log}",
        failures,
    )
    expect(
        open_log == [arg],
...
def test_non_file_scheme_html_passthrough(failures: list[str]) -> None:
    ...
    expect(phatmux_log == [], f"non-file scheme html: phatmux should not be called, got {phatmux_log}", failures)
...
def test_mailto_html_passthrough(failures: list[str]) -> None:
    ...
    expect(phatmux_log == [], f"mailto html: phatmux should not be called, got {phatmux_log}", failures)
...
def test_local_non_html_file_passthrough(failures: list[str]) -> None:
    ...
    expect(phatmux_log == [], f"local non-html file: phatmux should not be called, got {phatmux_log}", failures)
...
def test_unicode_whitelist_matches_punycode_url(failures: list[str]) -> None:
    ...
    expect(phatmux_log == [f"browser open {url}"], f"unicode whitelist: unexpected phatmux log {phatmux_log}", failures)
...
def test_punycode_whitelist_matches_unicode_url(failures: list[str]) -> None:
    ...
    expect(phatmux_log == [f"browser open {url}"], f"punycode whitelist: unexpected phatmux log {phatmux_log}", failures)
...
def main() -> int:
    ...
    test_whitelist_match_routes_to_phatmux(failures)
    ...
    test_uppercase_scheme_routes_to_phatmux(failures)
    test_local_html_file_routes_to_phatmux(failures)
    test_file_url_html_routes_to_phatmux(failures)
    test_file_url_html_routes_to_phatmux_without_python_binary(failures)
    test_local_html_file_routes_to_phatmux_without_python_binary(failures)
