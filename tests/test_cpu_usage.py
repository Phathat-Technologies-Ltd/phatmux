#!/usr/bin/env python3
"""
CPU usage test for phatmux.

This test monitors phatmux's CPU usage during idle periods to catch
performance regressions like runaway animations or continuous view updates.

Run this test after launching phatmux:
...
# Allow importing tests/phatmux.py when running from repo root.
...
def main():
    print("=" * 60)
    print("phatmux CPU Usage Test")
    print("=" * 60)

    # Find phatmux process
    pid = get_phatmux_pid()
    if pid is None:
        print("\n❌ SKIP: phatmux is not running")
        print("Start phatmux and run this test again.")
    ...
    if avg_cpu > MAX_IDLE_CPU_PERCENT:
        ...
        sample_file = Path(f"/tmp/phatmux_cpu_test_sample_{pid}.txt")
        ...
        relevant_lines = [
            l for l in lines
            if "phatmux" in l and ("body" in l or "Animation" in l or "Timer" in l)
        ][:10]
