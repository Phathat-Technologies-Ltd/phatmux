...
phatmux also injects its own zsh integration by setting ZDOTDIR to
Resources/shell-integration. If phatmux incorrectly treats Ghostty's injected
...
def main() -> int:
    ...
    phatmux_wrapper_dir = root / "Resources" / "shell-integration"
    ghostty_zsh_dir = root / "ghostty" / "src" / "shell-integration" / "zsh"

    if not (phatmux_wrapper_dir / ".zshenv").exists():
        print(f"SKIP: missing phatmux wrapper .zshenv at {phatmux_wrapper_dir}")
        return 0
    if not (ghostty_zsh_dir / ".zshenv").exists():
        print(f"SKIP: missing Ghostty zsh .zshenv at {ghostty_zsh_dir}")
        return 0

    base = Path("/tmp") / f"phatmux_histfile_ghostty_stack_{os.getpid()}"
    try:
        ...
        # Simulate the buggy situation: phatmux stores Ghostty's injected ZDOTDIR
        # as the "original" ZDOTDIR, then sets ZDOTDIR to its own wrapper.
        env["PHATMUX_ORIGINAL_ZDOTDIR"] = str(ghostty_zsh_dir)
        env["ZDOTDIR"] = str(phatmux_wrapper_dir)
        env["PHATMUX_SHELL_INTEGRATION"] = "0"
        ...
        if seen != expected:
            ...
            print(f"  phatmux_wrapper_dir={phatmux_wrapper_dir}")
