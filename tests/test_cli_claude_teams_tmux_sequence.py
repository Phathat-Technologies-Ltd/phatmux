#!/usr/bin/env python3
"""
Regression test: `phatmux claude-teams` supports Claude's tmux teammate flow.
"""
    ...
    def handle(self, method: str, params: dict[str, object]) -> dict[str, object]:
        with self.lock:
            ...
            if method == "surface.send_text":
                return {"ok": True}
            raise RuntimeError(f"Unsupported fake phatmux method: {method}")
...
def main() -> int:
    ...
    with tempfile.TemporaryDirectory(prefix="phatmux-claude-teams-seq-") as td:
        tmp = Path(td)
        home = tmp / "home"
        home.mkdir(parents=True, exist_ok=True)

        socket_path = tmp / "fake-phatmux.sock"
        state = FakeCmuxState()
...
printf '%s\\n' "${PHATMUX_SOCKET_PATH-__UNSET__}" > "$FAKE_SOCKET_LOG"
...
""",
        ...
        try:
            ...
            print("FAIL: `phatmux claude-teams --version` timed out")
        ...
        if socket_value != str(socket_path):
            print(f"FAIL: expected PHATMUX_SOCKET_PATH={socket_path}, got {socket_value!r}")
            return 1

        window_target = read_text(window_target_log)
        if window_target != "phatmux:1":
            print(f"FAIL: expected tmux window target 'phatmux:1', got {window_target!r}")
            return 1
    ...
    print("PASS: phatmux claude-teams supports Claude's tmux teammate flow")
    return 0
