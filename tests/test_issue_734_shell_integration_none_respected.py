...
phatmux wrapper .zshenv should only source Ghostty zsh integration when Ghostty
...
def _run_case(
    *,
    ...
    env["PHATMUX_TEST_OUT"] = str(out_path)

    # Keep input deterministic and local to this test.
    for key in (
        "GHOSTTY_ZSH_ZDOTDIR",
        "PHATMUX_ZSH_ZDOTDIR",
        "PHATMUX_ORIGINAL_ZDOTDIR",
    ...
    if ghostty_enabled:
        ...
        env["PHATMUX_ZSH_ZDOTDIR"] = str(orig_zdotdir)
...
def main() -> int:
    ...
    base = Path("/tmp") / f"phatmux_issue_734_{os.getpid()}"
    try:
