#!/usr/bin/env python3
"""
phatmux Python Client

A client library for programmatically controlling phatmux via Unix socket.
...
_DEFAULT_DEBUG_BUNDLE_ID = "com.phatmux.app.debug"
...
def _default_bundle_id() -> str:
    override = os.environ.get("PHATMUX_BUNDLE_ID")
    if override:
        return override

    tag = os.environ.get("PHATMUX_TAG")
    if tag:
...
def _default_socket_path() -> str:
    ...
    if tag:
        ...
        tagged_candidates = [
            f"/tmp/phatmux-debug-{slug}.sock",
            f"/tmp/phatmux-{slug}.sock",
        ]
    ...
    override = os.environ.get("PHATMUX_SOCKET_PATH")
    if override:
    ...
    tagged = glob.glob("/tmp/phatmux-debug-*.sock")
    tagged.extend(glob.glob(os.path.join(_APP_SUPPORT_DIR, "phatmux*.sock")))
...
class phatmux:
    """Client for controlling phatmux via Unix socket"""
    ...
    def _send_command(self, command: str) -> str:
        ...
        try:
            ...
            while True:
                ...
                try:
                    ...
                    if time.time() - start >= 5.0:
                        raise phatmuxError("Command timed out")
                    continue
                if not chunk:
                    break
                data += chunk.decode()
            ...
            raise phatmuxError(f"Socket error: {e}")
    ...
    def seed_drag_pasteboard_types(self, types: List[str]) -> None:
        ...
        if not types:
            raise phatmuxError("seed_drag_pasteboard_types requires at least one type")
        payload = ",".join(t.strip() for t in types if t and t.strip())
        if not payload:
            raise phatmuxError("seed_drag_pasteboard_types requires at least one non-empty type")
    ...
    def layout_debug(self) -> dict:
        ...
        try:
            ...
            raise phatmuxError(f"layout_debug JSON decode failed: {e}: {payload[:200]}")
    ...
    def render_stats(self, panel: Union[str, int, None] = None) -> dict:
        ...
        try:
            ...
            raise phatmuxError(f"render_stats JSON decode failed: {e}: {payload[:200]}")
    ...
    def panel_snapshot(self, panel: Union[str, int], label: str = "") -> dict:
        ...
        if len(parts) != 5:
            raise phatmuxError(f"panel_snapshot parse failed: {response}")
...
def main():
    """CLI interface for phatmux"""
    import sys
    import argparse

    parser = argparse.ArgumentParser(description="phatmux CLI")
    ...
    try:
        with phatmux(args.socket) as client:
            if not args.command:
                # Interactive mode
                print("phatmux CLI (type 'help' for commands, 'quit' to exit)")
                while True:
                    try:
                        line = input("> ").strip()
