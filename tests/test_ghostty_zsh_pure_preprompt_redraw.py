...
typeset -g PHATMUX_TOP='%F{4}%~%f'
typeset -g PHATMUX_LAST_PROMPT=''
typeset -gi PHATMUX_ASYNC_DONE=0
typeset -g PHATMUX_ASYNC_FD=''

phatmux_render_prompt() {
  local cleaned_ps1=$PROMPT
  if [[ $PROMPT = *$prompt_newline* ]]; then
    cleaned_ps1=${PROMPT##*${prompt_newline}}
  fi

  PROMPT="${PHATMUX_TOP}${prompt_newline}${cleaned_ps1:-%F{5}❯%f }"

  local expanded_prompt="${(S%%)PROMPT}"
  if [[ ${1:-} == precmd ]]; then
    print
  elif [[ $PHATMUX_LAST_PROMPT != $expanded_prompt ]]; then
    zle && zle .reset-prompt
  fi
  typeset -g PHATMUX_LAST_PROMPT=$expanded_prompt
}

phatmux_async_ready() {
  emulate -L zsh
  local fd="${1:-$PHATMUX_ASYNC_FD}"
  if [[ -n $fd ]]; then
    zle -F "$fd"
    exec {fd}<&-
  fi
  ...
  (( PHATMUX_ASYNC_DONE )) && return
  PHATMUX_ASYNC_DONE=1
  PHATMUX_TOP='%F{4}%~%f %F{242}main%f%F{218}*%f %F{6}⇣⇡%f'
  phatmux_render_prompt async
}

precmd() {
  PHATMUX_ASYNC_DONE=0
  phatmux_render_prompt precmd
}

phatmux_line_init() {
  if (( !PHATMUX_ASYNC_DONE )) && [[ -z $PHATMUX_ASYNC_FD ]]; then
    exec {PHATMUX_ASYNC_FD}< <(
      sleep 0.05
      printf 'ready\n'
    )
    zle -F "$PHATMUX_ASYNC_FD" phatmux_async_ready
  fi
}

zle -N zle-line-init phatmux_line_init
PROMPT='%F{5}❯%f '
""".lstrip()
...
def _capture_session(
    *,
    ...
    base = Path(tempfile.mkdtemp(prefix="phatmux_ghostty_pure_preprompt_"))
    try:
        home = base / "home"
