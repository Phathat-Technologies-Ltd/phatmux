#!/usr/bin/env python3
"""
Regression: the phatmux zsh wrapper should prefer a bundled Ghostty zsh
integration file in PHATMUX_SHELL_INTEGRATION_DIR over the fallback integration
under GHOSTTY_RESOURCES_DIR.

Without this, tagged phatmux builds can silently load Ghostty's installed app
...
def main() -> int:
    ...
    base = Path("/tmp") / f"phatmux_bundled_ghostty_zsh_{os.getpid()}"
    try:
        ...
        env["PHATMUX_SHELL_INTEGRATION_DIR"] = str(bundled)
        env["GHOSTTY_RESOURCES_DIR"] = str(fallback)
        env["PHATMUX_LOAD_GHOSTTY_ZSH_INTEGRATION"] = "1"
        env["PHATMUX_SHELL_INTEGRATION"] = "0"
        env["PHATMUX_TEST_OUT"] = str(marker)
