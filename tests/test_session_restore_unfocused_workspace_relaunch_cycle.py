...
def _launch(app_path: Path, socket_path: Path) -> None:
    ...
    subprocess.run(
        [
            ...
            str(app_path),
            "--env",
            f"PHATMUX_SOCKET_PATH={socket_path}",
            "--env",
...
def main() -> int:
    ...
    socket_path = Path(f"/tmp/phatmux-session-restore-cycle-{bundle_id.replace('.', '-')}.sock")

    markers = [f"PHATMUX_RESTORE_EDGE_{i}" for i in range(3)]
    failures: list[str] = []
