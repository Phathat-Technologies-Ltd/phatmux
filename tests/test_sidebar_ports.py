...
Run with a tagged instance to avoid unix socket conflicts:
  PHATMUX_TAG=<tag> python3 tests/test_sidebar_ports.py
"""
...
from phatmux import phatmux, phatmuxError  # noqa: E402
...
def _start_external_server(base: Path, port: int) -> subprocess.Popen:
    """
    Start an http.server outside phatmux and ensure it is actually listening.
...
def _wait_for_port(client: phatmux, port: int, timeout: float = 18.0) -> dict[str, str]:
    def pred():
...
def _wait_for_port_absent(client: phatmux, port: int, timeout: float = 18.0) -> dict[str, str]:
    def pred():
        state = _parse_sidebar_state(client.sidebar_state())
        raw = state.get("ports", "")
        if raw == "none" or not raw:
...
def _assert_port_absent_for_duration(client: phatmux, port: int, duration: float = 6.0, interval: float = 0.15) -> None:
    """
...
def main() -> int:
    tag = os.environ.get("PHATMUX_TAG") or ""
    if not tag:
        print("Tip: set PHATMUX_TAG=<tag> when running this test to avoid socket conflicts.")

    base = Path("/tmp") / f"phatmux_ports_test_{os.getpid()}"
    ...
    try:
        ...
        # Start a listening server outside phatmux. A fresh tab should NOT show this port,
        ...
        with phatmux() as client:
            ...
            client.send("echo phatmux_ports_test\n")
    ...
    except (phatmuxError, AssertionError, RuntimeError, ValueError) as e:
