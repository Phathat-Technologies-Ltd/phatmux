...
This document tracks implemented behavior and remaining parity gaps for the phatmux browser port.

## Goals

1. Provide an LLM-friendly browser automation API in phatmux with stable handles.
...
1. `./scripts/run-tests-v1.sh` passes on `phatmux-vm`.
2. `./scripts/run-tests-v2.sh` passes on `phatmux-vm`.
...
## phatmux Target API (v2)

### Already Present in phatmux
...
6. `browser.tab.*` compatibility aliases mapped to phatmux surfaces
7. `browser.console.list`
8. `browser.errors.list`
9. `browser.highlight`
10. `browser.state.save|load` (browser state in phatmux context)
...
Primary form:
```bash
phatmux browser --surface <surface-id> <agent-browser-style-command...>
```

Shorthand:
```bash
phatmux browser <surface-id> <agent-browser-style-command...>
```

Agent discovery:
```bash
phatmux identify
phatmux capabilities
phatmux browser identify --surface <surface-id>   # wrapper over system.identify + browser fields
```

Flash:
```bash
phatmux trigger-flash [--workspace <id>] [--surface <id>]
```
...
- [x] Add `browser` command group in `CLI/phatmux.swift` that accepts agent-browser-style command grammar.
...
3. `src/protocol.test.ts` -> adapted browser command/shape validation in `tests_v2/test_browser_api_unsupported_matrix.py` and existing `CLI/phatmux.swift` command grammar checks.
...
5. `src/daemon.test.ts`, `src/stream-server.test.ts`, `test/serverless.test.ts`, `src/ios-manager.test.ts` -> out-of-scope for phatmux browser parity (different transport/runtime).

### Implemented phatmux Browser Suites
...
1. `ssh phatmux-vm 'cd /Users/phatmux/GhosttyTabs && ./scripts/run-tests-v2.sh'`
2. `ssh phatmux-vm 'cd /Users/phatmux/GhosttyTabs && ./scripts/run-tests-v1.sh'`

## Decision Log (Locked - February 12, 2026)

1. `phatmux browser tab ...` maps to browser `surface` tabs only (no separate workspace-level tab meaning inside `browser` namespace).
...
5. Output shape is phatmux-native overall, but `browser.snapshot` and selector `not_found` diagnostics intentionally mirror agent-browser semantics for agent usability.
...
2. Whether to expose protocol-only agent-browser actions in first public release of `phatmux browser` or gate them behind a second rollout phase.
