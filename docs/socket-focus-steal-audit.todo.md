...
## Goal
Ensure commands run through the phatmux Unix socket/CLI do not steal user focus from the current UI workflow.
...
- [x] Add explicit `rename-tab` command (defaults to `PHATMUX_TAB_ID` / `PHATMUX_SURFACE_ID` / `PHATMUX_WORKSPACE_ID` when flags omitted).
