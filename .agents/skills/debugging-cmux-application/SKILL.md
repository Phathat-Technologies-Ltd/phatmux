---
name: debugging-cmux-application
description: >-
  Explains how to debug the cmux macOS app (SwiftUI + AppKit + libghostty):
  ...
  Use when the user asks how to debug cmux, diagnose focus/input issues, trace
...
1. **Reproduce with an isolated app** — always `./scripts/reload.sh --tag <slug>`. Never rely on an untagged `cmux DEV.app` (shared socket, bundle ID, focus theft). See `AGENTS.md` "Local dev".
...
- **Typical paths**: untagged `/tmp/cmux-debug.log`; tagged `/tmp/cmux-debug-<tag>.log`.
...
2. Find PID: `pgrep -f "cmux DEV <tag>"` (adjust pattern to match the tagged app name).
3. Attach: `lldb -p <pid>`.
...
3. Xcode DerivedData for the **tagged** app — delete `~/Library/Developer/Xcode/DerivedData/cmux-<tag>` and reload.

**Sanity-check the binary** (optional):

```bash
strings "/path/to/cmux DEV <tag>.app/Contents/MacOS/cmux DEV <tag>" | grep "unique-marker"
```
...
Python socket tests need a **tagged** socket, e.g. `CMUX_SOCKET=/tmp/cmux-debug-<tag>.sock`. Do not drive tests against an untagged dev instance if another agent or the user already has one.
