...
APP_NAME="phatmux STAGING"
BUNDLE_ID="com.phatmux.app.staging"
BASE_APP_NAME="phatmux"
DERIVED_DATA=""
NAME_SET=0
BUNDLE_SET=0
DERIVED_SET=0
...
Release build with isolated "phatmux STAGING" identity. Runs side-by-side with
the production phatmux app.
...
if [[ -n "$TAG" ]]; then
  ...
  if [[ "$NAME_SET" -eq 0 ]]; then
    APP_NAME="phatmux STAGING ${TAG}"
  fi
  if [[ "$BUNDLE_SET" -eq 0 ]]; then
    BUNDLE_ID="com.phatmux.app.staging.${TAG_ID}"
  fi
  if [[ "$DERIVED_SET" -eq 0 ]]; then
    DERIVED_DATA="/tmp/phatmux-staging-${TAG_SLUG}"
  fi
fi
...
if [[ -f "$INFO_PLIST" ]]; then
  ...
  PHATMUXD_SOCKET="${APP_SUPPORT_DIR}/phatmuxd-${STAGING_SLUG}.sock"
  PHATMUX_SOCKET="/tmp/phatmux-${STAGING_SLUG}.sock"
...
"${OPEN_CLEAN_ENV[@]}" PHATMUX_SOCKET_PATH="$PHATMUX_SOCKET" PHATMUXD_UNIX_PATH="$PHATMUXD_SOCKET" open -g "$APP_PATH"
