...
Run this from a spare phatmux pane. The script creates a temporary workspace,
...
def _prompt_block(text: str) -> tuple[list[str], str]:
    ...
    for i in range(len(lines) - 1, -1, -1):
        if _is_prompt_line(lines[i]):
            ...
            break
    if prompt_idx == -1:
        raise phatmuxError(f"Could not find prompt line in surface text:\n{text}")
...
def _read_text(client: phatmux, workspace_id: str, surface_id: str) -> str:
...
def main() -> int:
    ...
    parser.add_argument(
        "--socket",
        default=os.environ.get("PHATMUX_SOCKET") or os.environ.get("PHATMUX_SOCKET_PATH") or "/tmp/phatmux-debug.sock",
    )
    ...
    with phatmux(args.socket) as client:
        ...
        if not original_workspace_id:
            raise phatmuxError(f"workspace.current returned no workspace_id: {current}")

        created = client._call("workspace.create", {}) or {}
        workspace_id = str(created.get("workspace_id") or "")
        if not workspace_id:
            raise phatmuxError(f"workspace.create returned no workspace_id: {created}")
        ...
        while True:
            try:
                ...
                if surface_id:
                    ...
                    probe_text = baseline
                    break
                raise phatmuxError("surface not ready yet")
            except Exception as exc:
                probe_text = str(exc)
                if time.time() - start > 10:
                    raise phatmuxError(f"Timed out waiting for readable terminal surface: {probe_text}")
                time.sleep(0.2)
