...
# Requires: source ~/.secrets/phatmux.env && export SPARKLE_PRIVATE_KEY
...
ENTITLEMENTS="phatmux.entitlements"
APP_PATH="build/Build/Products/Release/phatmux.app"

# --- Pre-flight ---
source ~/.secrets/phatmux.env
...
echo "Building app..."
rm -rf build/
xcodebuild -scheme phatmux -configuration Release -derivedDataPath build CODE_SIGNING_ALLOWED=NO build 2>&1 | tail -5
...
# --- Notarize app ---
echo "Notarizing app..."
ditto -c -k --sequesterRsrc --keepParent "$APP_PATH" phatmux-notary.zip
xcrun notarytool submit phatmux-notary.zip \
  --apple-id "$APPLE_ID" --team-id "$APPLE_TEAM_ID" --password "$APPLE_APP_SPECIFIC_PASSWORD" --wait
xcrun stapler staple "$APP_PATH"
xcrun stapler validate "$APP_PATH"
rm -f phatmux-notary.zip
echo "App notarized"

# --- Create and notarize DMG ---
echo "Creating DMG..."
rm -f phatmux-macos.dmg
create-dmg --codesign "$SIGN_HASH" phatmux-macos.dmg "$APP_PATH"
echo "Notarizing DMG..."
xcrun notarytool submit phatmux-macos.dmg \
  --apple-id "$APPLE_ID" --team-id "$APPLE_TEAM_ID" --password "$APPLE_APP_SPECIFIC_PASSWORD" --wait
xcrun stapler staple phatmux-macos.dmg
xcrun stapler validate phatmux-macos.dmg
echo "DMG notarized"

# --- Generate Sparkle appcast ---
echo "Generating appcast..."
./scripts/sparkle_generate_appcast.sh phatmux-macos.dmg "$TAG" appcast.xml
...
if gh release view "$TAG" >/dev/null 2>&1; then
  ...
  for asset in phatmux-macos.dmg appcast.xml; do
  ...
  if [[ "$ALLOW_OVERWRITE" == "true" ]]; then
    ...
    gh release upload "$TAG" phatmux-macos.dmg appcast.xml --clobber
  else
    echo "Uploading to existing release $TAG..."
    gh release upload "$TAG" phatmux-macos.dmg appcast.xml
  fi
else
  echo "Creating release $TAG and uploading..."
  gh release create "$TAG" phatmux-macos.dmg appcast.xml --title "$TAG" --notes "See CHANGELOG.md for details"
fi
...
if [[ "$TAG" != *"-nightly"* ]]; then
  VERSION="${TAG#v}"
  DMG_SHA256=$(shasum -a 256 phatmux-macos.dmg | cut -d' ' -f1)
  echo "Updating homebrew cask to $VERSION (SHA: $DMG_SHA256)..."
  CASK_FILE="homebrew-phatmux/Casks/phatmux.rb"
  if [ -f "$CASK_FILE" ]; then
...
cask "phatmux" do
  ...
  zap trash: [
    ...
    "~/Library/Preferences/ai.phatmux.plist",
  ]
end
CASKEOF
    cd homebrew-phatmux
    git add Casks/phatmux.rb
    if git diff --staged --quiet; then
      echo "Homebrew cask already up to date"
    else
      git commit -m "Update phatmux to ${VERSION}"
      git push
      echo "Homebrew cask updated"
    fi
    cd ..
  else
    echo "WARNING: homebrew-phatmux submodule not found, skipping cask update"
  fi
fi

# --- Cleanup ---
rm -rf build/ phatmux-macos.dmg appcast.xml
echo ""
echo "=== Release $TAG complete ==="
say "phatmux release complete"
