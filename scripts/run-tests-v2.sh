...
if [ "$(id -un)" != "phatmux" ]; then
  ...
  echo "Run via: ssh phatmux-vm 'cd /Users/phatmux/GhosttyTabs && ./scripts/run-tests-v2.sh'" >&2
  exit 2
fi

cd "$(dirname "$0")/.."

DERIVED_DATA_PATH="$HOME/Library/Developer/Xcode/DerivedData/phatmux-tests-v2"
