...
# This runner is intended for the UTM macOS VM (ssh phatmux-vm).
# It is intentionally guarded so we don't accidentally kill the host user's phatmux instances.
if [ "$(id -un)" != "phatmux" ]; then
  echo "ERROR: This script is intended to be run on the phatmux-vm (user: phatmux)." >&2
  echo "Run via: ssh phatmux-vm 'cd /Users/phatmux/GhosttyTabs && ./scripts/run-tests-v1.sh'" >&2
  exit 2
fi

cd "$(dirname "$0")/.."

DERIVED_DATA_PATH="$HOME/Library/Developer/Xcode/DerivedData/phatmux-tests-v1"
APP="$DERIVED_DATA_PATH/Build/Products/Debug/phatmux DEV.app"
RUN_TAG="tests-v1"
...
if [ ! -d "$APP" ]; then
  echo "ERROR: phatmux DEV.app not found at expected path: $APP" >&2
  exit 1
fi

cleanup() {
  pkill -x "phatmux DEV" || true
  pkill -x "phatmux" || true
  rm -f /tmp/phatmux*.sock || true
}
...
launch_and_wait() {
  ...
  for _ in {1..50}; do
    pgrep -x "phatmux DEV" >/dev/null 2>&1 || break
    sleep 0.1
  done

  # Force socket mode for deterministic automation runs, independent of prior user settings.
  defaults write com.phatmux.app.debug socketControlMode -string full >/dev/null 2>&1 || true

  # Launch directly with UI test mode enabled so startup follows deterministic test codepaths.
  PHATMUX_TAG="$RUN_TAG" PHATMUX_UI_TEST_MODE=1 "$APP/Contents/MacOS/phatmux DEV" >/dev/null 2>&1 &

  SOCK=""
  for _ in {1..120}; do
    SOCK=$(ls -t /tmp/phatmux-debug*.sock /tmp/phatmux*.sock 2>/dev/null | head -1 || true)
  ...
  if [ -z "$SOCK" ] || [ ! -S "$SOCK" ]; then
    echo "ERROR: Socket not ready (looked for /tmp/phatmux*.sock)" >&2
    exit 1
  fi
  export PHATMUX_SOCKET_PATH="$SOCK"
  export PHATMUX_SOCKET="$SOCK"

  # Ensure LaunchServices has a visible/main window attached for rendering checks.
  PHATMUX_TAG="$RUN_TAG" open "$APP" >/dev/null 2>&1 || true
  sleep 0.5
...
from phatmux import phatmux  # type: ignore
