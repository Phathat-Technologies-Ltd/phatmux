...
Builds phatmuxd-remote release assets for the supported remote platforms and emits:
  phatmuxd-remote-<goos>-<goarch>
  phatmuxd-remote-checksums.txt
  phatmuxd-remote-manifest.json
EOF
}
...
if ! command -v go >/dev/null 2>&1; then
  echo "error: go is required to build phatmuxd-remote release assets" >&2
  exit 1
fi
...
rm -f "$OUTPUT_DIR"/phatmuxd-remote-* "$OUTPUT_DIR"/phatmuxd-remote-checksums.txt "$OUTPUT_DIR"/phatmuxd-remote-manifest.json
...
CHECKSUMS_ASSET_NAME="phatmuxd-remote-checksums.txt"
CHECKSUMS_PATH="${OUTPUT_DIR}/${CHECKSUMS_ASSET_NAME}"
MANIFEST_PATH="${OUTPUT_DIR}/phatmuxd-remote-manifest.json"
...
: > "$CHECKSUMS_PATH"
ENTRIES_FILE="$(mktemp "${TMPDIR:-/tmp}/phatmuxd-remote-entries.XXXXXX")"
trap 'rm -f "$ENTRIES_FILE"' EXIT
: > "$ENTRIES_FILE"

for target in "${TARGETS[@]}"; do
  read -r GOOS GOARCH <<<"$target"
  ASSET_NAME="phatmuxd-remote-${GOOS}-${GOARCH}"
...
echo "Built phatmuxd-remote assets in ${OUTPUT_DIR}"
