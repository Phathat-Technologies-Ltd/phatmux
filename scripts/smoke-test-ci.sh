...
SOCKET_PATH="/tmp/phatmux-debug.sock"
STABILITY_WAIT=15

echo "=== Smoke Test ==="

# --- Find the built app ---
APP=$(find ~/Library/Developer/Xcode/DerivedData -path "*/Build/Products/Debug/phatmux DEV.app" -print -quit 2>/dev/null || true)
if [ -z "$APP" ]; then
  echo "ERROR: Built app not found in DerivedData"
  exit 1
fi
echo "App: $APP"
BINARY="$APP/Contents/MacOS/phatmux DEV"
if [ ! -x "$BINARY" ]; then
...
rm -f "$SOCKET_PATH"
pkill -x "phatmux DEV" 2>/dev/null || true
sleep 1

# --- Launch the app directly (not via `open`, which can silently fail on CI) ---
echo "Launching app..."
PHATMUX_SOCKET_MODE=allowAll PHATMUX_UI_TEST_MODE=1 "$BINARY" > /tmp/phatmux-smoke-stdout.log 2>&1 &
APP_PID=$!
...
if ! kill -0 "$APP_PID" 2>/dev/null; then
  ...
  echo "--- stdout/stderr ---"
  cat /tmp/phatmux-smoke-stdout.log 2>/dev/null | tail -50 || true
  echo "--- debug log ---"
  tail -50 /tmp/phatmux-debug.log 2>/dev/null || true
  echo "--- crash reports ---"
  ls -lt ~/Library/Logs/DiagnosticReports/*phatmux* 2>/dev/null | head -5 || echo "(none)"
  exit 1
fi
...
if [ "$SOCKET_READY" != "true" ]; then
  ...
  echo "--- stdout/stderr ---"
  cat /tmp/phatmux-smoke-stdout.log 2>/dev/null | tail -30 || true
  echo "--- debug log ---"
  tail -30 /tmp/phatmux-debug.log 2>/dev/null || true
  ls -la /tmp/phatmux-debug* 2>/dev/null || true
  pgrep -la "phatmux" || echo "No phatmux processes found"
  exit 1
fi
