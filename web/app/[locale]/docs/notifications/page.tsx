  ...
  return (
    <>
      ...
      <table>
        ...
        <tbody>
          <tr>
            <td><code>PHATMUX_NOTIFICATION_TITLE</code></td>
            <td>{t("envTitle")}</td>
          </tr>
          <tr>
            <td><code>PHATMUX_NOTIFICATION_SUBTITLE</code></td>
            <td>{t("envSubtitle")}</td>
          </tr>
          <tr>
            <td><code>PHATMUX_NOTIFICATION_BODY</code></td>
            <td>{t("envBody")}</td>
          </tr>
        </tbody>
      </table>
      <CodeBlock title="Examples" lang="bash">{`# Text-to-speech
say "$PHATMUX_NOTIFICATION_TITLE"

# Custom sound file
afplay /path/to/sound.aiff

# Log to file
echo "$PHATMUX_NOTIFICATION_TITLE: $PHATMUX_NOTIFICATION_BODY" >> ~/notifications.log`}</CodeBlock>
      <p>{t("customCommandNote")}</p>

      <h2>{t("sending")}</h2>

      <h3>{t("cli")}</h3>
      <CodeBlock lang="bash">{`phatmux notify --title "Task Complete" --body "Your build finished"
phatmux notify --title "Claude Code" --subtitle "Waiting" --body "Agent needs input"`}</CodeBlock>
...
printf '\\e]99;i=1;e=1;d=1;p=body:All tests passed\\e\\\\'`}</CodeBlock>
      ...
      <CodeBlock title="~/.claude/hooks/phatmux-notify.sh" lang="bash">{`#!/bin/bash
# Skip if not in phatmux
[ -S /tmp/phatmux.sock ] || exit 0
...
case "$EVENT_TYPE" in
    "Stop")
        phatmux notify --title "Claude Code" --body "Session complete"
        ;;
    "PostToolUse")
        [ "$TOOL" = "Task" ] && phatmux notify --title "Claude Code" --body "Agent finished"
        ;;
esac`}</CodeBlock>
      <CodeBlock lang="bash">{`chmod +x ~/.claude/hooks/phatmux-notify.sh`}</CodeBlock>
    ...
    "Stop": [
      {
        ...
        "hooks": [
          {
            "type": "command",
            "command": "~/.claude/hooks/phatmux-notify.sh"
          }
        ]
      }
    ],
...
notify-after() {
  ...
  if [ $exit_code -eq 0 ]; then
    phatmux notify --title "✓ Command Complete" --body "$1"
  else
    phatmux notify --title "✗ Command Failed" --body "$1 (exit $exit_code)"
  fi
  return $exit_code
}
